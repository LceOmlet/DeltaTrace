...
For a signed source vector $A$, RISE uses descending signed scores $A_i$, while MAS and recovery use $\max(A_i,0)$. The original evaluator produces 21 response values across 20 deletions. Saved signed deletion curves supply RISE; when the positive and signed views have an identical ranked prefix until the monotone normalized response reaches zero, the recorded prefix proof permits reuse of that RISE value. MAS always comes from the positive view. The illustrative curves in Figures~\ref{fig:retrieval} and~\ref{fig:multihop} retain positive-ranking DT deletion and add the matching one-hop FT curve for each model. The score sums token log-probabilities over the entire fixed response plus EOS. Within each model and example, DT and FT share identical full-input and fully-deleted scores. The released evaluator normalizes by these endpoints, clips to $[0,1]$, and takes the cumulative minimum.

\paragraph{Published reference results.}
The full Qwen3 comparison quotes the authors' released task CSVs. The Perturbation, REAGENT, CLP, IFR, and AttnLRP entries reproduce the aggregates reported in the original Tables 1 and 2. Each value is matched back to its CSV and aggregation row at full precision: these use the row-attribution mean, with the published IFR MQ-Q8 faithfulness pair using the recursive-attribution mean. The original fast perturbation variants are retained outside MQ-Q2 and MoreHopQA. All 95 additional source CSVs are verified against the released archive, and all 160 added metric values agree with the printed tables to their reporting precision. Baseline target aggregation follows the publication; deletion faithfulness evaluates the complete stored response. Their accompanying per-example \texttt{n1} trace records reproduce the CSV means; input text, target, eligible positions, and gold mappings were checked against each paired \dt{} example. Thus the full table identifies FlashTrace by its published result and matched archive trace. The separately executed development comparison uses one recursive hop for faithfulness and three for recovery, as stated in Table~\ref{tab:development}. Qwen3.5 uses the same fixed inputs and responses with both methods evaluated on the same model.
...
The two examples below preserve their full response targets, original token scores, and recorded deletion trajectories. A common signed color scale is used within each example; the legend sits below the deletion panel.
\input{figures/cases}
