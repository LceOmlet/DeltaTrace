...
\centering
\small
\caption{Fixed propagation rules. Bars denote endpoint averages. Common attention and MLP operations share their rules across architectures; every GDN block uses the same symmetric memory construction.}
\label{tab:rules}
...
Attention output gating & $\D(ns)=s_1\odot\D n+n_0\odot\D s$ \\
GDN output gating & $\D(ns)=\bar s\odot\D n+\bar n\odot\D s$ \\
...
\end{align}
The query includes the model's fixed attention scale. The state $T_t$ contains retained evidence, $r_t$ reads existing content, and $u_t$ writes the update. One endpoint orientation gives the following finite recurrence:
\begin{align}
...
\end{align}
Let $D^{01}$ denote the linear map obtained by composing this recurrence over the memory block. Swapping endpoints gives $D^{10}$: both the input and output differences change sign, so $D^{10}$ also maps the original input difference to the original output difference. The memory rule is
\begin{equation}
D_{\mathrm{mem}}=\tfrac12(D^{01}+D^{10}),\qquad
m_{\mathrm{in}}=\tfrac12\bigl((D^{01})^\top m_{\mathrm{out}}+(D^{10})^\top m_{\mathrm{out}}\bigr).
\label{eq:gdn-symmetric}
\end{equation}
The average is taken after composing each ordered recurrence. It assigns contributions to values, keys, queries, and both gates while preserving the finite identity by linearity. The same rule applies to every GDN layer. Scalar gate nonlinearities and query/key normalization use the finite construction of the surrounding network. For the subsequent normalized-content/output-gate product, $n\odot s$ with $s=\silu(z)$, the rule is $\D(n\odot s)=\bar s\odot\D n+\bar n\odot\D s$.
...
\paragraph{Memory.}
The implementation processes 64-token chunks using FLA's native chunk-state adjoints and batched contractions with saved endpoint states. It evaluates both orders of the existing endpoint captures and averages their memory coefficients as in Equation~\ref{eq:gdn-symmetric}. An associative affine scan computes the finite decay contribution. Convolution and surrounding projections use the installed model primitives and their linear transpose actions.
...
\paragraph{Execution.}
Qwen3 uses FP16; Qwen3.5 uses BF16 attention and FLA, with FP32 accumulation where supplied by the underlying operations.
...
\paragraph{Replay retention.}
On the Qwen3-8B development examples, retaining replay reduces warm attribution time by 5.80\% (Table~\ref{tab:retention}).
