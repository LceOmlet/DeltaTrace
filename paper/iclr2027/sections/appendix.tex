...
\end{proof}
The operator is symmetric. For incoming coefficients $m_p$ at probabilities $p$, the coefficient at query--key score $z_i$ is
\begin{equation}
...
\label{app:operators}
For the attention example with output $Y=PV$, Equation~\ref{eq:pv} gives the coefficients $m_V=\bar P^\top m_Y$ and $m_P=m_Y\bar V^\top$ for an incoming coefficient matrix $m_Y$.
