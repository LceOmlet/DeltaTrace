...
We use the two-run means so that the weighted input changes sum exactly to the observed output change. Let $\D P$, $\D V$, and $\D Y$ denote the differences in weights, values, and output between the two executions. Splitting the interaction term $\D P\,\D V$ equally between the inputs gives equal weight to the two possible change orders:
\begin{equation}
\begin{aligned}
\D Y
...
\label{eq:pv}
\end{equation}
Here $\bar P$ and $\bar V$ are the means of the original and reference weights and values. They provide the two sets of local slopes in $D_f$ (Equation~\ref{eq:reverse-contribution}): the mean weights multiply the value changes, and the mean values multiply the weight changes.
