...
def draw(data):
    ...
    fig._dt_case={'model':case['model'],'dataset':case['dataset'],'index':case['index'],
        ...
        'trace_directions':{'gray_upward':'two forward executions with the same fixed response',
                            'purple_downward':'one reverse traversal of effective secant slopes, seeded with 1 at the scalar score'},
        'minimum_label_pt_at_manuscript_width':20.4*5.5/W,
        'reading_order':['two forward executions identify the observed changes','operation-specific backward calculations connect changes to the response score','effective secant slopes reach the input embeddings','multiply displacements by slopes and sum within each token'],
        'phase_notation':{'activation_differences':'Delta, from the two forward executions','reverse_coefficients':'m, effective secant slopes under the fixed local allocation; subscript names the associated activation','token_score':'A_i = <m_(e_i), Delta e_i>'},
