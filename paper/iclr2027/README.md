...
当前稿件先定义输入、固定回答、参考输入和归因目标，再解释有限反传如何追踪内容、选择和记忆。正文保留5个核心公式，以表1并列普通反传和有限反传的变化关系、反向计算，并在注意力例子中给出对应的反向系数。主图和方法节沿“两次前向、局部规则、反向系数、输入端计分”的顺序解释计算，局部变化恒等式与反向实际返回的系数配对展示。定理、证明、完整算子规则和实现细节集中到附录。正式实验的数值段落由独立结果文件接入。排版稿保存在`output/pdf/deltatrace-iclr2027-draft.pdf`，页数与生成记录见`build_receipt.json`。
