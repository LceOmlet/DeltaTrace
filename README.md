...
**最新根因：两种softmax修正均未获得可推广的修复证据。** Euclidean规则使NI整网MAS退步；概率差规则局部中段4.99→4.62但早段2.80→2.93，停止扩跑。第0层已验证修复保留；下一步定位当前NI剩余最大实际错配。[连续结论](docs/history/MAS修复进展_两种softmax修正的边界_20260908.md)。
