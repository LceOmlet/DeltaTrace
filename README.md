...
**当前工作版本：按用户决定暂将DT视为胜出，FT固定。** 同输入NI0：DT37.5%，原FT10%/15%/15%/15%；DT诊断归因22.89秒、峰值allocated20.12GB。RISE/MAS尚未测。[DT同原版FT输入实测与优化优先级_20260908.md](docs/history/DT同原版FT输入实测与优化优先级_20260908.md)；[有明确原因的优化顺序](docs/dt_optimization_priorities_20260908.md)。
