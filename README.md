...
**当前结果：完整未修改官方FT已完成NI0，0跳10%，1—3跳15%。** 当前作者默认输入为588token，旧DT为605；先对齐DT输入，不用旧结果判胜负。原版FT保持不动。[完整原版FT首次实测与输入契约纠正_20260908.md](docs/history/完整原版FT首次实测与输入契约纠正_20260908.md)。
