...
**最新结果：Qwen3开发集综合领先；Qwen3.5的MAS仍落后。** 官方输出头选行已实测并设为默认，四次质量保持、输出减少337.7MB，但无明显整网速度或全程峰值收益。下一大头为重放/捕获/搬运及batch，质量先定位条件误差。[原生输出头选行配对与下一大头_20260908.md](docs/history/原生输出头选行配对与下一大头_20260908.md)。
