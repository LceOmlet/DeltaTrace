...
最新公平性修正：[原FT seq入口](docs/history/原FT_seq入口公平性修正_20260907.md)。用作者原函数省去无关row/rec视图后，16例完整FT分数相同、耗时约降30.4%；P1相对该入口反而慢37.7%（配对中位数，16/16均慢）。此前0.928仅为相对作者总入口，不构成效率胜出。下一步保留P1，优先省去无用输出头计算并减少小算子与同步；先小预算筛选，再按需要验证B4。
