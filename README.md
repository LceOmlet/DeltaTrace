...
**最新根因：FA19的主要剩余错配已定位到softmax条件斜率。** 中段路由过估4.54中，softmax项4.42、QK约0.13，系数重放差为0。下一唯一候选以最小改动保留原输入局部响应并满足端点守恒，复用FA框架及原GEMM次数；尚未声称候选有效。[实测与修法](docs/history/MAS根因与修法_softmax条件斜率_20260908.md)。
