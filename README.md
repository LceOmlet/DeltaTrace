...
**当前执行目标：**[当前执行目标](docs/current_goal.md)。当前固定完整原版FT，以暂定胜出的DT版本推进质量与效率优化；默认 FA/真实 FLA、批处理和性能约束继续有效，沿用官方评测并持续提交推送。应用 Goal 正文暂不能通过工具改写，后续执行以用户最新指令及该文件为准。以下为历史进展，旧优先级和固定显存容差不再作为当前约束，历史已执行协议和失败结果不改判。

**最新结果：第0层norm-gate修复已通过正常路径原指标与成本检查。** NI MAS 0.2888→0.1986、needle 92.1%→97.4%；MH同次原评分MAS 0.3183→0.2456，两例RISE改善。NI耗时+0.68%、峰值+56.53 MiB，FA/FLA调用不增加。MH新评分与历史FT不可直接比较；Goal继续定位剩余错配。[MAS条件错配修复_第0层normgate_20260908.md](docs/history/MAS条件错配修复_第0层normgate_20260908.md)。
