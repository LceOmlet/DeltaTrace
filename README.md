...
**当前执行目标：**[当前执行目标](docs/current_goal.md)。先量化剩余 FA 收益，有依据地转向其他主要耗时；完成 Qwen3.5-9B/真实 FLA 的归因与批处理支持，沿用官方评测并持续提交推送。应用 Goal 正文暂不能通过工具改写，后续执行以用户最新指令及该文件为准。以下为历史进展，旧优先级和固定显存容差不再作为当前约束，历史已执行协议和失败结果不改判。
