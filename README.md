...
**最新根因：FA19中段主要是条件路由过估，MLP6主要是乘积交互。** 原生FA拆分和真实MLP系数核对已完成；两个MLP端点顺序没有共同改善方向，停止整网扩跑。继续在现有FA扩展中区分QK交互与softmax有限斜率，生产规则和FT保持原状。[计算证据](docs/history/MAS计算根因_FA路由与MLP交互_20260908.md)。
