...
**当前执行目标：**[当前执行目标](docs/current_goal.md)。当前先定位 FT/DT 在 Qwen3.5 上共同丢失证据恢复能力的原因，核查 DT 目标迁移；默认 FA/真实 FLA、批处理和性能约束继续有效，沿用官方评测并持续提交推送。应用 Goal 正文暂不能通过工具改写，后续执行以用户最新指令及该文件为准。以下为历史进展，旧优先级和固定显存容差不再作为当前约束，历史已执行协议和失败结果不改判。

**当前结论：FT 和 DeltaTrace 的 Qwen3.5 版本均未通过首个 NI0 证据恢复验收。** 当前DT为1/40，FT为2—3/40；离线分析已定位DT整段轨迹→条件答案的目标变化，FT具体原因仍待定位。暂停97次B2删除评测，先做一次目标迁移回归控制。[Qwen35共同归因失败与目标错位诊断_20260908.md](docs/history/Qwen35共同归因失败与目标错位诊断_20260908.md)。
