...
**最新纠正：停止修改官方 FT，撤销自定义9B FT变体的官方基线资格。** 现有core0—3和全轨迹诊断改动了内容分配，不能代表完整原版；未执行的Both控制器草稿已撤出运行目录。先核对完整未修改官方入口及实际运行证据，不继续改FT后自证正确。DT目标回归实测保留，但不能据此判断对官方FT的胜负。见[纠正记录](docs/history/停止FT改造并纠正对照身份_20260908.md)。
