"""Screen uniform DT finite-rule variants under the unchanged paper objective."""
...
def main():
    ...
    for name in ('run_variants.py','dt_variants.py','run_causal.py'):(a.output/name).write_bytes(Path(__file__).with_name(name).read_bytes())
    ...
    try:
        ...
        for ordinal,(task,index,ex,old) in enumerate(examples):
            ...
            for name in plan['methods']:
                ...
                case['methods'][name]['recall']=float(ft.evaluate_attr_recovery_skip_tokens(torch.tensor(vectors[name])[None],
            ...
            if old:
                with np.load(full/task/'vectors.npz',allow_pickle=False) as vv:
                    ...
                    assert case['archive_dt_relative_l2']<1e-4
                    vectors['FT_K1']=vv[key+'_FT_K1_prompt'].copy();vectors['FT_K3']=vv[key+'_FT_K3_prompt'].copy()
                case['FT_K1']=dict(rise=old['metrics']['FT_K1']['rise'],mas=old['metrics']['FT_K1']['mas'],recall=old['metrics']['FT_K1']['needle'])
                case['FT_K3_recall']=old['FT_K3_needle']
            else:
                ...
                case['FT_K3_recall']=float(ft.evaluate_attr_recovery_skip_tokens(torch.tensor(vectors['FT_K3'])[None],
                    keep_prompt_token_indices=keep,gold_prompt_token_indices=gold,top_fraction=.1))
            ...
            ft_recall=float(ft.evaluate_attr_recovery_skip_tokens(torch.tensor(vectors['FT_K1'])[None],
                    keep_prompt_token_indices=keep,gold_prompt_token_indices=gold,top_fraction=.1))
            if old:
