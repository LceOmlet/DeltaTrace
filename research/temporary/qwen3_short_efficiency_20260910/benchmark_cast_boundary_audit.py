...
def main():
    ...
    try:
        ...
        from qwen3_root_cast_audit import propagate_root_tape as propagate_cast
        import qwen3_cast_boundary_audit as boundary_audit
        ...
        for input_len in map(int,args.lengths.split(',')):
            ...
            def make_runner():
                ...
                def call():
                    ...
                    if args.family=='qwen3':
                        ...
                        try:
                            ...
                            boundary_audit.enabled=phase=='audit' and mode=='root_cast'
                            if boundary_audit.enabled:boundary_audit.records.clear()
                ...
                return call
            modes=['retained','root_rope','root_cast']
            measured=modes
            ...
            if last_ok:
                for mode in modes:
                    ...
                    if mode=='root_cast':case['boundary_audit']=list(boundary_audit.records)
                ...
                # Diagnostic run intentionally preserves the known nonexact candidate; no adoption gate here.
            del ids,mask,engine
            gc.collect();torch.cuda.empty_cache()
