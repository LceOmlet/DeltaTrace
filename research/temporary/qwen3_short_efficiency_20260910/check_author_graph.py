...
def main():
    ...
    A=p.parse_args().root;R=A/'graph_production_release';B=Path('/tmp/codex_short_b1_efficiency_20260910_v1')
    plan_path=A/'author_graph_plan.json';plan=json.loads(plan_path.read_bytes());out=A/'author_graph';out.mkdir(exist_ok=False)
    ...
    try:
        ...
        os.environ.update(MACA_PATH='/opt/maca',HF_HUB_OFFLINE='1',TOKENIZERS_PARALLELISM='false',
            TRITON_CACHE_DIR=str(A/'triton_confirmation_v4'),TORCHINDUCTOR_CACHE_DIR=str(A/'inductor_confirmation_v4'))
        ...
        new,report['root_sources']=make_graphed_qwen3(R,model,env['finite_library'],env['finite_library_sha256'])
