...
class GraphValidation(BatchedValidation):
    def predicate(self,value,label):
        assert value.dtype==torch.bool and value.numel()==1
        handle=_Scalar();self.flag_batches.append(value.reshape(1))
        self.flags.append(handle);self.labels.append(label);return handle
