    ...
    try:
        ...
        for input_len in map(int,args.lengths.split(',')):
            ...
            def make_runner():
                ...
                def call():
                    ...
                    if args.family=='qwen3':
                        ...
                        tape=(LocalNativeRootTape if mode=='root_local' else NativeRootTape)(model,mutation_audit=phase=='audit')
                        try:
            ...
            if last_ok:
                ...
                phase='profile';mode='root_local';runner=make_runner()
                ...
                with torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CPU,torch.profiler.ProfilerActivity.CUDA],record_shapes=True) as prof:
                    cpu.enable();profile_result=runner();torch.cuda.synchronize();cpu.disable()
                report['profile_seconds']=time.perf_counter()-started
                prof.export_chrome_trace(str(out/'trace.json'))
                (out/'cpu_profile.txt').write_text('')
                stream=io.StringIO();pstats.Stats(cpu,stream=stream).strip_dirs().sort_stats('tottime').print_stats(120)
                (out/'cpu_profile.txt').write_text(stream.getvalue())
                (out/'torch_profile.txt').write_text(prof.key_averages().table(sort_by='self_cuda_time_total',row_limit=100))
                del profile_result,runner,prof,cpu
            if last_ok:
                for mode in modes:
                    phase='audit'
                    ...
                    del result,vec
                    identity();save()
                reference=vectors[str(input_len)+'_root_local']
