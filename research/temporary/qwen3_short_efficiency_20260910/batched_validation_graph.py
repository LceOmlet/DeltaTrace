...
def _group(equal_pairs,finite_values,positive_values,maximum_values,rope_values):
    ...
    for qnorm,knorm,qs,ks,cos,sin in rope_values:
        q,k=apply_rotary_pos_emb(qnorm.transpose(1,2),knorm.transpose(1,2),cos,sin)
        predicates.extend(((q==qs.transpose(1,2)).all(),(k==ks.transpose(1,2)).all()))
...
compiled_group=torch.compile(_group,fullgraph=True,dynamic=True,
  options={'triton.cudagraphs':True,'max_autotune':False,'emulate_precision_casts':True})
...
class BatchedValidation:
    ...
    def rope(self,qnorm,knorm,qs,ks,cos,sin,label_q,label_k):
        q=_Scalar();k=_Scalar()
        self.pending.append(('rope',(qnorm,knorm,qs,ks,cos,sin),(label_q,label_k),(q,k)))
        return q,k

    def flush(self):
        if not self.pending:return
        groups=[[x for x in self.pending if x[0]==kind] for kind in ['equal','finite','positive','maximum','rope']]
        ...
        # Own the tiny outputs before the next graph replay can reuse its storage.
        predicates=predicates.clone();statistics=statistics.clone()
        ...
        for row in groups[4]:
            self.flags.extend(row[3]);self.labels.extend(row[2])
