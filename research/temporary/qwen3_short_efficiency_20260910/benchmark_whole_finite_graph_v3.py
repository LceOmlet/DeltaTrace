...
def main():
    ...
    report={'status':'imports','family':args.family,'method':args.method,
      ...
      'purpose':'Qwen3 root retention plus compiled grouped exact diagnostics; all finite formulas and checks preserved',
      ...
      'warm_timing_scope':'One warm call per mode/length, then three rounds in forward/reverse/forward mode order. Original full-call timer and reset.',
    ...
    try:
        ...
        from qwen3_root_compiled import propagate_root_tape as propagate_compiled
        from qwen3_root_rope import propagate_root_tape as propagate_rope
        from qwen3_root_cast import propagate_root_tape as propagate_cast
        from qwen3_finite_graph_v3 import FiniteGraphQwen3
        graph_runner=FiniteGraphQwen3(model,finite_fa)
        from compiled_fa_runtime import CompiledInputsFA
        compiled_fa=CompiledInputsFA(env["finite_library"],env["finite_library_sha256"])
        from qwen3_root_tape_batched import propagate_root_tape as propagate_batched_root_tape
        from batched_validation import verify_rejection_and_statistics
        check_started=time.perf_counter()
        report['batched_validation_checks']=verify_rejection_and_statistics()
        report['batched_validation_check_seconds']=time.perf_counter()-check_started
        vectors={}
        for input_len in map(int,args.lengths.split(',')):
            ...
            def make_runner():
                ...
                def call():
                    ...
                    if args.family=='qwen3':
                        if mode=='root_whole_graph':return graph_runner.attribute(base_ids,actual,attention,p_len,mutation_audit=phase=='audit')
                        if mode=='retained':
                        ...
                        tape=(LocalNativeRootTape if mode in ('root_local','root_compiled','root_rope','root_cast') else NativeRootTape)(model,mutation_audit=phase=='audit')
                        try:
                            with tape:before,after=capture_checkpoint_pair(model,base_ids,actual,attention,p_len)
                            compiled_fa.audit_inputs=phase=='audit'
                            if mode=='root_cast':return propagate_cast(model,before,after,tape,pv_rule='content_P1',finite_attention=compiled_fa)
                            if mode=='root_rope':return propagate_rope(model,before,after,tape,pv_rule='content_P1',finite_attention=compiled_fa)
                            if mode=='root_compiled':return propagate_compiled(model,before,after,tape,pv_rule='content_P1',finite_attention=compiled_fa)
                            return (propagate_batched_root_tape if mode in ('root_batched','root_local') else propagate_root_tape)(model,before,after,tape,pv_rule='content_P1',finite_attention=finite_fa)
                        finally:tape.clear()
                ...
                return call
            modes=['retained','root_rope','root_whole_graph']
            ...
            if last_ok:
                ...
                math_keys=['target_delta_score32_sum64','target_delta_score16','signed_sum','unassigned_total','layer_checks']
                case['math_diagnostics_equal']={m:all(case['details'][m][k]==case['details']['retained'][k] for k in math_keys) for m in modes}
                assert all(case['candidate_vectors_equal'].values()),case['candidate_vectors_equal']
                assert all(case['math_diagnostics_equal'].values()),case['math_diagnostics_equal']
            if input_len==128:
                altered=ids.clone();altered[0,eligible[0]]=tokenizer.eos_token_id
                altered_base=altered.clone();altered_base[0,eligible]=tokenizer.eos_token_id
                t=time.perf_counter()
                new_result=graph_runner.attribute(altered_base,altered,mask,prompt_len,mutation_audit=True)
                b,a=capture_checkpoint_pair(model,altered_base,altered,mask,prompt_len)
                old_result=propagate(model,b,a,pv_rule='content_P1',finite_attention=finite_fa)
                torch.cuda.synchronize()
                nv=np.asarray(new_result['signed_full_sequence']);ov=np.asarray(old_result['signed_full_sequence'])
                vectors['same_shape_changed_input_root']=nv;vectors['same_shape_changed_input_retained']=ov
                report['same_shape_refresh_probe']={'seconds':time.perf_counter()-t,'exact':bool(np.array_equal(nv,ov)),
                    'input_differs':not torch.equal(ids,altered),'vector_differs':not np.array_equal(nv,vectors['128_retained']),
                    'graph_rebuilt':new_result['native_graph_execution']['build_this_call'] is not None,
                    'input_sha256':sha(altered.cpu().numpy().tobytes()),'details':new_result}
                np.savez(out/'vectors.npz',**vectors);save()
                assert report['same_shape_refresh_probe']['exact'] and report['same_shape_refresh_probe']['input_differs'] and report['same_shape_refresh_probe']['vector_differs'] and not report['same_shape_refresh_probe']['graph_rebuilt']
                phase='profile';runner=make_runner();mode='root_whole_graph';t=time.perf_counter()
                with torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CPU,torch.profiler.ProfilerActivity.CUDA]) as prof:
                    profile_result=runner();torch.cuda.synchronize()
                report['whole_graph_profile_seconds']=time.perf_counter()-t
                report['whole_graph_profile_events']=[{'key':e.key,'count':e.count} for e in prof.key_averages() if 'graph' in e.key.lower() or 'flash_fwd' in e.key or 'deltatrace_fa_finite_p1_kernel' in e.key]
                (out/'whole_graph_profile.txt').write_text(prof.key_averages().table(sort_by='self_cuda_time_total',row_limit=70))
                del new_result,old_result,b,a,nv,ov,altered,altered_base,profile_result,runner,prof;save()
            del ids,mask,engine
            gc.collect();torch.cuda.empty_cache()
