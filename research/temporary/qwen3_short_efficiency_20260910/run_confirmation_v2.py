...
env=dict(os.environ,MACA_PATH='/opt/maca',OMP_NUM_THREADS='4',PYTHONDONTWRITEBYTECODE='1',TRITON_CACHE_DIR=str(T/'triton_confirmation_v2'),TORCHINDUCTOR_CACHE_DIR=str(T/'inductor_confirmation_v2'))
 ...
 for round_index,methods in enumerate(P['method_rounds']):
  for method in methods:
   ...
   with (out/(name+'.log')).open('w') as log:
    command=['/mnt/geogpt-doc-new/deepresearch/lzq/contrastive_flashtrace_probe/env/bin/python','-u',str(T/'benchmark_confirmation_v2.py'),'--root','/tmp/codex_short_b1_efficiency_20260910_v1','--family','qwen3','--method',method,'--lengths',','.join(map(str,P['length_orders'][round_index])),'--protocol',str(T/'confirmation_v2_protocol.json'),'--output',str(out/name)]
