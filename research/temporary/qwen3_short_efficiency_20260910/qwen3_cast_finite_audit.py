from qwen3_cast_boundary_audit import attention_layout, norm_residual_two, norm_residual_three, qk_norms
...
from qwen3_cast_boundary_audit import norm as rmsnorm_secant_pullback
...
def propagate_signed_secant(model, before, after, variant='rescale', progress=None, pv_rule='content_P1', finite_attention=None, finite_activity=None,validation=None):
    ...
    def norm_back(module, x0, x1, m):
        answer = rmsnorm_secant_pullback(x0, x1, module.weight, m, module.variance_epsilon)
        return answer
    ...
    with torch.no_grad():
        ...
        del seed, z0, z1
        m = norm_back(model.model.norm, before['last'], after['last'], m_final)
        del m_final
        for li in reversed(range(len(model.model.layers))):
            layer = model.model.layers[li]
            raw0 = before['layers'][li]
            raw1 = after['layers'][li]
            a = {k: raw0[k] for k in ['kpre', 'mid', 'qpre', 'x']}
            b = {k: raw1[k] for k in ['kpre', 'mid', 'qpre', 'x']}
            ...
            mqp,mkp = qk_norms(a['qpre'],b['qpre'],layer.self_attn.q_norm.weight,mqn,layer.self_attn.q_norm.variance_epsilon,
                a['kpre'],b['kpre'],layer.self_attn.k_norm.weight,mkn,layer.self_attn.k_norm.variance_epsilon)
            ...
            m = norm_residual_three(a['x'], b['x'], layer.input_layernorm.weight, mqa, mka, mva, m_x_residual, layer.input_layernorm.variance_epsilon)
