...
save()
try:
 assert json.loads((P/'author_graph/results.json').read_bytes())['status']=='complete'
 ...
 env=json.loads((P/'graph_production_release/environment.json').read_bytes())['qwen3'];native=Path(env['finite_library']);actual=digest(native)
