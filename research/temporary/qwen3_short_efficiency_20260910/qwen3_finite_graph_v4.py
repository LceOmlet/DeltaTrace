...
from qwen3_root_tensor_graph_v3 import NativeRootTape,propagate_root_tape
...
class Program:
    def __init__(self,model,source,retained_bytes,finite):
        ...
        memo={};self.inputs=clone_tree(source,memo,self.paths)
    ...
    def refresh(self,source,*,audit=False):
        return refresh(source,self.paths,self.storage_groups,audit=audit)
    ...
    def build(self):
        ...
        with torch.no_grad(),torch.cuda.stream(stream):
            ...
            try:
                ...
                traceback.print_exc();self.graph=None;self.packed=None;raise
        ...
        self.build_info={'seconds':time.perf_counter()-start,'warm_finite_programs':warm,
            ...
            'static_input_tensors':len(self.paths),'static_input_bytes':sum(t.numel()*t.element_size() for _,t,_ in self.storage_groups),'static_input_storage_copies':len(self.storage_groups)}
        return self.build_info

    def replay(self,before,after,started):
        self.graph.replay()
        result=self.packed.resolve(before,after,0.0)
...
class FiniteGraphQwen3:
    ...
    def attribute(self,before_ids,after_ids,mask,prompt_len,*,mutation_audit=False):
        ...
        try:
            ...
            source=payload(before,after,tape);torch.cuda.synchronize()
            build=None
            if self.signature!=signature:
                self.program=None
                self.signature=None
                self.program=Program(self.model,source,tape.maximum_retained_bytes,self.finite)
            ...
            result['native_graph_execution']={'graph_replays':1,'geometry_cache_entries':1,'static_input_tensors':len(self.program.paths),
                'static_input_storage_copies':len(self.program.storage_groups),'every_input_tensor_refreshed':True,'attribution_results_reused':False,'build_this_call':build,
