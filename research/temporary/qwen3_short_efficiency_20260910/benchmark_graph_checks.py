...
def main():
    ...
    try:
        ...
        from qwen3_root_graph import propagate_root_tape as propagate_cast
        ...
        from batched_validation_graph import verify_rejection_and_statistics as verify_graph
        graph_check_start=time.perf_counter()
        report['graph_validation_checks']=verify_graph()
        report['graph_validation_check_seconds']=time.perf_counter()-graph_check_start
        vectors={}
        for input_len in map(int,args.lengths.split(',')):
            ...
            def make_runner():
                ...
                def call():
                    if mode=='root_graph':torch.compiler.cudagraph_mark_step_begin()
                    ...
                    if args.family=='qwen3':
                        ...
                        tape=(LocalNativeRootTape if mode in ('root_local','root_compiled','root_rope','root_graph') else NativeRootTape)(model,mutation_audit=phase=='audit')
                        try:
                            with tape:before,after=capture_checkpoint_pair(model,base_ids,actual,attention,p_len)
                            compiled_fa.audit_inputs=phase=='audit'
                            if mode=='root_graph':return propagate_cast(model,before,after,tape,pv_rule='content_P1',finite_attention=compiled_fa)
                ...
                return call
            modes=['retained','root_rope','root_graph']
            ...
            if input_len==128:
                phase='profile';mode='root_graph';runner=make_runner();started=time.perf_counter()
                with torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CPU,torch.profiler.ProfilerActivity.CUDA]) as prof:
                    prof_result=runner();torch.cuda.synchronize()
                report['graph_profile_seconds']=time.perf_counter()-started
                report['graph_profile_events']=[{'key':e.key,'count':e.count} for e in prof.key_averages() if 'graph' in e.key.lower()]
                (out/'graph_profile.txt').write_text(prof.key_averages().table(sort_by='self_cpu_time_total',row_limit=60))
                del prof_result,runner,prof;save()
            del ids,mask,engine
            gc.collect();torch.cuda.empty_cache()
