...
def main():
    ...
    try:
        ...
        is_dt=args.method=='deltatrace_root_retained'
        if is_dt:
            model.requires_grad_(False)
            if args.family=='qwen3':
                from root_retained_qwen3 import make_root_retained_qwen3
                init_start=time.perf_counter()
                dt_runner,report['acceleration_sources']=make_root_retained_qwen3(R,model,env['finite_library'],env['finite_library_sha256'])
            ...
            if last_ok:
                ...
                if is_dt:
                    ...
                    with np.load(reference_path) as reference:
                        assert np.array_equal(vec,reference[str(input_len)+'_retained'])
                    case['audit']['full_vector_exact_to_frozen_retained']=True
