...
def main():
    ...
    try:
        ...
        for case_number,c in enumerate(selected):
            ...
            for mode in (modes if case_number%2==0 else list(reversed(modes))):
                root_calls=[]
            ...
            for score_key in [m+'_'+view for m in modes for view in ('positive','signed')]:
                mode,view=score_key.rsplit('_',1)
                scores=torch.tensor(vectors[key+'_'+mode+'_positive']) if view=='positive' else torch.tensor(vectors[key+'_'+mode+'_signed_full'][positions],dtype=torch.float32)
                ...
                try:
                    with torch.no_grad():metrics=timed(key+'_'+score_key+'_original_metrics',lambda:ft.faithfulness_test_skip_tokens(
                ...
                curve['view']=view
                curve['MAS_is_valid_for_this_view']=view=='positive'
                row['metrics'][score_key]=curve;save()
