...
| --- | --- | ---: | ---: | ---: | ---: | --- |
| 最终答案 | 官方 ID，恢复正文坐标 | 48 | 68.58 | 66.15 | +2.43 | [-6.77, +12.15] |
| 最终答案 | 另修订两处关系标签 | 48 | 68.58 | 67.19 | +1.39 | [-7.64, +10.76] |
| 完整回答 | 官方 ID，恢复正文坐标 | 48 | 76.56 | 75.87 | +0.69 | [-7.64, +8.85] |
| 完整回答 | 另修订两处关系标签 | 48 | 77.08 | 77.43 | -0.35 | [-8.33, +7.47] |

## 逐项隔离修正：保持原候选集与原 10% 词元预算

下表继续计算旧式词元 Recall，因而可以隔离坐标、空格与句边界的影响。它包含标题等旧候选元数据，且允许在词元预算处截断，分母也与上面的原生整句主表不同；两表绝对值不可直接当成同一指标的提升。

| 固定目标 | 诊断视图 | DT | FT K3 | DT−FT |
| --- | --- | ---: | ---: | ---: |
| 完整回答 | 原始词元排序 + 缓存标签 | 40.63 | 45.35 | -4.71 |
| 完整回答 | 原始排序 + 正文坐标修正 | 40.60 | 45.35 | -4.74 |
| 完整回答 | 旧正则句均值 + 缓存标签 | 73.81 | 72.09 | +1.73 |
| 完整回答 | 旧正则句均值 + 正文坐标修正 | 73.81 | 72.09 | +1.73 |
| 完整回答 | 再修正前导空格归属 | 74.57 | 73.73 | +0.84 |
| 完整回答 | 再使用原生句边界（仍按词元截断） | 72.21 | 72.63 | -0.42 |
| 最终答案 | 原始词元排序 + 缓存标签 | 41.01 | 37.83 | +3.18 |
| 最终答案 | 原始排序 + 正文坐标修正 | 41.01 | 37.83 | +3.18 |
| 最终答案 | 旧正则句均值 + 缓存标签 | 65.80 | 60.94 | +4.85 |
| 最终答案 | 旧正则句均值 + 正文坐标修正 | 65.80 | 60.94 | +4.85 |
| 最终答案 | 再修正前导空格归属 | 67.37 | 61.78 | +5.60 |
| 最终答案 | 再使用原生句边界（仍按词元截断） | 65.37 | 61.51 | +3.86 |

完整回答目标下，仅换成原生句边界后，旧式词元 Recall 的 DT−FT 从旧正则视图的 +1.73 点变成 −0.42 点。可见旧分句方式确实会改变比较，不能把旧句聚合的优势直接当成方法优势。
...
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 最终答案 | DT | 39.64 | 68.58 | 47.58 | 6.25 | 33.33 | 114.71 | 2.92 | 5.02 |
| 最终答案 | FT K1 | 39.00 | 65.45 | 46.45 | 2.08 | 27.08 | 114.00 | 3.62 | 4.92 |
| 最终答案 | FT K3 | 41.12 | 66.15 | 47.78 | 2.08 | 29.17 | 114.10 | 3.52 | 4.83 |
| 完整回答 | DT | 43.85 | 76.56 | 53.37 | 6.25 | 43.75 | 114.46 | 3.17 | 5.02 |
| 完整回答 | FT K1 | 44.73 | 74.83 | 53.29 | 6.25 | 43.75 | 114.10 | 3.52 | 4.90 |
| 完整回答 | FT K3 | 47.27 | 75.87 | 55.03 | 6.25 | 45.83 | 114.21 | 3.42 | 4.77 |

答案目标的 DT Recall 略高，但其 F1（47.58%）低于 FT K3（47.78%）；修订两处标签后 FT K3 的 F1 为 48.30%。因此不把单列 Recall 均值当作整体优胜。
...
| --- | --- | ---: | ---: | ---: | --- |
| 最终答案 | 官方 ID，恢复正文坐标 | 69.26 | 68.33 | +0.93 | [-6.67, +8.52] |
| 最终答案 | 另修订两处关系标签 | 69.26 | 69.44 | -0.19 | [-7.41, +7.22] |
| 完整回答 | 官方 ID，恢复正文坐标 | 76.67 | 76.48 | +0.19 | [-6.67, +7.04] |
| 完整回答 | 另修订两处关系标签 | 77.22 | 78.15 | -0.93 | [-7.41, +5.74] |
