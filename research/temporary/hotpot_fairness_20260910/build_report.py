...
TARGET={'answer_only':'最终答案','full':'完整回答'}
...
def main():
    ...
    lines=['# HotpotQA 公平性修正：完整 48 例', '',
        '已修正可复现的标签坐标错误与分句归属错误，并建立双方共用的原生支持句评测。最终答案目标的主结果为 **DT 68.58%、FT K3 66.15%**；两处语义标签也修订后为 **68.58% 对 67.19%**。四项主比较的区间均跨零，不能确认 DT 的稳定优势。完整回答目标及不利结果全部保留。', '',
        '本次新增重跑全部 48 例的答案目标 DT、live FT K1/K3，完整回答使用已验证的 48 例原始向量。候选构造不读取归因值或 gold；规则和五个待审例索引在修正结果计算前固定。属于回顾性纠错，包含既有开发样本，不是新的独立留出实验。', '',
        '## 找到的问题及修正', '',
        '| 问题 | 直接证据 | 已实施的修正 |',
        '| --- | --- | --- |',
        ...
        '完整回答并非天然“不公平”：它衡量解释整个回答。最终答案衡量答案证据。现在将两个问题分开呈现，不根据得分选择目标。前两项是明确实现错误；长短加权是指标与事实单位不一致；语义标签修订是有原文依据、但未经多人独立复核的敏感性注释。', '',
        ...
        '词元预算为 `ceil(f × N_body)`，其中 `N_body` 为原正文句子的合资格词元数；沿排序访问整句，能够放入剩余预算才选入，否则跳过。整句所有合资格词元全部收费；不能只选一个词元就免费展开整句。停用词过滤沿用已有共同规则，因此这里的成本是“合资格词元”，不是所有可见文本词元。另报告固定 top 2/4/8 句以及实际词元成本，不能把句数预算和词元预算混称。', '',
    ...
    lines+=['','完整回答目标下，仅换成原生句边界后，旧式词元 Recall 的 DT−FT 从旧正则视图的 +1.73 点变成 −0.42 点。可见旧分句方式确实会改变比较，不能把旧句聚合的优势直接当成方法优势。', '',
        ...
        '| 目标 | 方法 | P | R | F1 | EM | 完整支持 | 花费词元 | 未用词元 | 选中句数 |',
        '| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |']
    ...
    lines+=['','## 验证与可复现文件', '',
        ...
        '这次修复解决的是证据评测的具体错误和定义不清。原生支持事实仍可能省略替代证明、标题或指代信息，单个助手的语义修订也不能视为官方真值；证据重合率不能单独证明归因的因果忠实度。', '']
