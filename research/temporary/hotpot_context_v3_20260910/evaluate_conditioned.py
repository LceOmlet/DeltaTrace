...
def arguments(argv=None):
    ...
    parser.add_argument('--stage', choices=['context_v3'], required=True)
    ...
    assert args.datasets == ['hotpotqa_long']
    return args
...
def main():
    ...
    plan = json.loads((FAIR/'protocol.json').read_bytes())
    ...
    split = {'plan_sha256':plan['spec_sha256'], 'tasks':{'hotpotqa_long':{
        'context_v3':plan['chunks'][args.chunk], 'cache_sha256':plan['tasks']['hotpotqa_long']['cache_sha256']}}}
    choice = {'targets':{'hotpotqa_long':'answer_conditioned'}, 'weighted_sources':plan['weighted_sources'],
    ...
    control_cases = {r['index']:r for r in control_report['cases']
        if r['dataset']=='hotpotqa_long' and r['target_mode']=='answer_conditioned'}
    ...
    report.update(experiment='hotpot-context-cost-v3', stage=args.stage, chunk=args.chunk, indices=plan['chunks'][args.chunk], fair_plan_sha256=sha((FAIR/'protocol.json').read_bytes()), plan_sha256=split['plan_sha256'], choice=choice)
    vectors = {}
