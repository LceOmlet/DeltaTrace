"""Run all 48 answer-conditioned cases with exact old-development overlap controls."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    identity=dict(plan_sha256=sha(HERE/'protocol.json'),driver_sha256=sha(HERE/'evaluate_conditioned.py'),
    ...
    try:
        for name,indices in plan['chunks'].items():
            ...
            with (a.output/(name+'.log')).open('wb') as log:
                result=subprocess.run([sys.executable,'-u',str(HERE/'evaluate_conditioned.py'),'--environment',str(a.environment.resolve()),
                    '--control',str(a.control.resolve()),'--stage','context_v3','--datasets','hotpotqa_long',
