"""Measure complete DT calls through the unchanged formal batch helper.
...
def main():
    ...
    try:
        ...
        baseline = make_qwen35_clean_runner(model, finite_fa, finite_fla)
        accelerated, sources = make_accelerated_runner(args.release, model, finite_fa, finite_fla)
        ...
        for group in groups:
            for case in group:
                execute('warm_baseline/' + case['key'], baseline, [case])
                ...
                for mode in modes:
                    if mode == 'baseline':
                        for case in groups[index]:
                            execute(f'measured_r{repeat}_baseline/' + case['key'], baseline, [case])
                    else:
                        execute(f'measured_r{repeat}_accelerated/{index}', accelerated, groups[index], plan['batches'][index]['actual_root'])
