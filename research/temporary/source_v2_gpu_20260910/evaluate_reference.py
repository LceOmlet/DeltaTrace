...
def main():
    ...
    split = json.loads((PILOT / 'reference_split.json').read_bytes())
    assert split['plan_sha256'] == sha((PILOT / 'REFERENCE_PILOT.md').read_bytes())
    choice = json.loads(args.choice.read_bytes()) if args.choice else None
    if choice:
        assert choice['split_sha256'] == sha((PILOT / 'reference_split.json').read_bytes())
    ...
    report.update(experiment='reference-pilot-v1', stage=args.stage, split_sha256=sha((PILOT / 'reference_split.json').read_bytes()), plan_sha256=split['plan_sha256'], choice=choice)
    vectors = {}
        ...
        for dataset, raw_cases in caches.items():
            ...
            for index, raw_case in raw_cases:
                ...
                for target_mode in modes:
                    ...
                    refs = {name: reference_token_ids(row['input_ids'], [positions[j] for j in author_keep], token)
