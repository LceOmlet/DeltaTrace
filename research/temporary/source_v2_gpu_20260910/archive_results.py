...
def main():
    ...
    parser.add_argument('--single-run', action='store_true', help='Archive one complete evaluator result, without a suite controller.')
    ...
    if args.restore:
        ...
        return
    if args.single_run:
        state = json.loads((args.source / 'results.json').read_bytes())
        assert state['status'] == 'complete' and all(r['status'] == 'complete' for r in state['cases'])
        assert digest((args.source / 'vectors.npz').read_bytes()) == state['vectors_sha256']
        tasks = list(state['selected_counts'])
        paths = [args.source / name for name in ('results.json', 'vectors.npz')]
    else:
    ...
    manifest = {'format': 'gzip_original_bytes_v1', 'suite_status': state['status'],
                'single_run': args.single_run, 'tasks': tasks, 'files': rows}
