...
def main():
    ...
    split = json.loads((PILOT / 'recall_split.json').read_bytes())
    ...
    report.update(experiment='recall-pilot-v1', stage=args.stage, split_sha256=sha((PILOT / 'recall_split.json').read_bytes()), plan_sha256=split['plan_sha256'], choice=choice)
    vectors = {}
    ...
    try:
        ...
        for dataset, raw_cases in caches.items():
            ...
            for index, raw_case in raw_cases:
                ...
                def attribute(reference_override=None, reverse=False):
                    ...
                    result['pilot_reported_sign'] = -1 if reverse else 1
