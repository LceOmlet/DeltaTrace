...
**最新 80 例内部留出验证已完成并复核：已测出 VT 原始 token Recall 的优势，HotpotQA 尚不确定。** VT 四任务等权：DT 59.79%、FT K3 55.70%，差值 +4.09 点，四项比较校正后区间 [+2.78, +5.56]。双方句聚合后均为 63.58%，每例达到各自 10% token 预算的上限。HotpotQA 句聚合为 75.94% 对 69.22%，差值 +6.72 点，校正区间 [−4.55, +17.92]，尚不能确认稳定优势。见 [最新完整结果](target_scope/RESULTS.md) 与 [目标构造修复说明及运行入口](TARGET_CONSTRUCTION_FIX.md)。
