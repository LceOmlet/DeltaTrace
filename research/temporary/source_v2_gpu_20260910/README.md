...
**后续 HotpotQA 公平性修正已完成 48/48 例。** 标题/正文 gold 坐标、前导空格分句归属已修正，使用双方共用的原生支持句、整句成本与事实等权评分。答案目标的支持事实 Recall 为 DT **68.58%**、FT K3 **66.15%**；两处关系标签修订后为 **68.58% 对 67.19%**，区间跨零。完整结果与显式新版入口见 [HotpotQA 修正报告](../hotpot_fairness_20260910/RESULTS.md)。本目录以下 HotpotQA 数字保留旧缓存标签与正则分句口径，用于历史重放；四组 VT 全量结果保留。
