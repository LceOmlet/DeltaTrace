...
**后续 HotpotQA v3 已完成全部 48 例上下文保留与计费修正。** 仅改变答案归因的初始目标权重，保留完整推理和原模型输入；整句计入全部正文词元，按排序前缀选择。保留原推理的答案条件中，10% 预算的有符号总和 Recall 为 DT 35.59%、FT K3 51.04%，正值均值为 48.96%/64.76%；完整回答两项差值区间均跨零。见 [v3 结果与验证](../hotpot_context_v3_20260910/RESULTS.md)。旧归档、旧数值及四组 VT 结果保持不变。

**后续 HotpotQA 复审更正：定位修正成立，但不能把整套新协议称为公平性已验证。** 答案条件实验删除了原推理前缀；原生整句表收费的有效词元排除了标点/空白；共同句均值与装箱也未证明中立。见 [复审与最小修正结果](../hotpot_fairness_20260910/REVIEW.md)。[48 例探索性对照](../hotpot_fairness_20260910/RESULTS.md) 全部保留。本目录以下 HotpotQA 数字保留旧标签与正则分句口径，用于历史重放；四组 VT 数值未改。这里既有 VT 的 `answer_only` 同样表示截取答案后重建目标输入，不能理解为保留原推理、仅改变目标种子。
