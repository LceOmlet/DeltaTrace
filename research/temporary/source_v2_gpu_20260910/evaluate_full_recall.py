...
def main():
    ...
    split = json.loads((PILOT / 'scope_split.json').read_bytes())
    assert split['plan_sha256'] == sha((PILOT / 'TARGET_SCOPE_VALIDATION.md').read_bytes())
    choice = json.loads(args.choice.read_bytes()) if args.choice else None
    if choice:
        assert choice['split_sha256'] == sha((PILOT / 'scope_split.json').read_bytes())
    ...
    full_plan=json.loads((PILOT/'full_recall_plan.json').read_bytes())
    ...
    for dataset in args.datasets:
        ...
        assert sha(raw) == full_plan['tasks'][dataset]['cache_sha256']
    ...
    report.update(experiment='target-full-v1', stage=args.stage, split_sha256=sha((PILOT / 'scope_split.json').read_bytes()), plan_sha256=split['plan_sha256'], choice=choice)
    report.update(full_plan_sha256=sha((PILOT/'full_recall_plan.json').read_bytes()),full_spec_sha256=full_plan['spec_sha256'],chunk=args.chunk,chunk_spec=chunk,parent_results_sha256=full_plan['parent_results_sha256'],parent_vectors_sha256=full_plan['parent_vectors_sha256'])
        ...
        for dataset, raw_cases in caches.items():
            ...
            for index, raw_case in raw_cases:
                ...
                for target_mode in modes:
                    ...
                    if index in chunk['control_indices']:
