...
def summarize_full_recall(rows, task_indices):
    ...
    for t in TASKS:
        indices=task_indices[t]
        assert len(indices)==(48 if t=='hotpotqa_long' else 100)
    ...
    return dict(task_comparisons=task_stats,groups=groups,primary_comparisons=primary,
        ...
        positive_adjusted_intervals=[k for k,v in primary.items() if v['adjusted_verdict']=='advantage'],
        negative_adjusted_intervals=[k for k,v in primary.items() if v['adjusted_verdict']=='disadvantage'],
        same_view_positive_intervals=[v for v in VIEWS if all(primary[g+'_'+v]['adjusted_verdict']=='advantage' for g in ('VT','HotpotQA'))],
