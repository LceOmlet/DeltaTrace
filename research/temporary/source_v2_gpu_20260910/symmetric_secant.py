# Experimental output-seed selection; derived from frozen source SHA256 3a90b10824cde448886a1ca69acaee7a3eccf0bd4dc7bd59d7e7463bc25322d7
from compiled_secant_boundaries import attention_layout, norm_residual_two, norm_residual_three
...
def propagate_signed_secant(model, before, after, variant='rescale', progress=None, pv_rule='content_P1', finite_attention=None, finite_activity=None, target_weights=None):
    import torch
    ...
    with torch.no_grad():
        ...
        for li in reversed(range(len(model.model.layers))):
            ...
            checks.append({'layer': li, 'native_attention_qkv_and_output_captured': True, 'native_LSE_probability_reconstructed_only_in_finite_tiles': True, 'native_swiglu_product_endpoints_exact': True, 'max_abs_input_multiplier': float(m.abs().max())})
