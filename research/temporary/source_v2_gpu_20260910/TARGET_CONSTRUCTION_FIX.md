...
**全量复核已完成：** 四组 VT 各 100 例、HotpotQA 48 例，共 448 例，采用下述固定策略。三链 VT 原始 Recall 为 **85.62% 对 72.41%**，双方句聚合均为 **100%**；四组 VT 原始宏平均为 **60.20% 对 56.14%**。HotpotQA 原始排序为 **40.63% 对 45.35%**，句聚合为 **73.81% 对 72.09%**，后者区间仍跨零。完整表、区间、原始数据及 `analyze_full_recall.py` 复现入口见 [448 例全量报告](full_recall/RESULTS.md)。全量包含开发数据，下文 80 例保留验证仍作为独立标记的历史验证结果保存。
