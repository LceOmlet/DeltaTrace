...
def arguments(argv=None):
    ...
    if args.stage == 'validation':
        ...
        assert args.choice is None and args.parent is not None
    return args
...
def main():
    ...
    split = json.loads((PILOT / 'symmetric_split.json').read_bytes())
    assert split['plan_sha256'] == sha((PILOT / 'SYMMETRIC_PILOT.md').read_bytes())
    choice = json.loads(args.choice.read_bytes()) if args.choice else None
    if choice:
        assert choice['split_sha256'] == sha((PILOT / 'symmetric_split.json').read_bytes())
    ...
    report.update(experiment='symmetric-pilot-v1', stage=args.stage, split_sha256=sha((PILOT / 'symmetric_split.json').read_bytes()), plan_sha256=split['plan_sha256'], choice=choice)
    vectors = {}
    ...
    try:
        ...
        control_vectors = control_rows = None
        if args.parent:
        ...
        report['weighted_sources'] = {name: sha((PILOT / name).read_bytes()) for name in ('symmetric_secant.py', 'symmetric_paired.py')}
        for dataset, raw_cases in caches.items():
            ...
            for index, raw_case in raw_cases:
                ...
                for target_mode in modes:
                    ...
                    def attribute(reference_override=None, weights=None, rule='content_P1'):
                        ...
                        if args.family == 'qwen3':
                            ...
                            result = (propagate_paired_secant(model, before, after, pv_rule='content_P1', finite_attention=finite_fa) if weights is None else
                                propagate_weighted(model, before, after, pv_rule=rule, finite_attention=finite_fa, target_weights=weights))
                    ...
                    reference = reference_token_ids(row['input_ids'], [positions[j] for j in author_keep], tokenizer.eos_token_id)
