"""Independently verify fixed answer-only EOS/space reference comparisons."""
import argparse
import csv
import gzip
import hashlib
...
def main():
    ...
    split=json.loads((HERE/'reference_split.json').read_bytes())
    assert r['status']=='complete' and r['experiment']=='reference-pilot-v1'
    assert r['driver_sha256']==digest(HERE/'evaluate_reference.py')
    assert r['split_sha256']==digest(HERE/'reference_split.json')
    assert r['plan_sha256']==split['plan_sha256']==digest(HERE/'REFERENCE_PILOT.md')
    assert r['vectors_sha256']==digest(a.run/'vectors.npz')
    for name,value in r['weighted_sources'].items():assert value==digest(HERE/name)
    space,=tok.encode(' ',add_special_tokens=False).ids
    assert r['space_reference_token_id']==space
    ...
    for row in r['cases']:
        ...
        assert eos!=space and hashlib.sha256(ids.tobytes()).hexdigest()==row['input_sha256']
        ...
        for name,token in [('eos',eos),('space',space)]:
        ...
        mean=.5*(vectors[prefix+'DT_eos_signed_full']+vectors[prefix+'DT_space_signed_full'])
        assert np.array_equal(mean,vectors[prefix+'DT_mean_signed_full'])
        if parent_rows is not None:
            assert row['input_sha256']==parent_rows[t,i]['input_sha256']
            for method,old_method in [('DT_eos_signed_full','DT_target_signed_full'),('FT_K1_prompt','FT_K1_prompt'),('FT_K3_prompt','FT_K3_prompt')]:
        ...
        for method,views in row['metrics'].items():
            if method.startswith('DT_'):
                ...
                if ref!='mean':
                    ...
                    residuals.append({'dataset':t,'index':i,'reference':ref,'delta':delta,'residual':detail['unassigned_total']})
            else:
        ...
        residuals.append({'dataset':t,'index':i,'reference':'mean','delta':.5*sum(deltas.values()),
            'residual':.5*sum(deltas.values())-float(mean.sum())})
    ...
    if stage=='development':
        ...
        if a.freeze_choice:
            ...
            choice={'status':'frozen_for_validation','reference':chosen,'split_sha256':r['split_sha256'],
        ...
        chosen=r['choice']['reference'];groups={}
