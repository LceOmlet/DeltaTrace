...
def main():
    ...
    for name in (Path(__file__).name,'dt_variants.py','run_causal.py','qwen35_gdn_symmetric.py','deltatrace/profiles/official.py'):(a.output/Path(name).name).write_bytes((Path(__file__).parent/name).read_bytes())
