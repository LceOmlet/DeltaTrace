        ...
        if is_dt:
            ...
            if args.family=='qwen3':
                ...
                if args.method=='deltatrace_streamed':
                    ...
                    import streamed_qwen3_v2 as streamed_qwen3
                    assert sha(Path(streamed_qwen3.__file__).read_bytes())==protocol['candidate_sha256']
                    assert sha(Path(sys.modules['streamed_qwen3'].__file__).read_bytes())==protocol['base_stream_sha256']
                    dt_runner=streamed_qwen3.StreamedBuildFiniteGraphQwen3(model,dt_runner.finite)
