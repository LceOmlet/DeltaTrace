        ...
        with np.load(folder/'vectors.npz') as vectors:
            for case in data['cases']:
                ...
                if is_dt:
                    ...
                    if job['method']=='deltatrace_streamed':assert case['original_full_layout_metadata_exact']
                    ...
                    if 'native_graph_execution' in detail:
                        ...
                        expected_views,expected_storages=(739,514) if job['method']=='deltatrace_streamed' else (883,658)
                        assert execution['static_input_tensors']==expected_views and execution['static_input_storage_copies']==expected_storages
                        ...
                        assert detail['graph_input_copy_audit']=={'enabled':True,'predicates':expected_views,'all_passed':True}
...
def enrich_costs(summary,raw,run_name):
    ...
    goal_memory=[]
    for n in [128,256,512,1024]:
        dt=next(c for c in summary['pooled_cells'] if c['method']=='deltatrace_streamed' and c['target_input_tokens']==n)
        for method in ['ifr_multi_hop_both','ifr_multi_hop']:
            ft=next(c for c in summary['pooled_cells'] if c['method']==method and c['target_input_tokens']==n)
            goal_memory.append({'target_input_tokens':n,'FT_method':method,
                'DT_peak_allocated_gb':dt['peak_allocated_gb'],'FT_peak_allocated_gb':ft['peak_allocated_gb'],
                'DT_peak_reserved_gb':dt['peak_reserved_gb'],'FT_peak_reserved_gb':ft['peak_reserved_gb'],
                'DT_allocated_below_FT':dt['peak_allocated_gb']<ft['peak_allocated_gb'],
                'DT_reserved_below_FT':dt['peak_reserved_gb']<ft['peak_reserved_gb']})
    summary['new_user_goal_memory_against_FT']=goal_memory
    summary['new_user_goal_achieved']=summary['speed_acceptance_passed'] and all(c['DT_allocated_below_FT'] and c['DT_reserved_below_FT'] for c in goal_memory)
    summary['acceptance_scope']='The frozen confirmation-v2 acceptance compares speed against FT and memory against preceding DT. The later user goal additionally requires memory below FT; see new_user_goal_achieved.'
