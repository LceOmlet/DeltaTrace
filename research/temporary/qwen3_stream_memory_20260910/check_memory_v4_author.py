...
def main():
    ...
    A=p.parse_args().root;R=A/'memory_production_v4_release';B=Path('/tmp/codex_short_b1_efficiency_20260910_v1')
    plan_path=A/'memory_v4_author_plan.json';plan=json.loads(plan_path.read_bytes());out=A/'memory_v4_author';out.mkdir(exist_ok=False)
    ...
    assert sha((A/'check_memory_v4_graph_probes.py').read_bytes())==plan['probe_sha256']
    for name,digest in plan['runtime_files'].items():assert sha((R/name).read_bytes())==digest,name
    for name,digest in plan['common_loading_helpers'].items():assert sha((A/name).read_bytes())==digest,name
    try:
        ...
        os.environ.update(MACA_PATH='/opt/maca',HF_HUB_OFFLINE='1',TOKENIZERS_PARALLELISM='false',
            TRITON_CACHE_DIR=str(A/'triton_memory_v4_confirmation_deltatrace_streamed'),TORCHINDUCTOR_CACHE_DIR=str(A/'inductor_memory_v4_confirmation_deltatrace_streamed'))
        ...
        model,tokenizer,report['model_loading_strategy']=load_common_staged_model(bench,env['checkpoint'],'cuda:0');torch.cuda.synchronize()
        import resource
        report['model_load_memory']={'peak_allocated_gb':torch.cuda.max_memory_allocated()/1e9,'peak_reserved_gb':torch.cuda.max_memory_reserved()/1e9,'host_process_highwater_bytes':resource.getrusage(resource.RUSAGE_SELF).ru_maxrss*1024}
        ...
        from check_memory_v4_graph_probes import run as graph_probes
