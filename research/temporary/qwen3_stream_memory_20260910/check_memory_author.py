...
def main():
    ...
    A=p.parse_args().root;R=A/'memory_production_release';B=Path('/tmp/codex_short_b1_efficiency_20260910_v1')
    plan_path=A/'memory_author_plan.json';plan=json.loads(plan_path.read_bytes());out=A/'memory_author';out.mkdir(exist_ok=False)
    ...
    try:
        ...
        os.environ.update(MACA_PATH='/opt/maca',HF_HUB_OFFLINE='1',TOKENIZERS_PARALLELISM='false',
            TRITON_CACHE_DIR=str(A/'triton_memory_confirmation_deltatrace_streamed'),TORCHINDUCTOR_CACHE_DIR=str(A/'inductor_memory_confirmation_deltatrace_streamed'))
