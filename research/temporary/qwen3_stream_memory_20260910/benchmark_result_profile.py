            ...
            if last_ok:
                ...
                if is_dt:
                    ...
                    case['metadata_mutation_control']={'seconds':time.perf_counter()-t,
                        ...
                        'fresh_nested_metadata':len(again['native_projection_SAC']['regions'][0]['operations'])==7}
                    assert all(case['metadata_mutation_control'][k] for k in ['vector_exact','math_exact','fresh_nested_metadata'])
                    del again
                    # Diagnostic-only complete API call. Profiler overhead is
                    # separately charged and excluded from no measured row.
                    t=time.perf_counter()
                    with torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CPU,torch.profiler.ProfilerActivity.CUDA],record_shapes=True) as prof:
                        profiled=dt_runner.attribute(audit_base,ids,mask,prompt_len,mutation_audit=True)
                        torch.cuda.synchronize()
                    case['profile']={'seconds':time.perf_counter()-t,
                        'vector_exact':bool(np.array_equal(np.asarray(profiled['signed_full_sequence'],dtype=np.float64),retained_vec)),
                        'math_exact':all(profiled[k]==baseline[k] for k in keys),
                        'host_sections':profiled['whole_root_graph']['host_sections_seconds']}
                    assert case['profile']['vector_exact'] and case['profile']['math_exact']
                    prof.export_chrome_trace(str(out/'trace.json'))
                    (out/'profile.txt').write_text(prof.key_averages().table(sort_by='self_cuda_time_total',row_limit=100))
                    del profiled,prof
