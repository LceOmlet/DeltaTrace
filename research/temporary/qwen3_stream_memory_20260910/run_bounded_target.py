...
T=Path(__file__).resolve().parent;out=T/'bounded_target';out.mkdir(exist_ok=False)
report={'status':'running','jobs':[],'started':time.time()}
def save():(out/'queue.json').write_text(json.dumps(report,indent=2)+'\n')
save()
try:
 for name,driver,protocol,lengths in [('rollout','benchmark_bounded_target_rollout.py','bounded_target_rollout_protocol.json','10,100,500'),('short','benchmark_bounded_target_short.py','bounded_target_short_protocol.json','128,256,512,1024')]:
