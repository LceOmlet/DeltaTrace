...
def main():
    ...
    try:
        ...
        if is_dt:
            ...
            if args.family=='qwen3':
                ...
                if args.method=='deltatrace_streamed':
                    from whole_remat_qwen3 import WholeRematFiniteGraphQwen3
                    dt_runner=WholeRematFiniteGraphQwen3(model,dt_runner.finite)
            ...
            def make_runner():
                ...
                def call():
                    ...
                    if args.family=='qwen3':
                        ...
                        last_cost.clear();last_cost.update({k:answer.get(k) for k in ['streamed_root','rematerialized_root','whole_root_graph','native_graph_execution','root_retention_mutation_audit','graph_input_copy_audit']})
                        return answer
        ...
        if is_dt:
            ...
            with torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CPU,torch.profiler.ProfilerActivity.CUDA],record_shapes=True,profile_memory=True) as profile:
                with torch.no_grad():
                    packed_profile=dt_runner.program.invoke()
                    torch.cuda.synchronize()
                    profile_result=packed_profile.resolve(dt_runner.program.inputs['before'],dt_runner.program.inputs['after'],0.0)
            torch.cuda.synchronize()
            report['profile_seconds']=time.perf_counter()-started
            assert np.array_equal(np.asarray(profile_result['signed_full_sequence'],dtype=np.float64),vectors['1024_retained'])
            report['profile_exact_to_original']=True
            events=profile.key_averages(group_by_input_shape=True)
            rows=[]
            for event in events:
                rows.append({key:getattr(event,key,None) for key in ['key','count','self_cpu_time_total','cpu_time_total','self_device_time_total','device_time_total','self_cpu_memory_usage','self_device_memory_usage','input_shapes']})
            (out/'profile_events.json').write_text(json.dumps(rows,indent=2)+'\n')
            (out/'profile_table.txt').write_text(events.table(sort_by='self_device_time_total',row_limit=60))
            profile.export_chrome_trace(str(out/'profile_trace.json'))
            del packed_profile,profile_result,profile,events
            save()
        if is_dt:
