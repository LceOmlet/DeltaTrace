...
import time
import torch
from streamed_qwen3_v3_base import StreamTape
from streamed_qwen3_v3_build import BuildingStreamTape,StreamedBuildFiniteGraphQwen3
    ...
    def attribute(self,*args,**kwargs):
        ...
        for row in result['native_paired_root_layouts']:
            ...
            row['tensors']={name:omitted[name] if name in omitted else kept[name]
                            for name in self.program.layout_order[index]}
