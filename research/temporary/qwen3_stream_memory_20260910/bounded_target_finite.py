from compiled_secant_boundaries import norm_residual_three,norm_residual_two
from bounded_target_ops import target_seed
from qwen3_input_cast_boundaries import attention_layout,norm as cast_final_norm
from qwen3_projection_cast_boundaries import swiglu_half as swiglu_secant_multipliers,seed_half as compiled_seed,native_precast_half_linear,qk_norm_half
...
def propagate_signed_secant(model, before, after, variant='rescale', progress=None, pv_rule='content_P1', finite_attention=None, finite_activity=None,validation=None):
    ...
    with torch.no_grad():
        seed, seed_check = target_seed(before['logits'], after['logits'], after['target'].to(device))
        ...
        m_final[0, start:-1] = native_precast_half_linear(seed, model.lm_head.weight)
        del seed
        m = cast_final_norm(before['last'],after['last'],model.model.norm.weight,m_final,model.model.norm.variance_epsilon)
        del m_final
        if progress is not None:progress(None,variant,'seed_done')
        for li in reversed(range(len(model.model.layers))):
            ...
            m_x_up = native_precast_half_linear(mu,layer.mlp.up_proj.weight)
            del mu
            m_x_gate = native_precast_half_linear(m_gate,layer.mlp.gate_proj.weight)
            del m_gate
            m_mid = norm_residual_two(f(a['mid']), f(b['mid']), f(layer.post_attention_layernorm.weight), m_x_gate, m_x_up, m_mid, layer.post_attention_layernorm.variance_epsilon)
            del m_x_gate,m_x_up
            a.clear();b.clear()
            ...
            mqp = qk_norm_half(f(raw0['qpre']),f(raw1['qpre']),f(layer.self_attn.q_norm.weight),mqn,layer.self_attn.q_norm.variance_epsilon)
            mkp = qk_norm_half(f(raw0['kpre']),f(raw1['kpre']),f(layer.self_attn.k_norm.weight),mkn,layer.self_attn.k_norm.variance_epsilon)
            mqa = native_precast_half_linear(mqp.reshape(1,n,-1),layer.self_attn.q_proj.weight)
            mka = native_precast_half_linear(mkp.reshape(1,n,-1),layer.self_attn.k_proj.weight)
