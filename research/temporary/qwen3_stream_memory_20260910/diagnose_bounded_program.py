        ...
        if is_dt:
            ...
            if args.family=='qwen3':
                ...
                if args.method=='deltatrace_streamed':
                    ...
                    from bounded_target_graph import SACFiniteGraphQwen3
            ...
            if last_ok:
                ...
                if is_dt:
                    ...
                    if ids.shape[1]<=1024:
                        ...
                        from bounded_target_core import root_endpoints
                        ...
                        np.savez(out/'vectors.npz',**vectors);save()
                        import bounded_diagnostic_ops as db
                        from bounded_diagnostic_graph import SACProgram
                        diag=SACProgram(model,pair,prompt_len,db.PairedFA(dt_runner.finite,original_fa))
                        with torch.no_grad():
                            dp=diag.invoke();dv=dp.resolve(diag.inputs['before'],diag.inputs['after'],0)
                        da=np.asarray(dv['signed_full_sequence'],dtype=np.float64)
                        vectors[str(output_len)+'_observed_boundaries']=da
                        case['observed_direct_vector_exact']=bool(np.array_equal(da,eager_vec))
                        case['boundary_comparisons']=db.records
                        np.savez(out/'vectors.npz',**vectors);save()
                        dt_runner.close()
                        del before,after,baseline,retained_vec,packed,eager,eager_vec,diag,dp,dv,da
                    del pair
