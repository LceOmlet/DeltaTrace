...
def verify(raw):
    plan=json.loads((HERE/'memory_v2_author_plan.json').read_bytes())
    path=raw/'memory_v2_author/results.json';data=json.loads(path.read_bytes())
    assert data['status']=='complete' and data['all_vectors_exact']
    assert data['driver_sha256']==plan['driver_sha256'] and data['plan_sha256']==sha((HERE/'memory_v2_author_plan.json').read_bytes())
    assert len(data['cases'])==len(plan['cases'])==11
    assert data['generation_calls']==data['metric_calls']==data['FT_calls']==0
    checks=[]
    with np.load(raw/'memory_v2_author/vectors.npz') as z:
...
if __name__=='__main__':
    ...
    (HERE/'memory_v2_author_verification.json').write_text(json.dumps(report,indent=2)+'\n',newline='\n');print(json.dumps(report))
