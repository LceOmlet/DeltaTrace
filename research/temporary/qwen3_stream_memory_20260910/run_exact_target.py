...
T=Path(__file__).resolve().parent;out=T/'exact_target';out.mkdir(exist_ok=False)
report={'status':'running','jobs':[],'started':time.time()}
def save():(out/'queue.json').write_text(json.dumps(report,indent=2)+'\n')
save()
try:
 for name,driver,protocol,lengths in [('rollout','benchmark_exact_target_rollout.py','exact_target_rollout_protocol.json','10,100,500'),('short','benchmark_exact_target_short.py','exact_target_short_protocol.json','128,256,512,1024')]:
