        ...
        if is_dt:
            ...
            if args.family=='qwen3':
                ...
                if args.method=='deltatrace_streamed':
                    ...
                    dt_runner,report['streaming_sources']=make_streamed_qwen3(R,model,env['finite_library'],env['finite_library_sha256'])
        ...
        for input_len in map(int,args.lengths.split(',')):
            ...
            if last_ok:
                ...
                if is_dt:
                    ...
                    if args.method=='deltatrace_streamed':
                        prior=json.loads((Path(protocol['release_root']).parent/'confirmation/round1_deltatrace_graphed/results.json').read_bytes())
                        reference_case=next(c for c in prior['cases'] if c['input_length']==input_len)
