...
T=Path(__file__).resolve().parent;P=json.loads((T/'memory_v4_confirmation_protocol.json').read_bytes());out=T/'memory_v4_confirmation';out.mkdir(exist_ok=False)
...
try:
    for method in P['methods']:
        for prefix in ['triton','inductor']:assert not (T/(prefix+'_memory_v4_confirmation_'+method)).exists()
    for round_index,lengths in enumerate(P['length_orders'],1):
        for index,n in enumerate(lengths):
            ...
            for method in order:
                ...
                env=dict(os.environ,MACA_PATH='/opt/maca',OMP_NUM_THREADS='4',PYTHONDONTWRITEBYTECODE='1',TRITON_CACHE_DIR=str(T/('triton_memory_v4_confirmation_'+method)),TORCHINDUCTOR_CACHE_DIR=str(T/('inductor_memory_v4_confirmation_'+method)))
                command=['/mnt/geogpt-doc-new/deepresearch/lzq/contrastive_flashtrace_probe/env/bin/python','-u',str(T/'benchmark_memory_v4.py'),'--root','/tmp/codex_short_b1_efficiency_20260910_v1','--family','qwen3','--method',method,'--lengths',str(n),'--protocol',str(T/'memory_v4_confirmation_protocol.json'),'--output',str(out/name)]
