...
def qk_norm_half_rule(x0,x1,weight,upstream,eps):
    assert x0.dtype==x1.dtype==weight.dtype==upstream.dtype==torch.float32
    return rmsnorm_secant_pullback(x0,x1,weight,upstream,eps).to(torch.float16)

qk_norm_half=torch.compile(qk_norm_half_rule,fullgraph=True,dynamic=True,options=options)
