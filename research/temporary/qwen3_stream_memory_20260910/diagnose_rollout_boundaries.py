...
def main():
    ...
    p.add_argument('--lengths',default='10,100,500,1000,2000,5000,10000')
    ...
    for name,digest in protocol['diagnostic_sources'].items():assert sha((Path(__file__).parent/name).read_bytes())==digest,name
    ...
    report={'status':'imports','family':args.family,'method':args.method,
      ...
      'sample_batch':1,'generation_calls':0,'nominal_input_tokens':10,'rollout_curve':True,'cases':[],'rows':[],'initialization':[],
      ...
      'warm_timing_scope':'Two explicitly recorded warm calls per rollout length, then three measured complete calls; unchanged author reset/empty_cache before every call. Each method/length uses a fresh model process; all21 jobs serial on one device. Same-method compiler disk cache reused in the stated order; each graph is newly constructed.',
    ...
    try:
        ...
        if is_dt:
            ...
            if args.family=='qwen3':
                ...
                if args.method=='deltatrace_streamed':
                    ...
                    dt_runner,report['memory_sources']=make_memory_efficient_qwen3(R,model,protocol['candidate_finite_library']['path'],protocol['candidate_finite_library']['sha256'])
                else:
        ...
        vectors={}
        for output_len in map(int,args.lengths.split(',')):
            input_len=10
            assert output_len in protocol['output_lengths']
            prompt,actual_input=bench.build_prompt_to_length(tokenizer,base,input_len)
            target,actual_output=bench.build_output_to_length(tokenizer,' The answer is 42.',output_len)
            ...
            case={'input_length':input_len,'output_length':output_len,'prompt':prompt,'target':target,
            ...
            def make_runner():
                ...
                def call():
                    ...
                    if args.family=='qwen3':
                        ...
                        last_cost.clear();last_cost.update({k:answer.get(k) for k in ['checkpoint_lifetime','native_projection_SAC','streamed_root','rematerialized_root','whole_root_graph','native_graph_execution','root_retention_mutation_audit','graph_input_copy_audit']})
                        return answer
            ...
            for rep in range(n_runs):
                ...
                report['status']=f'{output_len}_{phase}_{rep}';save()
                ...
                row={'family':args.family,'attr_func':args.method,'phase':phase,'repeat':rep,
                  'target_input_tokens':input_len,'target_output_tokens':output_len,'target_total_tokens':input_len+output_len,
            ...
            if last_ok:
                phase='audit'
                report['status']=f'{output_len}_audit';save()
                actual_calls=[]
                ...
                handle=None if args.method=='deltatrace_streamed' else model.register_forward_pre_hook(observe,with_kwargs=True)
                ...
                if is_dt:
                    assert args.method=='deltatrace_streamed'
                    pair=torch.cat((ids.clone(),ids))
                    pair[0,eligible]=tokenizer.eos_token_id
                    pair_hash=sha(pair.cpu().numpy().tobytes())
                    assert result['whole_root_graph']['fresh_graph_input_sha256']==pair_hash
                    assert not actual_calls
                    execution=result['native_graph_execution']
                    assert execution['native_model_GPU_root_executions']==1 and execution['native_model_Python_root_calls']==0
                    ...
                    assert result['deferred_validation']['all_passed']
                    assert all(c['native_input_exact'] and c['native_output_exact'] for c in result['native_layer_boundary_checks']['paired_batch'])
                    assert all(c['public_output_exact_to_actual_model_FA'] for c in result['public_FA_capture_checks'])
                    case['details']={k:v for k,v in result.items() if k!='signed_full_sequence'}
                    case['graph_build_info']=dt_runner.program.build_info
                    case['fresh_paired_input_sha256']=pair_hash
                    case['retained_reference_scope']='Additional exact full-vector and scalar/layer check for actual sequence<=1024; longer cells retain all829/865 runtime predicates and original native boundary checks, without claiming an unexecuted full-logit retained comparison.'
                    if ids.shape[1]<=1024:
                        ...
                        def observe_baseline(_m,args,kwargs):
                            tensor=kwargs['input_ids'];assert torch.equal(tensor,pair)
                        ...
                        t=time.perf_counter()
                        try:
                            before,after=capture_checkpoint_pair(model,pair[0:1],ids,mask,prompt_len)
                            baseline=retained(model,before,after,pv_rule='content_P1',finite_attention=original_fa)
                        finally:baseline_handle.remove()
                        torch.cuda.synchronize();case['retained_comparison_seconds']=time.perf_counter()-t
                        assert len(baseline_calls)==1 and baseline_calls[0]['input_sha256']==pair_hash
                        case['retained_actual_calls']=baseline_calls
                        retained_vec=np.asarray(baseline['signed_full_sequence'],dtype=np.float64)
                        vectors[str(output_len)+'_retained']=retained_vec
                        case['retained_vector_exact']=bool(np.array_equal(vec,retained_vec))
                        case['retained_math_exact']=all(result[k]==baseline[k] for k in ['target_delta_score32_sum64','target_delta_score16','signed_sum','unassigned_total','layer_checks'])
                        ...
                        case['math_field_equality']={k:result[k]==baseline[k] for k in keys}
                        case['vector_difference']={'max_abs':float(np.max(np.abs(vec-retained_vec))),'relative_l2':float(np.linalg.norm(vec-retained_vec)/max(np.linalg.norm(retained_vec),1e-300))}
                        vectors[str(output_len)+'_new']=vec
                        np.savez(out/'vectors.npz',**vectors);save()
                        from qwen3_whole_graph_core import root_endpoints
                        with torch.no_grad():selected,selected_scores=root_endpoints(model,pair,prompt_len)
                        def tensor_diff(a,b):
                            a=a.detach();b=b.detach();return {'shape':list(a.shape),'exact':bool(torch.equal(a,b)),'max_abs':float((a.float()-b.float()).abs().max())}
                        root_diffs={}
                        for e,(old_root,new_root) in enumerate(zip((before,after),selected)):
                            root_diffs[str(e)]={k:tensor_diff(old_root[k],new_root[k]) for k in ['last','norm_out','logits','target_logprobs32','target']}
                            root_diffs[str(e)]['score16']={'original':old_root['score16'],'selected':float(new_root['score16'])}
                            root_diffs[str(e)]['score32_sum64']={'original':old_root['score32_sum64'],'selected':float(new_root['score32_sum64'])}
                        root_diffs['layers']=[tensor_diff(a['x'],b['x']) for a,b in zip(before['paired_checkpoint']['layers'],selected[0]['paired_checkpoint']['layers'])]
                        case['original_full_vs_selected_native_root']=root_diffs;save()
                        del selected,selected_scores
                        with torch.no_grad():
                            packed=dt_runner.program.invoke()
                            eager=packed.resolve(dt_runner.program.inputs['before'],dt_runner.program.inputs['after'],0)
                        eager_vec=np.asarray(eager['signed_full_sequence'],dtype=np.float64)
                        vectors[str(output_len)+'_direct_new']=eager_vec
                        case['graph_vs_direct_new']={'vector_exact':bool(np.array_equal(vec,eager_vec)),'max_abs':float(np.max(np.abs(vec-eager_vec))), 'math_fields':{k:result[k]==eager[k] for k in keys}}
                        case['direct_new_vs_original']={'vector_exact':bool(np.array_equal(eager_vec,retained_vec)),'max_abs':float(np.max(np.abs(eager_vec-retained_vec))), 'math_fields':{k:baseline[k]==eager[k] for k in keys}}
                        np.savez(out/'vectors.npz',**vectors);save()
                        from qwen3_whole_graph_core import WholeProgram
                        old_program=WholeProgram(model,pair,prompt_len,original_fa)
                        with torch.no_grad():
                            old_packed=old_program.invoke();old_finite=old_packed.resolve(old_program.inputs['before'],old_program.inputs['after'],0)
                        old_finite_vec=np.asarray(old_finite['signed_full_sequence'],dtype=np.float64)
                        vectors[str(output_len)+'_original_finite_selected_root']=old_finite_vec
                        case['original_finite_selected_root_comparison']={'vector_exact':bool(np.array_equal(old_finite_vec,retained_vec)),'max_abs':float(np.max(np.abs(old_finite_vec-retained_vec))), 'math_fields':{k:baseline[k]==old_finite[k] for k in keys}}
                        del old_program,old_packed,old_finite,old_finite_vec
                        import diagnostic_boundary_ops as db
                        from diagnostic_boundary_graph import SACProgram as DiagnosticProgram
                        diagnostic=DiagnosticProgram(model,pair,prompt_len,db.PairedFA(dt_runner.finite,original_fa))
                        with torch.no_grad():
                            diagnostic_packed=diagnostic.invoke();diagnostic_result=diagnostic_packed.resolve(diagnostic.inputs['before'],diagnostic.inputs['after'],0)
                        dv=np.asarray(diagnostic_result['signed_full_sequence'],dtype=np.float64)
                        vectors[str(output_len)+'_observed_boundaries_new']=dv
                        case['boundary_observations_preserve_new_vector']=bool(np.array_equal(dv,vec));assert case['boundary_observations_preserve_new_vector']
                        case['boundary_comparisons']=db.records;np.savez(out/'vectors.npz',**vectors);save()
                        dt_runner.close()
                        del before,after,baseline,retained_vec,packed,eager,eager_vec,diagnostic,diagnostic_packed,diagnostic_result,dv
                    del pair
                vectors[str(output_len)]=vec
