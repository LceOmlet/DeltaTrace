...
T=Path(__file__).resolve().parent;P=json.loads((T/'memory_v4_rollout_protocol.json').read_bytes())
out=T/'memory_v4_rollout';out.mkdir(exist_ok=False)
...
try:
 for index,n in enumerate(P['output_lengths']):
  ...
  for method in methods:
   ...
   env=dict(os.environ,MACA_PATH='/opt/maca',OMP_NUM_THREADS='4',PYTHONDONTWRITEBYTECODE='1',TRITON_CACHE_DIR=str(T/('triton_memory_v4_rollout_'+method)),TORCHINDUCTOR_CACHE_DIR=str(T/('inductor_memory_v4_rollout_'+method)))
   if index==0:assert not Path(env['TRITON_CACHE_DIR']).exists() and not Path(env['TORCHINDUCTOR_CACHE_DIR']).exists()
   command=['/mnt/geogpt-doc-new/deepresearch/lzq/contrastive_flashtrace_probe/env/bin/python','-u',str(T/'benchmark_memory_v4_rollout.py'),'--root','/tmp/codex_short_b1_efficiency_20260910_v1','--family','qwen3','--method',method,'--lengths',str(n),'--protocol',str(T/'memory_v4_rollout_protocol.json'),'--output',str(out/name)]
