...
def enrich_costs(summary,raw,run_name):
    ...
    summary.update(cost_cells=costs,process_costs=processes,memory_comparisons=memory,cold_amortization=amortization,
        ...
        previous_v1_transition_gate='The original descending mixed-geometry v1 gate failed and remains unchanged in memory_confirmation_summary.json and its raw archive. This independent-geometry protocol was frozen before v2 measurement.',
...
if __name__=='__main__':
    ...
    p.add_argument('--run',default='memory_v2_confirmation');p.add_argument('--output-prefix',type=Path,required=True);a=p.parse_args()
