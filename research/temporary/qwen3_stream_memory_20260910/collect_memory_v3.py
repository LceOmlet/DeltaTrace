...
root=Path(__file__).resolve().parent;sha=lambda b:hashlib.sha256(b).hexdigest();phase='memory_v3_'+args.phase;folder=root/phase
...
names=['collect_memory_v3.py']
if args.phase=='confirmation':
    names+=['benchmark_memory_v3.py','memory_v3_confirmation_protocol.json','run_memory_v3_confirmation.py',
            'memory_v3_confirmation_controller.log','summarize_memory_v3_confirmation.py','rollout_termination.json']
elif args.phase=='author':
    names+=['check_memory_v3_author.py','memory_v3_author_plan.json','memory_v3_author.log','check_memory_contract.py','check_memory_v3_graph_probes.py']
else:
    names+=['benchmark_memory_v3_rollout.py','memory_v3_rollout_protocol.json','run_memory_v3_rollout.py','memory_v3_rollout_controller.log']
selected=[root/n for n in names]
selected.extend(f for f in folder.rglob('*') if f.is_file())
for n in ['memory_v3_confirmation_summary.json','memory_v3_confirmation_summary.csv','run_memory_v3_followup.py']:
    if (root/n).exists():selected.append(root/n)
if args.phase=='confirmation':
    selected.extend(f for f in (root/'memory_production_v3_release').rglob('*') if f.is_file() and '__pycache__' not in f.parts and f.suffix!='.pyc')
members={};total=0
