...
p={'status':'running','stages':[]};save=lambda:(A/'controller.json').write_text(json.dumps(p,indent=2)+'\n')
save()
for script,python in [('study.py','/opt/conda/bin/python'),('operator.py','/mnt/geogpt-doc-new/deepresearch/lzq/contrastive_flashtrace_probe/env/bin/python')]:
    t=time.time()
