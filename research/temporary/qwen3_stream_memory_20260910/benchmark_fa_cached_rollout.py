...
def main():
    ...
    assert len(args.lengths.split(','))==1
    assert args.method in protocol['methods']
    ...
    for name,digest in protocol['repair_modules'].items():assert sha((Path(__file__).parent/name).read_bytes())==digest,name
    ...
    try:
        ...
        import resource
        report['model_load_memory']={'peak_allocated_gb':torch.cuda.max_memory_allocated()/1e9,'peak_reserved_gb':torch.cuda.max_memory_reserved()/1e9,'resident_allocated_gb':torch.cuda.memory_allocated()/1e9,'resident_reserved_gb':torch.cuda.memory_reserved()/1e9,'host_process_highwater_bytes':resource.getrusage(resource.RUSAGE_SELF).ru_maxrss*1024}
        ...
        if is_dt:
            ...
            if args.family=='qwen3':
                ...
                if args.method=='deltatrace_streamed':
                    ...
                    from result_template_graph import SACFiniteGraphQwen3
                    from qwen3_shared_mean_fa import VendorFAFiniteP1SharedMeanReuse
                    candidate=protocol['experimental_finite_library']
                    dt_runner=SACFiniteGraphQwen3(model,VendorFAFiniteP1SharedMeanReuse(candidate['path'],candidate['sha256']))
                    report['experimental_finite_library']=candidate
                    report['rounding_repair_sources']=protocol['repair_modules']
                else:
        ...
        report['pre_call_initialization_memory']={'peak_allocated_gb':torch.cuda.max_memory_allocated()/1e9,'peak_reserved_gb':torch.cuda.max_memory_reserved()/1e9,'scope':'Cumulative model load and public controller setup, before attribution peak resets.'}
            ...
            if last_ok:
                ...
                if is_dt:
                    ...
                    if ids.shape[1]<=1024:
                        ...
                        assert case['retained_vector_exact'] and case['retained_math_exact']
                        del before,after,baseline,retained_vec
                    del pair
