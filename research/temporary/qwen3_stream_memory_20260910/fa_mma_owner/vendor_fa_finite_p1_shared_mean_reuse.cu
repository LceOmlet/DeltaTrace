...
extern "C" int deltatrace_fa_finite_p1_shared_mean_reuse(
    ...
    constexpr int shared=std::max(size(typename FiniteTraits::SmemLayoutQ{}),size(typename FiniteTraits::SmemLayoutKV{}))*sizeof(E);
    ...
    constexpr int owner_shared=std::max(size(typename OwnerTraits::SmemLayoutQ{}),size(typename OwnerTraits::SmemLayoutKV{}))*sizeof(E);
