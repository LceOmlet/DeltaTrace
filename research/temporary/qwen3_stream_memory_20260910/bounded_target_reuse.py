...
class NativeProjectionReuse:
    def __init__(self,model,selected):
        ...
        self.activity={'public_API':'torch.utils.checkpoint.create_selective_checkpoint_contexts',
            ...
            'recording_memory_scope':'Allocator counters in per-layer records are graph-recording snapshots, not per-replay measurements. The outer complete caller measures every current peak.', 'selected_projections':list(selected),'policy_detail':'Omit q caching in final16 native layers to leave allocator headroom; other selections unchanged','allow_cache_entry_mutation':False,
        ...
        for index,layer in enumerate(model.model.layers):
            ...
            local_selected=[name for name in selected if not (name=='q' and index>=len(model.model.layers)-16)]
