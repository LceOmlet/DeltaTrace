...
def root_endpoints(model,ids,prompt_len):
    ...
    try:
        with torch.no_grad():
            ...
            result=model(input_ids=ids,attention_mask=None,use_cache=False,logits_to_keep=torch.arange(prompt_len-1,ids.shape[1]-1,device=ids.device))
            logits=result.logits;target=ids[:,prompt_len:]
            assert result.logits.shape[1]==ids.shape[1]-prompt_len
