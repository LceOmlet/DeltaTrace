...
from whole_selected_head_base import WholeProgram,WholeRematFiniteGraphQwen3,root_endpoints,LiveScoresPacked
from native_projection_sac_memory import NativeProjectionReuse
...
from remat_finite_compact_fa import propagate_signed_secant


class SACProgram(WholeProgram):
    from released_graph_build import build
    def invoke(self):
        self.inputs=None
        n=self.ids.shape[1]
        selected=[] if n<=300 else ['down'] if n<=700 else ['k','v','gate','down']
