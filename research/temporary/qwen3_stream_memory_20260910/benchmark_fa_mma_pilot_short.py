...
def main():
    ...
    report={'status':'imports','family':args.family,'method':args.method,
      ...
      'warm_timing_scope':protocol['warm_timing_scope'],
            ...
            if last_ok:
                ...
                if is_dt:
                    ...
                    if args.method=='deltatrace_streamed':
                        ...
                        assert actual_calls==baseline_calls
                    ...
                    import copy
                    case['details']=copy.deepcopy({k:v for k,v in result.items() if k!='signed_full_sequence'})
                    ...
                    if args.method=='deltatrace_streamed':
                        ...
                        assert result['deferred_validation']['predicates']==829
                    ...
                    # A caller may mutate returned metadata. The next call must
                    # produce fresh containers and current numerical values.
                    result['layer_checks'][0]['max_abs_input_multiplier']=-123456.0
                    if result['native_projection_SAC']['regions']:
                        result['native_projection_SAC']['regions'][0]['operations'].clear()
                    t=time.perf_counter();again=dt_runner.attribute(audit_base,ids,mask,prompt_len,mutation_audit=True);torch.cuda.synchronize()
                    case['metadata_mutation_control']={'seconds':time.perf_counter()-t,
                        'vector_exact':bool(np.array_equal(np.asarray(again['signed_full_sequence'],dtype=np.float64),retained_vec)),
                        'math_exact':all(again[k]==baseline[k] for k in keys),
                        'fresh_nested_metadata':all(len(region['operations'])==7 for region in again['native_projection_SAC']['regions'])}
                    assert all(case['metadata_mutation_control'][k] for k in ['vector_exact','math_exact','fresh_nested_metadata'])
                    del again
