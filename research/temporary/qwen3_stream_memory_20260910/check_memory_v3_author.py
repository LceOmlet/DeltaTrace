...
def main():
    ...
    A=p.parse_args().root;R=A/'memory_production_v3_release';B=Path('/tmp/codex_short_b1_efficiency_20260910_v1')
    plan_path=A/'memory_v3_author_plan.json';plan=json.loads(plan_path.read_bytes());out=A/'memory_v3_author';out.mkdir(exist_ok=False)
    ...
    assert sha((A/'check_memory_v3_graph_probes.py').read_bytes())==plan['probe_sha256']
    ...
    try:
        ...
        os.environ.update(MACA_PATH='/opt/maca',HF_HUB_OFFLINE='1',TOKENIZERS_PARALLELISM='false',
            TRITON_CACHE_DIR=str(A/'triton_memory_v3_confirmation_deltatrace_streamed'),TORCHINDUCTOR_CACHE_DIR=str(A/'inductor_memory_v3_confirmation_deltatrace_streamed'))
        ...
        from check_memory_v3_graph_probes import run as graph_probes
