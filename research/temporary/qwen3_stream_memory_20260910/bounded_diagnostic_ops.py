...
def norm_residual_two(x0,x1,w,a,b,res,eps):
 value=old_two(x0,x1,w,a,b,res,eps);reference=old_two(x0.float(),x1.float(),w.float(),a,b,res,eps);compare('norm_two',value,reference);return value
...
from bounded_target_ops import target_seed as bounded_seed,target_logprobs as bounded_logprobs
def target_seed(z0,z1,target):
 a=bounded_seed(z0,z1,target);b=old_seed(z0.float(),z1.float(),target)
 compare('bounded_seed',a,(b[0].half(),b[1]));return a
def target_logprobs(z,target):
 a=bounded_logprobs(z,target)
 b=(z.log_softmax(-1).gather(2,target[:,:,None]).squeeze(-1),z.float().log_softmax(-1).gather(2,target[:,:,None]).squeeze(-1))
 compare('bounded_root_lp',a,b);return a
