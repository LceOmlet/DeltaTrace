...
P=Path(__file__).resolve().parent;plan=json.loads((P/'postflight_memory_v3_plan.json').read_bytes());started=time.perf_counter()
target=P/'postflight_memory_v3.json';assert not target.exists()
...
save()
try:
 assert json.loads((P/'memory_v3_author/results.json').read_bytes())['status']=='complete'
 q=json.loads((P/'memory_v3_rollout/queue.json').read_bytes());assert q['status']=='complete' and len(q['jobs'])==21
 ...
 env=json.loads((P/'memory_production_v3_release/environment.json').read_bytes())['qwen3']
 ...
 (P/'final_memory_v3_mx_smi.txt').write_text(smi.stdout);report['GPU_status_text']=smi.stdout
