...
def main():
    ...
    for name,digest in protocol['pilot_helpers'].items():assert sha((Path(__file__).parent/name).read_bytes())==digest,name
    ...
    try:
        ...
        if args.family=='qwen3':
            from cpu_staged_model_loading import load_common_staged_model
