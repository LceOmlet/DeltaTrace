...
from qwen3_rematerialized_graph import RematProgram
from bounded_diagnostic_ops import target_logprobs
...
def root_endpoints(model,ids,prompt_len):
    ...
    try:
        with torch.no_grad():
            ...
            result=model(input_ids=ids,attention_mask=None,use_cache=False)
            logits=result.logits[:,prompt_len-1:-1];target=ids[:,prompt_len:]
            assert logits.shape[1]==ids.shape[1]-prompt_len
            lp16,lp32=target_logprobs(logits,target)
            ...
            # Release the full original model logits after cloning the original target slice.
...
class WholeRematFiniteGraphQwen3(FiniteGraphQwen3):
    ...
    def __init__(self,model,finite):
        ...
        from qwen3_fixed_graph_contract import FixedModelGraphContract
        self.contract=FixedModelGraphContract(model)
