...
using FiniteTraits=Flash_fwd_kernel_traits<128,64,32,4,false,false,mctlass::half_t>;
...
__global__ void deltatrace_fa_finite_p1_kernel(FiniteParams p) {
    ...
    auto owner0=make_fragment_like(toA);
    auto owner1=make_fragment_like(toA);
    auto ownerU=make_fragment_like(toA);
    {
        auto load_owner=[&](const E *left,auto &reg) {
            ...
            __syncthreads();cute::copy(toA,reg);__syncthreads();
        };
    ...
    for(int col0=begin;col0<end;col0+=N) {
        auto pair=[&](const E *left,const E *right,auto &acc,int endpoint) {
            ...
            if(endpoint==0)cute::copy(owner0,toA);
            else if(endpoint==1)cute::copy(owner1,toA);
            else cute::copy(ownerU,toA);
...
extern "C" int deltatrace_fa_finite_p1_shared_mean_reuse(
    ...
    constexpr int owner_shared=(size(typename OwnerTraits::SmemLayoutQ{})+size(typename OwnerTraits::SmemLayoutKV{}))*sizeof(E);
