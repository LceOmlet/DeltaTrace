...
class NativeProjectionReuse:
    def __init__(self,model,selected):
        ...
        self.activity={'public_API':'torch.utils.checkpoint.create_selective_checkpoint_contexts',
            ...
            'selected_projections':list(selected),'allow_cache_entry_mutation':False,
        ...
        for index,layer in enumerate(model.model.layers):
            ...
            rec={'layer':index,'save_mm_visits':0,'reuse_mm_visits':0,'input_exact':None,'cached_output_bytes':0,'operations':[],'completed':False}
            state={'rec':rec,'weights':weights,'versions':[w._version for w in weights],'phase':0,'input':None,'contexts':None,'native_forward':type(layer).forward}
    ...
    def __exit__(self,kind,error,tb):
        ...
        if kind is None:
            ...
            assert self.activity['saved_mm_outputs']==self.activity['reused_mm_outputs']==len(self.regions)*len(self.activity['selected_projections'])
