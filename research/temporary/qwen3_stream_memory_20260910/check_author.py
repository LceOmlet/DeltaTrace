...
def main():
    ...
    A=p.parse_args().root;R=A/'compact_production_release';B=Path('/tmp/codex_short_b1_efficiency_20260910_v1')
    plan_path=A/'author_plan.json';plan=json.loads(plan_path.read_bytes());out=A/'author';out.mkdir(exist_ok=False)
    ...
    try:
        ...
        os.environ.update(MACA_PATH='/opt/maca',HF_HUB_OFFLINE='1',TOKENIZERS_PARALLELISM='false',
            TRITON_CACHE_DIR=str(A/'triton_confirmation_v2_deltatrace_streamed'),TORCHINDUCTOR_CACHE_DIR=str(A/'inductor_confirmation_v2_deltatrace_streamed'))
        ...
        from graphed_qwen3_streamed import make_streamed_qwen3
        ...
        new,report['root_sources']=make_streamed_qwen3(R,model,env['finite_library'],env['finite_library_sha256'])
        ...
        from check_stream_storage import storage_probes,late_mutation_probes
        assert sha((A/'check_stream_storage.py').read_bytes())==plan['storage_probe_sha256']
        report['stream_storage_controls']=storage_probes(model);save()
        vectors={}
        for index,row in enumerate(plan['cases']):
            ...
            if index==len(plan['cases'])-1:
                report['late_mutation_controls']=late_mutation_probes(model,new,base,ids,mask,row['prompt_length'])
                for control in report['late_mutation_controls']:
                    recovered=np.asarray(control.pop('recovery_vector'),dtype=np.float64)
                    vectors[row['key']+'/'+control['case']+'_recovery']=recovered
                    control['recovery_exact']=bool(np.array_equal(recovered,vectors[row['key']+'/retained']))
                    control['recovery_math_exact']=control.pop('recovery_math')==maths['retained']
                    assert control['recovery_exact'] and control['recovery_math_exact']
