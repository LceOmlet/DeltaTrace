        ...
        if is_dt:
            ...
            if args.family=='qwen3':
                ...
                if args.method=='deltatrace_streamed':
                    ...
                    import compact_qwen3_v2 as streamed_qwen3
                    assert sha(Path(streamed_qwen3.__file__).read_bytes())==protocol['candidate_sha256']
                    for module,digest in protocol['candidate_modules'].items():
                        assert sha(Path(sys.modules[module].__file__).read_bytes())==digest,module
                    dt_runner=streamed_qwen3.StreamedCompactFiniteGraphQwen3(model,dt_runner.finite)
        ...
        for input_len in map(int,args.lengths.split(',')):
            ...
            def make_runner():
                ...
                def call():
                    ...
                    if args.family=='qwen3':
                        ...
                        last_cost.clear();last_cost.update({k:answer.get(k) for k in ['streamed_root','native_graph_execution','root_retention_mutation_audit','graph_input_copy_audit']})
                        return answer
            ...
            if last_ok:
                ...
                if is_dt:
                    ...
                    reference_report=json.loads(Path(protocol['layout_reference_report']).read_bytes())
                    reference_case=next(c for c in reference_report['cases'] if c['input_length']==input_len)
                    case['original_full_layout_metadata_exact']=result['native_paired_root_layouts']==reference_case['details']['native_paired_root_layouts']
                    assert case['original_full_layout_metadata_exact']
