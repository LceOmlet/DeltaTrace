...
try:
 for ri,methods in enumerate(P['method_rounds']):
  for method in methods:
   ...
   env=dict(os.environ,MACA_PATH='/opt/maca',OMP_NUM_THREADS='4',PYTHONDONTWRITEBYTECODE='1',TRITON_CACHE_DIR=str(T/('triton_confirmation_'+method)),TORCHINDUCTOR_CACHE_DIR=str(T/('inductor_confirmation_'+method)))
   if ri==0:assert not Path(env['TRITON_CACHE_DIR']).exists() and not Path(env['TORCHINDUCTOR_CACHE_DIR']).exists()
   command=['/mnt/geogpt-doc-new/deepresearch/lzq/contrastive_flashtrace_probe/env/bin/python','-u',str(T/'benchmark_confirmation.py'),'--root','/tmp/codex_short_b1_efficiency_20260910_v1','--family','qwen3','--method',method,'--lengths',','.join(map(str,P['length_orders'][ri])),'--protocol',str(T/'confirmation_protocol.json'),'--output',str(out/name)]
