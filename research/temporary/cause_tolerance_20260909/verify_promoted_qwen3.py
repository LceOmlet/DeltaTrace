"""Bounded Qwen3 NI0/MH0 deferred-validation optimization pilot.
...
def main():
    ...
    candidate, promoted_receipt = make_deferred_qwen3(args.release)
    ...
    report = {'status': 'loading', 'promoted_receipt': promoted_receipt, 'script_sha256': sha(Path(__file__).read_bytes()),
              'candidate_sha256': sha(Path(inspect.getfile(candidate)).read_bytes()),
              'candidate_sources': {n:sha((args.release/'deltatrace/accelerated/qwen3'/n).read_bytes()) for n in ('deferred_validation.py','qwen3_deferred_pair.py','qwen3_deferred_finite.py','qwen3_deferred_replay.py')},
