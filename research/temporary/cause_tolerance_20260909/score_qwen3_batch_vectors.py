"""Original author metrics for both saved vectors from the paired batch run.
...
def main():
    ...
    for task in ('niah_mq_q2','morehopqa'):assert sha((official/'exp/exp2/data'/(task+'.jsonl')).read_bytes())==protocol['tasks'][task]['cache_sha256']
    ...
    assert len(cases)==len(set(k for k,_ in cases))==16
    ...
    report={'status':'loading','driver_sha256':sha(Path(__file__).read_bytes()),'batch_result_sha256':args.batch_result_sha256,
            ...
            'metric_sample_batch':1,'DT_sample_batch_by_mode':{'single':1,'batch':2}}
    def save():
    ...
    try:
        ...
        cached={task:dataset_utils.load_cached(official/'exp/exp2/data'/(task+'.jsonl')) for task in ('niah_mq_q2','morehopqa')}
