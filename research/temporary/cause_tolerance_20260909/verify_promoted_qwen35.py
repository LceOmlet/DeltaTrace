"""Cost-first pilot of controller diagnostics scheduling; all finite formulas unchanged.

Defers the same controller statistics and finite checks until API return.
Ordinary model/FA/FLA and finite arithmetic are unchanged. Full native DT calls on NI0/MH0,
...
def main():
    ...
    try:
        ...
        runner=configure_dynamic_finite(Qwen35DenseFiniteRunner(model,fa,None,checkpoint_device='cuda'))
        original=runner.finite_fla
        baseline_runner=runner
        deferred_runner,report['promoted_receipt']=make_deferred_qwen35(args.release,model,fa,original)
        report['deferred_controller_sha256']=sha(Path(inspect.getfile(type(deferred_runner).attribute)).read_bytes())
        for row in rows:
            ...
            for repeat,mode in enumerate(('baseline','deferred','baseline','deferred','deferred','baseline')):
                runner=baseline_runner if mode=='baseline' else deferred_runner
            ...
            a=vectors[case+'_baseline_measured5'];b=vectors[case+'_deferred_measured4']
            report['comparisons'].append({'case':case,'relative_l2':float(np.linalg.norm(b-a)/max(np.linalg.norm(a),1e-30)),
                ...
                'deferred_repeat_equal':bool(np.array_equal(b,vectors[case+'_deferred_measured3']))})
        total=lambda mode:sum(c['seconds'] for c in report['calls'] if c.get('warm') and '_'+mode+'_' in c['name'])
        report['mean_latency_reduction_fraction']=1-total('deferred')/total('baseline')
