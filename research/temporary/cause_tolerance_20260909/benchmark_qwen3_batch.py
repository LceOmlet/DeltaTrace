...
def main():
    ...
    reference=json.loads(args.reference.read_bytes());assert reference['status']=='complete' and reference['family']=='qwen3'
    ...
    for task in ('niah_mq_q2','morehopqa'):
        ...
        groups.extend([selected[i:i+2] for i in (range(0,8,2) if args.phase=='development16' else (0,))])
    ...
    report={'status':'loading','driver_sha256':sha(Path(__file__).read_bytes()),'reference_sha256':args.reference_sha256,
            'phase':args.phase,'baseline_sources':source,'candidate_sources':{n:sha((Path(__file__).parent/n).read_bytes()) for n in
                ('qwen3_batch_pair.py','qwen3_batch_capture.py','qwen3_batch_layout.py','qwen3_batch_finite.py')},
            'groups':[[f"{c['dataset']}_{c['index']}" for c in g] for g in groups],
            ...
            def run():
                ...
                if label.startswith('warm'):
                    for b,row in enumerate(group):
                        ...
                        # Detect a real packing/causality error before interpreting quality.
                        assert check['last_hidden_relative_l2']<.01 and check['target_logp_max_absolute']<.1,check
                        del a,v,lp,newlp
