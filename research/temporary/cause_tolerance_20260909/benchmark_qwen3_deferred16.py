"""Qwen3 original development16 validation-scheduling regression and full-call cost.
...
def main():
    ...
    rows = sorted([r for r in reference['cases'] if r['status']=='complete'],key=lambda r:(r['dataset'],r['index']))
    assert len(rows)==16 and {(r['dataset'],r['index']) for r in rows}=={(t,i) for t in ('niah_mq_q2','morehopqa') for i in range(8)}
    ...
    from qwen3_deferred_pair import propagate_paired_secant as candidate
    ...
    report = {'status': 'loading', 'script_sha256': sha(Path(__file__).read_bytes()),
              ...
              'candidate_sources': {n:sha((Path(__file__).parent/n).read_bytes()) for n in ('deferred_validation.py','qwen3_deferred_pair.py','qwen3_deferred_finite.py','qwen3_deferred_replay.py')},
    ...
    try:
        ...
        report['delayed_guard_checks'] = {}
        for label, tensor in [('valid', torch.tensor([1.],device='cuda')),('nonfinite',torch.tensor([float('nan')],device='cuda'))]:
            ledger=DeferredValidation(); ledger.finite(tensor,label)
            failed=False
            try: ledger.finish({})
            except ValueError: failed=True
            assert failed==(label=='nonfinite')
            report['delayed_guard_checks'][label]={'raised_before_return':failed,'expected':label=='nonfinite'}
        ...
        for i, row in enumerate(rows):
            ...
            for repeat, mode in enumerate(('baseline', 'candidate', 'baseline', 'candidate', 'candidate', 'baseline')):
                ...
                def attribute():
                    ...
                    start = time.perf_counter()
                    propagate = propagate_paired_secant if mode == 'baseline' else candidate
                    result = propagate(model, before, after, pv_rule='content_P1', finite_attention=finite_fa)
                    stages['validation'] = result.get('deferred_validation')
                    torch.cuda.synchronize()
