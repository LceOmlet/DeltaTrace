...
def save():(p/'profile_queue.json').write_text(json.dumps(r,indent=2)+'\n')
for family in ['qwen35','qwen3']:
 ...
 with (p/(family+'_profile.log')).open('w') as log:
  child=subprocess.Popen([python,'-u',str(p/'profile.py'),'--root',str(base),'--family',family,'--method','deltatrace_retained','--lengths','128','--protocol','benchmark_protocol_v2.json','--output',str(p/(family+'_profile'))],stdout=log,stderr=subprocess.STDOUT,stdin=subprocess.DEVNULL,start_new_session=True,env=dict(os.environ,MACA_PATH='/opt/maca',OMP_NUM_THREADS='4',PYTHONDONTWRITEBYTECODE='1'))
  row['pid']=child.pid;save();row['exit_code']=child.wait(timeout=1200)
 row['seconds']=time.time()-row['started'];row['status']=json.loads((p/(family+'_profile/results.json')).read_bytes())['status'];save()
