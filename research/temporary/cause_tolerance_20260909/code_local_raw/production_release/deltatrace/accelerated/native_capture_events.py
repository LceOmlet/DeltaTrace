...
class LocalCaptureEvents:
    ...
    def __enter__(self):
        if self.callback is None:raise RuntimeError('Capture context is single-use.')
    ...
    def __exit__(self,*args):
        ...
        try:
            ...
            # The bound callback owns the capture, which owns this context.
            # Break that cycle immediately so replay tensors die at layer exit.
            self.callback=None
...
def check_runtime():
    ...
    import weakref
    class Owner:
        def observe(self,frame,event,value):pass
    owner=Owner();reference=weakref.ref(owner)
    owner.events=LocalCaptureEvents([observed.__code__],owner.observe)
    with owner.events:observed(1)
    del owner
    assert reference() is None, 'No cyclic-GC pass should be needed to release the capture.'
    return {'callback_frames_and_values':True,'tool_ids_restored':True,
            'exception_cleanup':True,'callback_owner_released_without_gc':True}
