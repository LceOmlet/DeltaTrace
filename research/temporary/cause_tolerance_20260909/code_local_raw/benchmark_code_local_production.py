...
def main():
    ...
    report={'status':'imports','family':args.family,'method':args.method,
      ...
      'warm_timing_scope':'One warm call per mode and length, then retained/local/local/retained/retained/local measured calls. Author reset/empty_cache before each call.',
    ...
    try:
        ...
        if is_dt:
            ...
            if args.family=='qwen3':
                ...
                local_runner,report['code_local_sources']=make_code_local_qwen35(R,model,finite_fa,finite_fla)
        ...
        for input_len in map(int,args.lengths.split(',')):
            ...
            def make_runner():
                ...
                def call():
                    ...
                    signed,detail=(dt_runner if mode=='retained' else local_runner).attribute(pair,torch.ones_like(pair),selection,select_output_rows=True,observer=None)
                    return {'signed_full_sequence':signed[0].cpu().tolist(),'details':detail}
                return call
            modes=['retained','local']
            ...
            if last_ok:
                for mode in modes:
                    ...
                    if args.family=='qwen35':case.setdefault('details',{})[mode]=result['details']
