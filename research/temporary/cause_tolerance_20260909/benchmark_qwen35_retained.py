...
def main():
    ...
    try:
        ...
        from qwen35_retained_controller import Qwen35DenseFiniteRunner as RetainedRunner
        from dynamic_finite import configure_dynamic_finite
        accelerated = configure_dynamic_finite(RetainedRunner(model,finite_fa,finite_fla,checkpoint_device='cuda'))
        report['candidate_sources'] = {n:sha((Path(__file__).parent/n).read_bytes()) for n in ('qwen35_retained_controller.py','qwen35_retained_capture.py')}
        ...
        for index in group_indices:
            ...
            if args.phase=='pilot':
                capture.AUDIT=True
                try:execute(f'audit_capture/{index}',accelerated,group,plan['batches'][index]['actual_root'])
                finally:capture.AUDIT=False
                report['capture_mutation_audit']=capture.AUDIT_RECORDS.copy()
