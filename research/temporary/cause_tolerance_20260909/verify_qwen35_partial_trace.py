"""Reconstruct fixed deletion sets and all signed telescoping sums from receipts."""
import argparse
import hashlib
import json
...
def main():
    ...
    rows = {}; reports = {}; cases = []
    for job, driver, count in [('qwen35_partial_trace', 'trace_qwen35_partial_deletion.py', 4),
                               ('qwen35_partial_remaining12', 'trace_qwen35_partial_deletion_v2.py', 12)]:
        folder = HERE / job; r = json.loads((folder / 'results.json').read_bytes())
        assert r['status'] == 'complete' and len(r['cases']) == count
        assert r['script_sha256'] == sha((HERE / driver).read_bytes())
        assert r['vectors_sha256'] == sha((folder / 'vectors.npz').read_bytes())
        assert len(r['calls']) == 4 * count + 2
        for path, digest in r['references']:
            source = a.snapshots / path.lstrip('/'); raw = source.read_bytes(); assert sha(raw) == digest
            for c in json.loads(raw)['cases']:
                if c['status'] == 'complete': rows[f"{c['dataset']}_{c['index']}"] = c
        ...
        for c in r['cases']:
            ...
            assert len(c['layers']) == 32 and {l['layer'] for l in c['layers']} == set(range(32))
            for method in ('DT', 'FT_K1'):
                indices = old['metrics'][method]['deleted_user_indices'][2]
                assert c['deleted_sets'][method] == indices
                ids = np.array(old['input_ids'], dtype=np.int64); ids[[old['user_positions'][j] for j in indices]] = ids[-1]
                digest = sha(ids.tobytes()); assert digest == c['partial_input_hashes'][method] == old['metrics'][method]['actual_input_hashes'][2]
                ...
                assert abs(mass - saved['allocated_mass']) < 1e-9 and actual == saved['actual_drop']
                assert abs(mass - c['boundaries']['0'][method]) < 1e-7
                groups = {'head_seed': c['boundaries']['norm'][method] - actual,
                          'final_norm': c['boundaries']['32'][method] - c['boundaries']['norm'][method]}
                for layer in c['layers']:
                    ...
                    for name, value in v['errors'].items():
                        field = layer['mixer'] if name == 'mixer' else name; groups[field] = groups.get(field, 0.) + value
        ...
        reports[job] = {'raw_sha256': sha((folder / 'results.json').read_bytes()), 'vectors_sha256': r['vectors_sha256'],
                        'cases': count, 'complete_DT_calls': 2 * count, 'partial_paired_root_calls': 2 * count,
                        'initialization_root_calls': 1, 'charged_seconds_excluding_load': sum(c['seconds'] for c in r['calls'] if c['name'] != 'load')}
    assert len(cases) == len({c['case'] for c in cases}) == 16
    summaries = []
    for dataset in ('niah_mq_q2', 'morehopqa'):
        selected = [c for c in cases if c['dataset'] == dataset]; assert len(selected) == 8
        ...
        summaries.append({'dataset': dataset, 'cases': 8, 'DT_set_less_destructive': sum(c['native_FA_drop_gap_DT_minus_FT'] < 0 for c in selected),
    ...
    report = {'status': 'verified', 'runs': reports, 'cases': cases, 'summaries': summaries,
              'scope': 'Same native paired FA/default-BF16 model, FP32 logsoftmax/FP64 sum, full target; diagnostic objective matches DT. Original frozen 10% sets retained. This does not rerun the author 21-point metric or assign exact RISE-gap percentages.',
              'limits': 'Signed local residuals account for the fixed linearization error, not causal mediation or a mandate to patch a layer. Pilot4 is reused with remaining12, each sample internally same-process. Qwen3 comparison and family-level causal repair remain outstanding.'}
    (HERE / 'qwen35_partial_trace_summary.json').write_text(json.dumps(report, indent=2) + '\n', newline='\n')
