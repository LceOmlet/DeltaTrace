...
Published factory integration check: NI0/MH0, 12 complete DT calls.
...
def main():
    ...
    if args.phase=='pilot':rows=[r for r in original if r['index']==0]
    else:rows=original if args.phase=='development16' else extra
    assert len(rows)=={'pilot':2,'development16':16,'extra6':6}[args.phase]
    ...
    candidate,candidate_receipt=make_retained_qwen3(args.release)
    ...
    report={'status':'loading','phase':args.phase,'script_sha256':sha(Path(__file__).read_bytes()),'baseline_sources':receipt,
        'candidate_sources':candidate_receipt,
