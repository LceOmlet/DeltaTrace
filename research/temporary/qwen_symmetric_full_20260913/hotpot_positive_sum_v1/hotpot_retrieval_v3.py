...
def rank_sentences(scores, units, all_groups, eligible_groups, *, pooling):
    ...
    if pooling not in ('signed_sum','positive_sum','positive_mean_eligible'):raise ValueError('Unknown pooling')
    ...
    for i in candidates:
        ...
        elif pooling=='positive_sum':
            # Clip each token before pooling so negative tokens cannot cancel positive evidence.
            pooled[i]=float(np.sum(np.maximum(values[all_groups[i]],0),dtype=np.float64))
        else:
