...
class NativeFLAStageCapture:
    ...
    def __enter__(self):
        if self.active or self.records or self.counts or sys.getprofile() is not None:
    ...
    def clear(self):
        ...
        # Invalidate borrowed stage dictionaries as well as our outer list.
        # Individual tensors deliberately retained by a caller remain its responsibility.
        for row in self.records:
            row['values'].clear()
            row['versions'].clear()
        self.records.clear()
