import hashlib,json,zipfile,statistics
from collections import Counter
from pathlib import Path
import numpy as np
A=Path(__file__).resolve().parent;F=A/'snapshot${ARTIFACT_ROOT}/codex_fa_shared_mean_integration_20260907_v1'
...
assert d['status']=='FA_shared_mean_B1_and_real_B4_exact'
...
out={'status':'FA_shared_mean_B1_and_real_B4_verified','raw_sha256':sha(F/'results.json'),
    'protocol_sha256':sha(F/'protocol.json'),'attributions':12,'quality_queries':0,'VJPs':0,
...
for index,g in enumerate(d['integration_groups']):
    ...
    for repeat in range(3):
        old,new=[next(r['result'] for r in g['runs'] if r['mode']==mode and r['repeat']==repeat) for mode in ['old','shared_mean']]
        ...
        for mode,result in [('old',old),('shared_mean',new)]:
            ...
            for item in finite:
                ...
                if mode=='shared_mean':assert item['endpoint_mean']=='FP32_add_FP16_store_in_FA_shared_tile_from_existing_loads'
        assert g['comparisons'][repeat]['full_signed_exact'] and g['comparisons'][repeat]['endpoints_exact']
    measured={mode:[r['result']['seconds'] for r in g['runs'] if r['mode']==mode and not r['warm']] for mode in ['old','shared_mean']}
    peaks={mode:[r['result']['peak_allocated_bytes'] for r in g['runs'] if r['mode']==mode and not r['warm']] for mode in ['old','shared_mean']}
    ...
    out['groups'].append({'selection':g['selection'],'example_batch_size':batch,
        ...
        'fused_to_old_median_ratio':statistics.median(measured['shared_mean'])/statistics.median(measured['old']),
        'timing_scope':'Two measured pairs per shape, same-job current compact-GQA baseline; no FT comparison.'})
    numeric.append(g)
profile=d['shared_mean_B4_profile'];assert sha(F/profile['file'])==profile['sha256']
...
(A/'fa_shared_mean_integration_summary_20260907.json').write_text(json.dumps(out,indent=2))
(A/'fa_shared_mean_integration_numeric_20260907.json').write_text(json.dumps({'groups':numeric},separators=(',',':')))
