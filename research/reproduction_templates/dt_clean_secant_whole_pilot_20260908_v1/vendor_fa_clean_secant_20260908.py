...
ROW_FIELDS = ('s_anchor', 's_mean_offset', 'g_anchor', 'g_mean_offset', 'inverse_p1_sum',
              'alpha', 'variance_M2', 'numerator', 'raw_p0_sum', 'raw_p1_sum',
              'raw_route_target', 'normalized_route_target', 'endpoint_positive',
              'endpoint_negative', 'base_gradient_contraction')


class VendorFACleanSecant:
    def __init__(self, library, expected_sha256):
        path = Path(library)
        if hashlib.sha256(path.read_bytes()).hexdigest() != expected_sha256:
            raise ValueError('Clean-secant library hash mismatch.')
        self.library = ctypes.CDLL(str(path))
        self.operation = self.library.deltatrace_fa_finite_p1_clean_secant
    ...
    def __call__(self, operands, scale, layout):
        ...
        if status != 0:
            raise RuntimeError(f'Clean-secant finite FA launch failed: {status}')
