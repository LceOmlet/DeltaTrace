"""Actual MH3 conditional decoder boundaries;one fixed DT and four original scores."""
...
def timeout(*args):raise TimeoutError('Frozen1DT4score MH3 actual-boundary budget expired.')
def timed(kind,fn):
...
class BoundaryObserver:
    def __init__(self):
        self.coeff={};self.paired={};self.endpoint_effect={};self.activations={};self.points={};self.counts={};self.logp={};self.current=None;self.k={'B1':{}}
    def wants_decoder(self,index):return index==1
...
try:
    ...
    assert p['call_schedule']==[['morehopqa_3','DT']] and p['capture_steps']==[0,3,10,20]
    ...
    source_case=prior['cases']['morehopqa_3'];case=cases['morehopqa_3']
    ...
    source_score=torch.from_numpy(source_vectors['morehopqa_3_'+p['source_method']+'_evaluated'])
    ...
    assert torch.equal(torch.from_numpy(source_vectors['morehopqa_3_'+p['source_method']+'_full'])[:len(source_score)].float(),source_score)
    ...
    key='morehopqa_3';case=cases[key];info=case['input'];row=r['cases'][key]
    ...
    try:
        for step in p['capture_steps']:
            ...
            with NativeFreshKCapture(layers[1].linear_attn) as captured_k,torch.no_grad():
    ...
    row['K_diagnostic'],kv=timed('CPU_K_conditional_geometry',lambda:analyze(observer.k,info['prompt_length']))
    ...
    path=A/'MH3_actual_boundary_private.pt';timed('save_actual_private_boundary_tensors',lambda:torch.save(private,path))
    ...
    r['vectors_sha256']=sha((A/'vectors.npz').read_bytes());r['status']='MH3_native_K_boundaries_1DT4score_complete'
    ...
    r['cost_scope']='One current DT with passive34boundary coefficient copies and focused GDN1 K diagnostics;four original B1 scorer calls on predeclared standalone masks0/3/10/20. Existing1model-load/1NI0eager initialization,32native decoder replays,8FAfinite/8FAaux,25FLAbackends/50nativeadjoints. Boundary CPU double contractions,FP32diagnostics of the same actual B1 logits and private tensor save cost are all measured. No new MAS curve,FT,generation,newcandidate propagation,extraGDN/conv or precision repair. Four passive native K captures and CPU current/candidate contractions are included; private boundaries retained only for layers0/1/2. All entered/returned and failed work retained;diagnostic cost is not production timing.'
    save()
