...
old=VendorFAFiniteP1(p['old_library'],p['old_library_sha256'])
new=VendorFAFiniteP1CompactGQA(A/'libdeltatrace_fa_finite_gqa.so',b['library']['sha256'])
...
try:
    for repeat in range(3):
        outputs={}
        for name in (['old','compact'] if repeat!=1 else ['compact','old']):
            ...
            outputs[name]=(old if name=='old' else new)(ops if name=='old' else compact,record['scale'],activity)
        ...
        for key in outputs['old']:
            a,c=outputs['old'][key],outputs['compact'][key]
        ...
        assert all(x['exact'] for x in checks.values()),'Address mapping must preserve existing operator outputs.'
        del outputs
    r['status']='operator_exact_no_end_to_end_claim'
except Exception:
