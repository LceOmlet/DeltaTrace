"""Actual MH0 conditional decoder boundaries;one fixed DT and four original scores."""
...
def timeout(*args):raise TimeoutError('Frozen1DT4score MH0 actual-boundary budget expired.')
def timed(kind,fn):
...
try:
    ...
    assert p['call_schedule']==[['morehopqa_0','DT']] and p['capture_steps']==[0,3,10,20]
    ...
    source_case=prior['cases']['morehopqa_0'];case=cases['morehopqa_0']
    assert source_case['input']==case['input']
    curve=source_case['curves']['DT'];assert curve['status']=='complete'
    order=curve['sorted_keep'];assert sorted(order)==case['input']['keep']
    source_score=torch.from_numpy(source_vectors['morehopqa_0_DT_evaluated'])
    ...
    assert torch.equal(torch.from_numpy(source_vectors['morehopqa_0_DT_full'])[:len(source_score)].float(),source_score)
    ...
    for number,(key,method) in enumerate(p['call_schedule']):
        ...
        reference=torch.from_numpy(source_vectors[key+'_DT_full'])
        ...
        row['source_B2_root_effect']=prior['runs'][0]['details']['root_effect']
    ...
    key='morehopqa_0';case=cases[key];info=case['input'];row=r['cases'][key]
    curve=prior['cases'][key]['curves']['DT'];row['source_curve_scope']='Standalone current-DT masks frozen before this diagnostic; no new sorting or original MAS curve.'
    row['points']={};row['decomposition']={}
    current_score=torch.from_numpy(vectors[key+'_DT_evaluated'])
    source_score=torch.from_numpy(source_vectors[key+'_DT_evaluated'])
    ...
    path=A/'MH0_actual_boundary_private.pt';timed('save_actual_private_boundary_tensors',lambda:torch.save(private,path))
    ...
    r['vectors_sha256']=sha((A/'vectors.npz').read_bytes());r['status']='MH0_current_conditional_boundaries_1DT4score_complete'
