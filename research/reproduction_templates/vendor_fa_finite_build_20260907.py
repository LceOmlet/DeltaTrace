...
assert sha(A/'vendor_fa_finite_p1.cu')==p['extension_sha256']
...
try:
    ...
    command+=[str(A/'vendor_fa_finite_p1.cu'),'-o',str(A/'libdeltatrace_fa_finite.so')]
    ...
    if result.returncode==0:
        lib=A/'libdeltatrace_fa_finite.so';r['library']={'bytes':lib.stat().st_size,'sha256':sha(lib)}
        ...
        assert 'deltatrace_fa_finite_p1' in r['symbols']['stdout']
except Exception:
    ...
    with zipfile.ZipFile(A/'review_bundle.zip','w',zipfile.ZIP_DEFLATED) as z:
        for name in ['study.py','protocol.json','vendor_fa_finite_p1.cu','results.json','compile.log']:
