"""Same MH0 actual FT0 and fixed current DT;one full Both trace,one DT,42originalscores."""
...
def timeout(*args):raise TimeoutError('Frozen1FT1DT42score MH0 comparison budget expired.')
def timed(kind,fn):
...
try:
    ...
    assert p['call_schedule']==[['morehopqa_0','DT']] and p['quality_schedule']==[['morehopqa_0','FT0'],['morehopqa_0','DT']]
    ...
    for key,case in cases.items():
        r['cases'][key]={'input':case['input'],'gold':case['gold'],'mapping':case['mapping'],
            ...
            'role':'initialization_only' if key=='niah_mq_q2_0' else 'same_input_FT0_DT_quality'}
    ...
    key='morehopqa_0';case=cases[key];info=case['input'];ft={'status':'entered','root_entered':0,'root_returned':0,'input_receipts':[],'events':[]}
    r['cases'][key]['FT']=ft;counts=Counter();r['status']='unchanged_complete_FT_Both_MH0';save()
    ...
    assert sys.getprofile() is None and r['FT_entered']==0;r['FT_entered']+=1;tick=time.perf_counter();sys.setprofile(observe_ft)
    try:
    ...
    assert r['FT_calls']==1 and ft['root_entered']==ft['root_returned']==1
    ...
    assert ifr['n_hops']==3
    ft['deletion_audit']=deletion_audit(all_scores[0],case['ids'],info['keep'],tokenizer.eos_token_id)
    ft['status']='complete';ft['scored_hop']=0;ft['unscored_hops']=[1,2,3]
    ...
    for key,method in p['quality_schedule']:
        ...
        run=r['cases'][key]['FT'] if method=='FT0' else next(x for x in r['runs'] if x['case']==key and x['method']==method)
        ...
        def before_score(_module,args,kw):
            step=len(curve['input_receipts']);assert step<21 and r['scorer_entered']<42
    ...
    assert r['scorer_entered']==r['scorer_returned']==42
    row=r['cases']['morehopqa_0'];dt=row['curves']['DT'];ft0=row['curves']['FT0']
    row['clean_and_allEOS_scores_equal_between_methods']=[dt['scores'][i]==ft0['scores'][i] for i in (0,20)]
    row['DT_minus_FT0_return_metrics']=[a-b for a,b in zip(dt['return_metrics'],ft0['return_metrics'])]
    row['scope']='Predeclared FT0 only;no quality inference about unscored FT1-3. Shared exact fixed input/target/keep/FA scorer;internal method targets remain original FT sink representation vs DT entire response logprob including EOS.'
    ...
    r['vectors_sha256']=sha((A/'vectors.npz').read_bytes());r['status']='MH0_current_DT_pristine_FT0_1DT1Both42score_complete'
    ...
    r['cost_scope']='Same full model resident. One normal current DT,one original complete FT Both(hops3),42actual common FA scores. DT includes checkpoint/replay/endpoint copies and extra original finite FLA;FT includes all original eager attention,hybrid replays,24identity-valued FLA probes and all recursive hops,although onlyFT0 is scored. FT public trace time includes passive Python counts;DT normal diagnostics differ. Cold/single-call costs reported,not a fair steady FT0-only speed estimate. All load/init/compile/scoring/failure wall time retained. No zeroing/clipping/repair of official FT scores or framework exceptions.'
    save()
