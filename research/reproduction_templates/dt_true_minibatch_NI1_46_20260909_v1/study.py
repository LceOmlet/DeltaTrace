"""True two-example NI1+46 batch comparison;9DT225FLA and no scorers."""
...
def timeout(*args):raise TimeoutError('Frozen9DT225FLA true batch budget expired.')
def timed(kind,fn):
...
try:
    ...
    prior_raw=Path(p['prior_stability']['results_path']).read_bytes()
    assert sha(prior_raw)==p['prior_stability']['results_sha256'];prior=json.loads(prior_raw)
    ...
    fas={'current':BudgetedFinite(original_fa,72)}
    finite_fla=BudgetedFinite(make_compiled_finite_pullback(reuse_scalar_products=False),225)
    candidate_backend=Layer0FLAEndpointAverage(finite_fla)
    runner=Qwen35DenseFiniteRunner(model,fas['current'],finite_fla,norm_gate_rules={0:'symmetric'},finite_fla_by_layer={0:candidate_backend})
    ...
    assert p['group_schedule']==[['single_pair','warm'],['batch2','warm'],['single_pair','measured'],
        ['batch2','measured'],['batch2','measured'],['single_pair','measured']]
    r['groups']=[]

    def invoke(selected,group_number,mode,phase):
        ...
        row={'number':number,'group':group_number,'mode':mode,'phase':phase,'samples':selected,
        ...
        oldfa=(fas['current'].entered,fas['current'].returned);oldfla=(finite_fla.entered,finite_fla.returned)
        ...
        assert r['DT_entered']<9;r['DT_entered']+=1;tick=time.perf_counter();details=None
        try:
            ...
            row['finite_callback_counts']={'FA_entered':fas['current'].entered-oldfa[0],
                'FA_returned':fas['current'].returned-oldfa[1],
        ...
        row['counts']=check_counts(details,fas['current'].returned-oldfa[1],finite_fla.returned-oldfla[1]-1)
    ...
    for group_number,(mode,phase) in enumerate(p['group_schedule']):
        group={'number':group_number,'mode':mode,'phase':phase,'samples':keys,'status':'entered','invocations':[]}
        r['groups'].append(group);group_tick=time.perf_counter()
        batches=[[key] for key in keys] if mode=='single_pair' else [keys]
        try:
            for selected in batches:group['invocations'].append(invoke(selected,group_number,mode,phase))
            group['attribute_seconds_sum']=sum(r['runs'][n]['outer_attribute_seconds'] for n in group['invocations'])
            group['runner_seconds_sum']=sum(r['runs'][n]['details']['complete_attribution_seconds_with_diagnostics'] for n in group['invocations'])
            group['peak_allocated']=max(r['runs'][n]['details']['peak_allocated'] for n in group['invocations'])
            group['peak_reserved']=max(r['runs'][n]['details']['peak_reserved'] for n in group['invocations'])
            group['status']='complete'
        finally:group['wall_seconds_including_audit_and_setup']=time.perf_counter()-group_tick;save()
    assert r['DT_entered']==r['DT_returned']==9 and sum(x['example_batch_size'] for x in r['runs'])==12
    assert fas['current'].entered==fas['current'].returned==72
    assert finite_fla.entered==finite_fla.returned==225 and candidate_backend.entered==candidate_backend.returned==9
    ...
    r['per_sample_comparisons']=[]
    for row in r['runs']:
        for record in row['sample_records']:
            reference=next(x for run in r['runs'] if run['mode']=='single_pair' and run['phase']=='warm'
            ...
            r['per_sample_comparisons'].append({'run':row['number'],'mode':row['mode'],'phase':row['phase'],'case':record['case'],
                'reference_scope':'Same-run warm singleton of the identical original sample; report drift, no precision threshold or repair.',
                ...
                'needle':record['needle'],'needle_minus_single':None if record['needle'] is None else record['needle']-reference['needle'],
    ...
    r['cost_summary']={}
    for mode in ['single_pair','batch2']:
        groups=[g for g in r['groups'] if g['phase']=='measured' and g['mode']==mode]
        assert [g['number'] for g in groups]==([2,5] if mode=='single_pair' else [3,4])
        times=[g['attribute_seconds_sum'] for g in groups]
        r['cost_summary'][mode]={'groups':[g['number'] for g in groups],'seconds_for_two_examples':times,
            'mean_seconds_for_two_examples':float(np.mean(times)),'median_seconds_for_two_examples':float(np.median(times)),
            'median_examples_per_second':float(2/np.median(times)),
            'peak_allocated':[g['peak_allocated'] for g in groups],'peak_reserved':[g['peak_reserved'] for g in groups]}
    r['cost_ratio_batch_over_sequential_pair']=r['cost_summary']['batch2']['median_seconds_for_two_examples']/r['cost_summary']['single_pair']['median_seconds_for_two_examples']
    ...
    r['vectors_sha256']=sha((A/'vectors.npz').read_bytes());r['status']='true_distinct_NI1_46_batch2_9DT225FLA_no_scores_complete'
    ...
    r['cost_scope']='Same model, same current method and two distinct original NI records. S=two sequential singleton attributes;B=one two-example attribute(4actual endpoint rows). Warm S/B also supplies the singleton/batch validation. Only subsequent S/B/B/S groups enter the primary cost ratio. Sum of full normal attribute wall times includes native head/checkpoints,endpoint copies,all FA/FLA/conv and six means;request packing/audit/vector persistence outside attribute is separately retained in group wall time. Two observations/mode;no length or larger-batch extrapolation. No scorer,FT,new generation,duplicate sample or unmasked EOS padding.'
    save()
