"""One fixed remaining-case PV19 regression segment;4DT100FLA84originalscores."""
...
def timeout(*args):raise TimeoutError('Frozen4DT100FLA84score layer19 PV budget expired.')
def timed(kind,fn):
...
try:
    ...
    remaining_pairs=[['niah_mq_q2_0','morehopqa_2'],['niah_mq_q2_2','morehopqa_1'],['niah_mq_q2_3','morehopqa_3']]
    assert type(p['segment_index']) is int and p['segment_index'] in range(3)
    ...
    r['completion_scope']='Only this fixed two-case segment; other prepared segments are not executed or claimed complete.'
    ...
    runners={method:Qwen35DenseFiniteRunner(model,backend,finite_fla,norm_gate_rules={0:'symmetric'},
        ...
        attention_pv_rules=({19:'content0'} if method=='candidate' else {})) for method,backend in fas.items()}
    assert runners['control'].attention_pv_rules=={} and runners['candidate'].attention_pv_rules=={19:'content0'}
    ...
    for number,(key,method,phase) in enumerate(p['call_schedule']):
        ...
        assert details['attention_pv_rules']==({'19':'content0'} if method=='candidate' else {})
        row['PV_rule_scope']=details['attention_pv_rules']
        row['finite_FA_phase_accounting']={'returned_calls':row['counts']['finite_FA_calls'],'phases_from_returned_calls':3*row['counts']['finite_FA_calls'],'library':p['finite_FA_library']}
    ...
    r['vectors_sha256']=sha((A/'vectors.npz').read_bytes());r['status']='layer19_PV_content0_remaining_segment_4DT100FLA84score_complete'
    ...
    r['cost_scope']='One resident full model,one observation per method/case in opposite order,pilot cost only. Both use identical original finite FA library and32 total existing3phase calls; candidate only swaps actual endpoints/value reference at decoder19. Common layer0 norm symmetric and two finite FLA orientations remain. All default native forwards,32-layer replay,endpoint copies,finite operations,normal stage diagnostics and CPU checkpoint copies are timed. No focused observer, extra activation capture,new kernel, or new extension compilation. Load,eager initialization and original scorers remain in wall time.'
