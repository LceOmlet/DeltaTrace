"""Fixed MH0/NI1 all8FA supported-secant pilot;4DT100FLA84originalscores."""
...
fas={};finite_fla=None;candidate_backend=None;supported_backend=None

def save():
    f=A/'results.partial';f.write_text(json.dumps(r,indent=2,allow_nan=False));f.replace(A/'results.json')
def timeout(*args):raise TimeoutError('Frozen4DT100FLA84score supported-secant budget expired.')
def timed(kind,fn):
...
try:
    ...
    from vendor_fa_supported_secant_runner_20260909 import AllFASupportedSecantBackend
    ...
    assert p['call_schedule']==[['morehopqa_0','control','quality'],['morehopqa_0','candidate','quality'],['niah_mq_q2_1','candidate','quality'],['niah_mq_q2_1','control','quality']]
    ...
    supported_backend=AllFASupportedSecantBackend(p['candidate_library'],p['candidate_library_sha256'])
    fas={'control':BudgetedFinite(original_fa,16),'candidate':BudgetedFinite(supported_backend,16)}
    ...
    for number,(key,method,phase) in enumerate(p['call_schedule']):
        ...
        old_supported_receipts=len(supported_backend.receipts)
        ...
        try:
            ...
            row['supported_FA_receipts']=supported_backend.receipts[old_supported_receipts:]
        ...
        assert len(row['supported_FA_receipts'])==(8 if method=='candidate' else 0)
        assert all(item['status']=='returned' for item in row['supported_FA_receipts'])
    ...
    assert supported_backend.calls_entered==supported_backend.calls_returned==16
    ...
    r['vectors_sha256']=sha((A/'vectors.npz').read_bytes());r['status']='all8FA_supported_secant_current_layer0_4DT100FLA84score_complete'
    ...
    r['cost_scope']='One resident full model; one observation per method/case in opposite order, pilot cost only. Both retain current layer0 norm symmetric and two finite FLA orientations. All default native forwards, 32-layer replay, endpoints/copies, finite operations, common diagnostics, existing supported-wrapper synchronization and21-row state allocation are timed. Signature adapter adds no tensor arithmetic or CPU row copy. Load, eager initialization and original scorers remain in job wall time; no repeat benchmark or new extension compilation.'
    if supported_backend is not None:r['finite_counts']['supported_FA_native_library']={'entered':supported_backend.calls_entered,'returned':supported_backend.calls_returned,'successful_phases':3*supported_backend.calls_returned,'phases_inside_nonreturned_calls':'unknown' if supported_backend.calls_entered!=supported_backend.calls_returned else 0,'receipts':supported_backend.receipts}
    save()
