"""Fixed NI0/MH0 stability and NI1 warm+ABBA cost;10DT245FLA84originalscores."""
...
def timeout(*args):raise TimeoutError('Frozen10DT245FLA84score stability-cost budget expired.')
def timed(kind,fn):
...
try:
    ...
    assert p['call_schedule']==[['niah_mq_q2_1','control','warm'],['niah_mq_q2_1','candidate','warm'],['niah_mq_q2_1','control','measured'],['niah_mq_q2_1','candidate','measured'],['niah_mq_q2_1','candidate','measured'],['niah_mq_q2_1','control','measured'],['morehopqa_0','control','quality'],['morehopqa_0','candidate','quality'],['niah_mq_q2_0','candidate','quality'],['niah_mq_q2_0','control','quality']]
    ...
    prior_raw=Path(p['prior_wholepilot']['results_path']).read_bytes()
    assert sha(prior_raw)==p['prior_wholepilot']['results_sha256'];prior=json.loads(prior_raw)
    ...
    fas={method:BudgetedFinite(original_fa,40) for method in ('control','candidate')}
    finite_fla=BudgetedFinite(make_compiled_finite_pullback(reuse_scalar_products=False),245)
    ...
    runners={method:Qwen35DenseFiniteRunner(model,backend,finite_fla,norm_gate_rules={0:'symmetric'},
        finite_fla_by_layer=({0:candidate_backend} if method=='candidate' else {})) for method,backend in fas.items()}
    assert runners['control'].finite_fla_by_layer=={}
    assert runners['candidate'].finite_fla_by_layer=={0:candidate_backend}
    ...
    for key,case in cases.items():
        r['cases'][key]={'input':case['input'],'gold':case['gold'],'mapping':case['mapping'],
            ...
            'role':'cost_only' if key=='niah_mq_q2_1' else 'fresh_original_quality'}
    for number,(key,method,phase) in enumerate(p['call_schedule']):
        ...
        gc.collect();torch.cuda.synchronize();row['GPU_allocated_before_attribute']=torch.cuda.memory_allocated();row['GPU_reserved_before_attribute']=torch.cuda.memory_reserved();assert r['DT_entered']<10;r['DT_entered']+=1;tick=time.perf_counter();details=None
        try:
    ...
    assert r['DT_entered']==r['DT_returned']==10
    cost=[x for x in r['runs'] if x['phase']=='measured'];assert [(x['method']) for x in cost]==['control','candidate','candidate','control']
    r['cost_summary']={}
    for method in ('control','candidate'):
        rows=[x for x in cost if x['method']==method]
        r['cost_summary'][method]={'runs':[x['number'] for x in rows],
            'outer_seconds':[x['outer_attribute_seconds'] for x in rows],
            'median_outer_seconds':float(np.median([x['outer_attribute_seconds'] for x in rows])),
            'runner_seconds':[x['details']['complete_attribution_seconds_with_diagnostics'] for x in rows],
            'peak_allocated':[x['details']['peak_allocated'] for x in rows],
            'peak_reserved':[x['details']['peak_reserved'] for x in rows],
            'scope':'Two measured observations on one fixed NI1 input,after C/S warmups;pilot ABBA comparison,not population speed estimate.'}
    r['warm_runs']=[{'number':x['number'],'method':x['method'],'seconds':x['outer_attribute_seconds'],'peak_allocated':x['details']['peak_allocated']} for x in r['runs'] if x['phase']=='warm']
    ...
    assert all(fas[method].entered==fas[method].returned==40 for method in fas)
    assert finite_fla.entered==finite_fla.returned==245
    assert candidate_backend.entered==candidate_backend.returned==5
    ...
    r['vectors_sha256']=sha((A/'vectors.npz').read_bytes());r['status']='layer0_FLA_endpoint_average_stability_cost_10DT245FLA84score_complete'
    ...
    r['cost_scope']='One full model remains resident. NI1 C/S warmups separate;measured C/S/S/C before quality/scoring. Same unchanged native FA/FLA/default upstream and normal-runner stage diagnostics;select_output_rows=True;all layer0 endpoint copies,two original backends,six FP32 averages and CPU checkpoint copies are included. B2 input/target setup outside attribute is recorded by resident/pre-attribute memory;runner peaks reset identically each call;no empty_cache. Full load,eager init,compilation,all warm calls,quality calls,original scorers and failures remain in total wall time. Two measured samples per method only;no universal latency or memory bound.'
    save()
