...
try:
    ...
    cfg=AutoConfig.from_pretrained(cp,local_files_only=True,trust_remote_code=False).text_config
    ...
    for shard in sorted(set(names.values())):
        with safe_open(cp/shard,framework='pt',device='cpu') as sf:
            for name in names:
                ...
                value=sf.get_tensor(name);state[name.removeprefix(prefix)]=value
    ...
    module.load_state_dict(state,strict=True,assign=True);module.eval().requires_grad_(False).to('cuda');del state
    ...
    r['status']='full_GDN_vectors_and_native_gradient_limit_recovered'
except Exception:
