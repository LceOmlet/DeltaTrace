 ...
 * minimum-Euclidean-norm zero-row-sum secant correction. Original QK midpoint
 * and raw P1 value branch retained. Real-arithmetic proof is in the protocol.
 * FP32 variance uses anchored tile centering and Welford tile merges, no epsilon.
 */
#include <cuda.h>
...
enum CleanRow {SAnchor,SMean,GAnchor,GMean,InvP1,Alpha,M2,Numerator,P0Sum,P1Sum,
               RawTarget,NormalizedTarget,EndpointPositive,EndpointNegative,BaseContraction,RowCount};
struct CleanSecantParams {
...
__global__ void deltatrace_fa_finite_p1_clean_secant_kernel(CleanSecantParams p) {
    ...
    auto running_mean=make_fragment_like(denominator),running_m2=make_fragment_like(denominator);
    auto running_count=make_fragment_like(denominator);
    clear(sum_p0);clear(sum_p1);clear(sum_g0);clear(sum_g1);clear(sum_s1);clear(sum_gs1);
    clear(s_anchor);clear(g_anchor);clear(running_mean);clear(running_m2);clear(running_count);
    ...
    for(int col0=begin;col0<end;col0+=N) {
        ...
        for(int i=0;i<size(a0);++i) {
            ...
            if(query<valid && key<valid && key<=query) {
                ...
                if constexpr(Phase==0) actual_p0=__builtin_exp2f(lp0*float(M_LOG2E));
                ...
                if constexpr(Phase!=0) {
                    ...
                    const float centered_s=(delta_score(i)-p.rows[SAnchor*row_stride+pos])-p.rows[SMean*row_stride+pos];
                    a0(i)=query==0 ? 0.f : actual_p1*p.rows[InvP1*row_stride+pos]*centered_g
                          +p.rows[Alpha*row_stride+pos]*centered_s;
                }
        ...
        if constexpr(Phase==0) {
            ...
            auto tile_sum=make_fragment_like(denominator);clear(tile_sum);
            #pragma unroll
            ...
            flash::thread_reduce_<false>(ss_rows,tile_sum,sum);flash::quadreduce_sum(tile_sum);
            #pragma unroll
            for(int mi=0;mi<size<0>(ds_rows);++mi) {
                const int query=row0+get<0>(rowcoords(mi));
                const float count=query<valid ? float(max(0,min(N,query-col0+1))) : 0.f;
                const float mean=count>0.f?tile_sum(mi)/count:0.f;
                #pragma unroll
                for(int ni=0;ni<size<1>(ds_rows);++ni) {
                    const int key=col0+get<1>(coord_rows(mi,ni));
                    const float centered=ss_rows(mi,ni)-mean;
                    sg_rows(mi,ni)=query<valid && key<valid && key<=query ? centered*centered : 0.f;
                }
            }
            auto tile_m2=make_fragment_like(denominator);clear(tile_m2);
            flash::thread_reduce_<false>(sg_rows,tile_m2,sum);flash::quadreduce_sum(tile_m2);
            #pragma unroll
            for(int mi=0;mi<size(running_mean);++mi) {
                const int query=row0+get<0>(rowcoords(mi));
                const float count=query<valid ? float(max(0,min(N,query-col0+1))) : 0.f;
                if(count>0.f) {
                    const float mean=tile_sum(mi)/count,total=running_count(mi)+count;
                    const float difference=mean-running_mean(mi);
                    running_m2(mi)+=tile_m2(mi)+difference*difference*(running_count(mi)*count/total);
                    running_mean(mi)+=difference*(count/total);
                    running_count(mi)=total;
                }
            }
        } else {
            if constexpr(Phase==1) {
                auto positive=make_fragment_like(a0),negative=make_fragment_like(a0);
                #pragma unroll
                for(int i=0;i<size(a0);++i) {
                    const int query=row0+get<0>(coords(i)),key=col0+get<1>(coords(i));
                    const int64_t pos=int64_t(bh)*length+query;
                    const float credit=query<valid && key<valid && key<=query ? a0(i)*
                        ((delta_score(i)-p.rows[SAnchor*row_stride+pos])-p.rows[SMean*row_stride+pos]) : 0.f;
                    positive(i)=fmaxf(credit,0.f);negative(i)=fminf(credit,0.f);
                }
                auto pos_rows=make_tensor(positive.data(),flash::convert_layout_acc_rowcol(positive.layout()));
                auto neg_rows=make_tensor(negative.data(),flash::convert_layout_acc_rowcol(negative.layout()));
                flash::SumOp<float> sum;
                flash::thread_reduce_<false>(pos_rows,denominator,sum);flash::thread_reduce_<false>(neg_rows,numerator,sum);
            }
    ...
    if constexpr(Phase==0) {
        ...
        if(get<1>(rowcoords(0))==0) {
            #pragma unroll
            for(int i=0;i<size(running_mean);++i) {
                ...
                if(r<length) {
                    ...
                    if(r>=valid) {for(int state=0;state<RowCount;++state)p.rows[state*row_stride+pos]=0.f;}
                        ...
                        const float inv1=1.f/sum_p1(i),gmean=sum_g1(i)*inv1;
                        ...
                        const float alpha=running_m2(i)>0.f && r>0 ? correction/running_m2(i):0.f;
                        p.rows[SAnchor*row_stride+pos]=s_anchor(i);p.rows[SMean*row_stride+pos]=running_mean(i);
                        p.rows[GAnchor*row_stride+pos]=g_anchor(i);p.rows[GMean*row_stride+pos]=gmean;
                        p.rows[InvP1*row_stride+pos]=inv1;p.rows[Alpha*row_stride+pos]=alpha;
                        p.rows[M2*row_stride+pos]=running_m2(i);p.rows[Numerator*row_stride+pos]=correction;
...
extern "C" int deltatrace_fa_finite_p1_clean_secant(
    ...
    CleanSecantParams p{(const E*)q0,(const E*)k0,(const E*)q1,(const E*)k1,(const E*)v0,
    ...
    deltatrace_fa_finite_p1_clean_secant_kernel<0><<<grid,FiniteTraits::kNThreads,shared,stream>>>(p);
    auto error=cudaGetLastError();if(error!=cudaSuccess)return int(error);
    deltatrace_fa_finite_p1_clean_secant_kernel<1><<<grid,FiniteTraits::kNThreads,shared,stream>>>(p);
    error=cudaGetLastError();if(error!=cudaSuccess)return int(error);
    deltatrace_fa_finite_p1_clean_secant_kernel<2><<<grid,FiniteTraits::kNThreads,shared,stream>>>(p);
