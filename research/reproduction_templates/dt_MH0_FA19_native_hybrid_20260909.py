...
Adapt only current MH0 source schema, actual captured native arguments and
...
try:
    ...
    assert p['frozen_input_receipts'] == {step:boundary['cases']['morehopqa_0']['points'][step]['input_receipt'] for step in ('0',*p['fixed_steps'])}
    for point in source['points'].values():
        assert point['native_dense_arguments'] == {k:p['native_FA_kwargs'][k] for k in ('dropout_p','softmax_scale','causal','return_attn_probs')}
    r['actual_native_argument_source'] = {'results_sha256':p['source_results_sha256'], 'native_dense_arguments':source['points']['0']['native_dense_arguments'],
        'scope':'Actual native FA arguments captured on all five current MH0 layer19 replays; config/AST checks below corroborate the captured scale.'}
    ...
    r['scaling_source'] = {'model_source_path': str(model_source_path), 'model_sha256': p['native_model_sha256'],
        ...
        'scope': 'Native scale is directly retained in the current source capture; pinned original config/AST independently corroborate it.'}
    ...
    for step in p['fixed_steps']:
        ...
        frozen = source['conditional_ledgers'][step]['replayed_9term_ledger']
    ...
    del private, layer, coeff, points, actual_ledger
    gc.collect()
    ...
    for item in p['successful_template_provenance']['files']:
        assert digest(Path(item['remote_path'])) == item['sha256']
    r['status'] = 'MH0_eleven_native_FA_hybrid_contrasts_complete'
except Exception:
    r['status'] = 'failed'
