"""Freeze V2 H2D layout-only repair; preserve V1 zero-call failure evidence."""
import ast
import base64
import hashlib
import json
...
files = {'study.py': (R / 'research/reproduction_templates/dt_fa19_native_hybrid_20260908_v2.py').read_bytes(),
...
directory = '${ARTIFACT_ROOT}/codex_dt_fa19_native_hybrid_20260908_v2'
...
failed_dir = A / 'snapshot${ARTIFACT_ROOT}/codex_dt_fa19_native_hybrid_20260908_v1'
failed_raw = (failed_dir / 'results.json').read_bytes()
failed = json.loads(failed_raw)
failed_receipt = json.loads((failed_dir / 'terminal_receipt.json').read_bytes())
assert failed['status'] == 'failed'
assert failed['native_FA_calls_entered'] == failed['native_FA_calls_returned'] == 0
assert 'x.dtype == torch.bfloat16 and x.is_cuda and x.is_contiguous()' in failed['error']
assert sha(failed_raw) == failed_receipt['files']['results.json']['sha256']
assert (failed_dir / 'study.py').read_bytes() == (R / 'research/reproduction_templates/dt_fa19_native_hybrid_20260908.py').read_bytes()
metadata_path = A / 'snapshot${ARTIFACT_ROOT}/codex_dt_fa19_h2d_metadata_20260908.json'
metadata = json.loads(metadata_path.read_bytes())
assert set(metadata) == {'query', 'key', 'value'}
assert all(item['BF16'] and item['is_cuda'] and not item['contiguous'] for item in metadata.values())
failure_files = [{'remote_path': '${ARTIFACT_ROOT}/codex_dt_fa19_native_hybrid_20260908_v1/' + name,
                  'sha256': sha((failed_dir / name).read_bytes())}
                 for name in ('results.json', 'protocol.json', 'study.py')]
failure_files.append({'remote_path': '/tmp/' + metadata_path.name,
                      'sha256': sha(metadata_path.read_bytes())})
protocol = {
    ...
    'version': 2,
    'v1_failure_provenance': {'files': failure_files,
        'status': failed['status'], 'native_FA_calls_entered': 0, 'native_FA_calls_returned': 0,
        'job_seconds': failed['job_seconds'],
        'cause': 'Recorded MACA H2D copy preserved BF16 and CUDA device but produced noncontiguous query/key/value strides after CPU contiguous.',
        'actual_H2D_metadata': metadata,
        'only_compute_change': 'Replace CPU transpose.contiguous().to(cuda) with transpose.to(cuda).contiguous() for four actual Q/K/V sets and paired EOS V0, using standard torch layout materialization. Preserve original dtype/device/layout guards, native FA backend, precision, coefficients, formulas and 11-call budget.'},
...
protocol_path = A / 'dt_fa19_native_hybrid_protocol_20260908_v2.json'
launch_path = A / 'launch_dt_fa19_native_hybrid_20260908_v2.json'
