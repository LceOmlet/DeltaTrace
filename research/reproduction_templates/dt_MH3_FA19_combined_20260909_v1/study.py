...
class BoundaryObserver:
    def __init__(self):
        self.coeff={};self.paired={};self.endpoint_effect={};self.activations={};self.points={};self.counts={};self.logp={};self.current=None;self.fa19={'B1':{},'B1_native_arguments':{}}
    def wants_decoder(self,index):return index==19
...
try:
    ...
    try:
        for step in p['capture_steps']:
            ...
            with NativeDecoderCapture(layers[19]) as dc,ScopedNativeDenseCapture(
    ...
    timed('FA19_existing_native_operator_diagnostic',lambda:analyze(observer.fa19,p['frozen_capture_receipts'],r,vectors))
    ...
    path=A/'MH3_FA19_combined_private.pt';timed('save_actual_private_boundary_tensors',lambda:torch.save(private,path))
    ...
    r['vectors_sha256']=sha((A/'vectors.npz').read_bytes());r['status']='MH3_FA19_combined_1DT4score11FA_complete'
    ...
    r['cost_scope']='One current DT with passive34boundary coefficient copies and focused FA19 native states;four original B1 scorer calls on predeclared standalone masks0/3/10/20. Existing1model-load/1NI0eager initialization,32native decoder replays,8FAfinite/8FAaux,25FLAbackends/50nativeadjoints. Boundary CPU double contractions,FP32diagnostics of the same actual B1 logits and private tensor save cost are all measured. Eleven public FA calls add four native-operand replays and seven labeled hybrids inside the same process. No new MAS curve,FT,generation,newcandidate,extraGDN/conv or precision repair. All entered/returned and failed work retained;diagnostic cost is not production timing.'
    save()
