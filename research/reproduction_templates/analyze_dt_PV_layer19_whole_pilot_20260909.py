"""Independent CPU audit of the frozen layer19 PV pilot; no torch/model calls."""
...
def provenance(D,p,r,receipt,complete):
    ...
    frozen=(f'dt_PV_layer19_remaining_regression_protocol_20260909_s{segment}.json' if segment is not None else 'dt_PV_layer19_whole_pilot_protocol_20260909.json')
...
def run_audit(run,p,r,z,inputs):
    ...
    assert d['attention_pv_rules']==run['PV_rule_scope']==({'19':'content0'} if method=='candidate' else {})
    assert run['finite_FA_phase_accounting']=={'returned_calls':8,'phases_from_returned_calls':24,'library':p['finite_FA_library']}
...
def audit(D):
    ...
    segment=p.get('segment_index');expected_status=('layer19_PV_content0_remaining_segment_4DT100FLA84score_complete' if segment is not None else 'layer19_PV_content0_current_layer0_4DT100FLA84score_complete')
    complete=r['status']==expected_status
    if segment is not None:
        pairs=[['niah_mq_q2_0','morehopqa_2'],['niah_mq_q2_2','morehopqa_1'],['niah_mq_q2_3','morehopqa_3']]
        assert type(segment) is int and segment in range(3) and p['quality_cases']==pairs[segment]
    ...
    out={'status':'independent_PV19_whole_pilot_audit_passed' if complete else 'independent_PV19_whole_pilot_partial_failure_audit',
    ...
    for method in ('control','candidate'):assert r['finite_counts'][method]=={'entered':16,'returned':16}
    ...
    assert r['finite_counts']['original_FA_phases_from_returned_calls']==96
    ...
    out['counts']={'models':1,'eager_NI0_initializations':1,'DT':4,'native_replays':128,'finite_decoders':128,'finite_FA':32,
        'public_FA_LSE':32,'finite_FA_phases':96,'content0_calls':2,'content1_calls':30,'finite_FLA':100,'native_FLA_adjoint_stages':200,
...
if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('directory',nargs='?',default=str(A/'snapshot${ARTIFACT_ROOT}/codex_dt_PV_layer19_whole_pilot_20260909_v1'));ap.add_argument('--output',type=Path);args=ap.parse_args()
    result=audit(args.directory);segment=result['segment_index']
    target=args.output or A/(f'dt_PV_layer19_remaining_regression_summary_20260909_s{segment}.json' if segment is not None else 'dt_PV_layer19_whole_pilot_summary_20260909.json')
    target.write_text(json.dumps(result,indent=2,allow_nan=False))
    print(json.dumps({'output':str(target),'sha256':sha(target),'status':result['status'],'deltas':{k:v['candidate_minus_control'] for k,v in result['cases'].items()}}))
