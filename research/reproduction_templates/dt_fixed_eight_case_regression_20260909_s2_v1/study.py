"""One fixed two-case shard of eight-author-case regression;C/S DT,original FT0/FT3."""
...
def timeout(*args):raise TimeoutError('Frozen2cases4DT2Both168score regression shard budget expired.')
def timed(kind,fn):
...
def sources():
    found={}
    found['regression_manifest.json']=sha((A/'regression_manifest.json').read_bytes());assert found['regression_manifest.json']==p['regression_manifest_sha256']
...
try:
    ...
    assert p['segment_index'] in range(4)
    assert p['case_indices']==[['niah_mq_q2',p['segment_index']],['morehopqa',p['segment_index']]]
    assert len(p['call_schedule'])==4 and len(p['quality_schedule'])==8
    assert set(method for _,method in p['quality_schedule'])=={'DT_control','DT_candidate','FT0','FT3'}
    ...
    def prepare(dataset,index):
        ...
        assert {name:info[name] for name in ('prompt_length','target_length','total_length')}==ref['expected_lengths']
    ...
    fas={method:BudgetedFinite(original_fa,16) for method in ('DT_control','DT_candidate')}
    finite_fla=BudgetedFinite(make_compiled_finite_pullback(reuse_scalar_products=False),98)
    ...
    runners={method:Qwen35DenseFiniteRunner(model,backend,finite_fla,norm_gate_rules={0:'symmetric'},
        finite_fla_by_layer=({0:candidate_backend} if method=='DT_candidate' else {})) for method,backend in fas.items()}
    assert runners['DT_candidate'].finite_fla_by_layer=={0:candidate_backend}
    assert runners['DT_control'].finite_fla_by_layer=={}
    ...
    for key,case in cases.items():
        r['cases'][key]={'input':case['input'],'gold':case['gold'],'mapping':case['mapping'],
            ...
            'role':'fixed_eight_case_regression'}
    ...
    def run_ft(key):
        ...
        assert sys.getprofile() is None and r['FT_entered']<2;r['FT_entered']+=1;tick=time.perf_counter();sys.setprofile(observe_ft)
        try:
    ...
    for key in p['quality_cases']:run_ft(key)
    assert r['FT_entered']==r['FT_calls']==2
    ...
    assert r['scorer_entered']==r['scorer_returned']==168
    for key in p['quality_cases']:
        row=r['cases'][key];curves=row['curves'];names=['DT_control','DT_candidate','FT0','FT3']
        row['actual_endpoint_scores']={name:[curves[name]['scores'][i] for i in (0,20)] for name in names}
        row['clean_and_allEOS_equal_all_methods']=[all(curves[name]['scores'][i]==curves[names[0]]['scores'][i] for name in names) for i in (0,20)]
        row['candidate_minus_comparators']={name:{'return_metrics':[a-b for a,b in zip(curves['DT_candidate']['return_metrics'],curves[name]['return_metrics'])],
            'needle':curves['DT_candidate']['needle']-curves[name]['needle'] if curves[name]['needle'] is not None else None} for name in ['DT_control','FT0','FT3']}
        row['scope']='Fixed C/S and predeclared original FT0/FT3;report every case including regressions. All same actual raw input,response,eligible set and common originalFA evaluator. Preserve internal FT representation versus DT response-logprob target differences.'
    ...
    assert finite_fla.entered==finite_fla.returned==98
    ...
    r['vectors_sha256']=sha((A/'vectors.npz').read_bytes());r['status']='fixed_eight_case_regression_shard_2cases4DT2Both168score_complete'
    ...
    r['cost_scope']='Fixed two-case shard of predeclared NI0-3/MH0-3 regression. One resident model,4normal DT(2control+2candidate),2unchanged Both traces(hops3),168fresh originalFA scores. Every endpoint copy,extra finiteFLA,originalFT identity-valued probe,load/init/compile and failed work remains counted. Timings include different normal/passive diagnostics and are descriptive,not warmed FT0-only speed claims. No historical score substitution,method/layer tuning,negative clipping,sample replacement or stopping based on metric improvement.'
    save()
    with zipfile.ZipFile(A/'review_bundle.zip','w',zipfile.ZIP_DEFLATED) as archive:
        for name in [*p['files_sha256'],'regression_manifest.json','protocol.json','results.json','vectors.npz']:
