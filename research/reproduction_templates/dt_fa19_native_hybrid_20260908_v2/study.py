...
V2 changes only H2D layout preparation: use standard torch contiguous after
transfer, because the recorded MACA H2D copy changed strides in failed V1.
"""
import ast
import gc
import hashlib
...
try:
    ...
    assert p['budget']['native_FA_calls'] == 11 and p['fixed_steps'] == ['1', '10', '20']
    for name, want in p['files_sha256'].items():
        assert digest(A / name) == want, name
    for item in p['v1_failure_provenance']['files']:
    ...
    r['scaling_source'] = {'model_source_path': str(model_source_path), 'model_sha256': p['native_model_sha256'],
        ...
        'scope': 'Pinned native source and original config verified; previous capture did not retain the scalar call argument separately.'}
    ...
    layer = private['layers']['19']
    coeff = layer['coeff']
    paired_value0 = layer['paired']['c']['value'][0:1].clone()
    points = {step: {'d': {k: private['scoring'][step]['19']['d'][k] for k in ledger.D_FIELDS},
                     'c': {k: private['scoring'][step]['19']['c'][k] for k in ledger.C_FIELDS}}
    ...
    for item in p['v1_failure_provenance']['files']:
        assert digest(Path(item['remote_path'])) == item['sha256']
    r['status'] = 'eleven_native_FA_hybrid_contrasts_complete'
except Exception:
    r['status'] = 'failed'
