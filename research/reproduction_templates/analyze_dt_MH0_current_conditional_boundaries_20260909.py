"""Independent NumPy audit of one MH0 DT/four native-score boundary diagnostic."""
...
assert p==r['protocol']==read(A/'dt_MH0_current_conditional_boundaries_protocol_20260909.json')
assert sha(D/'protocol.json')=='b12fe8409accdba58488a61b86421c0abd6680444d5d59bcfb0b9bce27f574c9'
assert p['files_sha256']['study.py']=='05c05a5805d5616feb0658516ff93ca475f77c138baac33870966a7278fdc240'
assert r['status']=='MH0_current_conditional_boundaries_1DT4score_complete'
...
assert p['capture_steps']==STEPS and p['call_schedule']==[['morehopqa_0','DT']]
...
prior=read(S/'results.json');source_case=prior['cases']['morehopqa_0'];curve=source_case['curves']['DT']
sv=np.load(S/'vectors.npz',allow_pickle=False);v=np.load(D/'vectors.npz',allow_pickle=False)
case=r['cases']['morehopqa_0'];info=case['input'];T,P=info['total_length'],info['prompt_length'];keep=set(info['keep'])
assert info==source_case['input'] and T==853 and P==404
...
ids=np.asarray(r['input_freeze_before_model_load']['morehopqa_0']['input_ids'],dtype=np.int64);eos=int(ids[-1])
...
w=v['morehopqa_0_DT_evaluated'];full=v['morehopqa_0_DT_full'];old=sv['morehopqa_0_DT_evaluated']
...
vector_drift=drift(full,sv['morehopqa_0_DT_full'])
for key,value in vector_drift.items():close(value,run['source_vector_drift_report_only'][key],key,1e-12)
close(run['source_B2_root_effect'],prior['runs'][0]['details']['root_effect'])
...
expected_keys={'morehopqa_0_DT_evaluated','morehopqa_0_DT_full'}
for step in [3,10,20]:
...
out={'status':'MH0_current_conditional_boundaries_independent_CPU_audit_passed',
...
assert private['file']=='MH0_actual_boundary_private.pt' and len(private['sha256'])==64
...
out['next']={'status':'internal_operator_measurement_needed_after_root_review',
    'predeclared_selection':'Largest negative mid-step decoder jump on the fixed standalone mask; early and allEOS retained as controls.',
...
path=A/'dt_MH0_current_conditional_boundaries_summary_20260909.json';path.write_text(json.dumps(out,indent=2,allow_nan=False))
