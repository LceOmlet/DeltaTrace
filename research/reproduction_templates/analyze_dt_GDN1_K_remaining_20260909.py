"""Independent CPU audit of the frozen GDN1 input-supported K pilot; no torch/model calls."""
...
def provenance(D,p,r,receipt,complete):
    ...
    frozen=f'dt_GDN1_K_remaining_protocol_20260909_s{segment}.json'
...
def run_audit(run,p,r,z,inputs):
    ...
    assert counts['finite_FA_backend_calls']==8
    ...
    assert d['attention_pv_rules']==run['PV_rule_scope']=={}
    assert d['key_norm_by_layer']==([1] if method=='candidate' else [])
    assert run['finite_FA_phase_accounting']=={'callback_sites':8,'returned_calls':8,'phases_from_returned_calls':3*(8),'library':p['finite_FA_library']}
    ...
    return {'case':key,'method':method,'rule':d['key_norm_by_layer'],'needle':nt,'outer_seconds':run['outer_attribute_seconds'],
...
def audit(D):
    ...
    segment=p.get('segment_index');expected_status=('GDN1_K_input_supported_4DT100FLA84score_complete' if segment is not None else 'layer19_PV_content0_current_layer0_4DT100FLA84score_complete')
    complete=r['status']==expected_status
    if segment is not None:
        pairs=[['niah_mq_q2_2','morehopqa_1'],['niah_mq_q2_1','morehopqa_0'],['niah_mq_q2_3','morehopqa_3']]
        assert type(segment) is int and 0<=segment<3 and p['quality_cases']==pairs[segment]
    ...
    out={'status':'independent_GDN1_K_whole_pilot_audit_passed' if complete else 'independent_PV19_whole_pilot_partial_failure_audit',
    ...
    assert len(r['key_norm_calls'])==2 and all(x['status']=='returned' for x in r['key_norm_calls'])
    ...
    out['counts']={'models':1,'eager_NI0_initializations':1,'DT':4,'native_replays':128,'finite_decoders':128,'finite_FA':32,
        'public_FA_LSE':32,'finite_FA_phases':96,'content0_calls':0,'content1_calls':32,'finite_FLA':100,'native_FLA_adjoint_stages':200,
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('segment',type=int,choices=[0,1,2]);segment=parser.parse_args().segment
    result=audit(A/f'snapshot${ARTIFACT_ROOT}/codex_dt_GDN1_K_remaining_20260909_s{segment}_v1')
    target=A/f'dt_GDN1_K_remaining_summary_20260909_s{segment}.json';target.write_text(json.dumps(result,indent=2,allow_nan=False))
