"""Freeze the existing eleven-call native FA hybrid diagnostic on actual MH2 operands."""
import ast,base64,copy,hashlib,json,shlex,zlib
from pathlib import Path, PurePosixPath
A=Path(__file__).resolve().parent;R=A.parent/'DeltaTrace';sha=lambda b:hashlib.sha256(b).hexdigest()
S=A/'snapshot${ARTIFACT_ROOT}/codex_dt_MH2_two_decoder_internal_20260909_v1';sp=json.loads((S/'protocol.json').read_bytes());sr=json.loads((S/'results.json').read_bytes())
assert sr['status']=='MH2_FA19_GDN1_1native10replay2finite_internal_complete' and sr['protocol']==sp
...
summary=A/'dt_MH2_two_decoder_internal_summary_20260909.json';su=json.loads(summary.read_bytes());assert su['status']=='MH2_two_decoder_internal_independent_CPU_audit_passed'
...
assert br['cases']['morehopqa_2']['input']==sr['input']
...
sraw=(R/'research/reproduction_templates/dt_MH2_FA19_native_hybrid_20260909.py').read_bytes()
...
assert artifact['sha256']=='598445e3869ccf14a5e18ae0d32aa4d0ea539d7d26c475e8d7ee4669bd8d1c7d' and artifact['bytes']==2626255118
...
kwargs=copy.deepcopy(vp['native_FA_kwargs'])
site=PurePosixPath(sp['isolated_site']);argument_sources=[]
for relative in ['transformers/integrations/flash_attention.py','transformers/modeling_flash_attention_utils.py']:
    local=A/'snapshot'/str(site/relative).lstrip('/')
    argument_sources.append({'path':str(site/relative),'sha256':sha(local.read_bytes())})
argument_sources.extend([{'path':'/tmp/'+S.name+'/'+name,'sha256':sp['files_sha256'][name]} for name in ['study.py','native_dense_attention_capture.py']])
p=copy.deepcopy(vp);p.pop('v1_failure_provenance',None)
remote='${ARTIFACT_ROOT}/codex_dt_MH2_FA19_native_hybrid_20260909_v1';python='${ARTIFACT_ROOT}/codex_qwen35_isolated_import_20260908_v1/env/bin/python'
p.update({
    'scope':'Fixed current MH2 decoder19 native FA hybrid contrasts on actual saved replayed coordinates and coefficients; source transfer to original boundary was audited. No model, DT, scorer, FT, backward, probability matrix or new candidate.',
    'version':'MH2_source_adaptation_of_successful_v2',
    ...
    'source_internal_audit':{'file':summary.name,'sha256':sha(summary.read_bytes()),'scope':'Nine-term scalar/group closure plus original boundary and three transfer vectors passed; FA19 regenerated m19 and native outputs match saved current boundary, all transfer terms zero. This does not pre-establish the hybrid decomposition result.'},
    'input_sha256':sr['input']['input_sha256'],'case':'morehopqa_2','decoder_index':19,'fixed_steps':['3','10','20'],
    'frozen_input_receipts':{step:br['cases']['morehopqa_2']['points'][step]['input_receipt'] for step in ['0','3','10','20']},
    'original_core_errors_including_seed_cast':{step:sr['layers']['19']['ledgers'][step]['internal']['terms']['finite_FA_core_including_seed_cast'] for step in ['3','10','20']},
    ...
    'checkpoint_config_path':sp['checkpoint']+'/config.json','checkpoint_config_sha256':sp['checkpoint_config_tokenizer_sha256']['config.json'],
     'scaling_verification':'Pinned official Attention sets head_dim**-.5 and eval dropout0; HF forwards scaling to softmax_scale. Source observer asserted windows(-1,-1),softcap0,alibiNone in all native dense calls; B2 study asserted dropout0/causalTrue. Actual source dense scalar metadata was not retained. Verify saved native kwargs contain no overrides and measure four public FA output replay drifts; no extra model required.',
    'argument_provenance':{'sources':argument_sources,'scope':'Read-only pinned official source/config plus successful executed observer assertions and saved actual native decoder kwargs. No claim of directly persisted dense_arguments; no invented position/mask/rotary or native forward replacement.'},
    ...
    'artifact_contract':'Current actual private2.626GB capture stays remote and is hash-verified before/after. No weights/model load or raw activations in public payload/review ZIP; only signed scalar/token contrasts and provenance are exported.',
...
files['protocol.json']=json.dumps(p,indent=2).encode();pp=A/'dt_MH2_FA19_native_hybrid_protocol_20260909.json';lp=A/'launch_dt_MH2_FA19_native_hybrid_20260909.json'
