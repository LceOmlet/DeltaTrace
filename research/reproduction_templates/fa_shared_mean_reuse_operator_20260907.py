...
extra_tile=VendorFAFiniteP1SharedMean(p['extra_tile_library'],p['extra_tile_library_sha256'])
new=VendorFAFiniteP1SharedMeanReuse(A/'libdeltatrace_fa_finite_shared_mean_reuse.so',b['library']['sha256'])
methods={'old':old,'extra_tile':extra_tile,'reuse_shared':new}
...
try:
    for repeat in range(3):
        outputs={}
        names=['old','extra_tile','reuse_shared'];names=names[repeat:]+names[:repeat]
        for name in names:
            assert len(r['calls'])<9
            gc.collect();torch.cuda.empty_cache();torch.cuda.synchronize()
            base=torch.cuda.memory_allocated();torch.cuda.reset_peak_memory_stats()
            activity={};tick=time.perf_counter()
            outputs[name]=methods[name](compact,record['scale'],activity)
        ...
        for key in outputs['old']:
            a,c=outputs['old'][key],outputs['reuse_shared'][key]
        ...
        del outputs
    r['status']='shared_mean_reuse_operator_exact_no_end_to_end_claim'
except Exception:
