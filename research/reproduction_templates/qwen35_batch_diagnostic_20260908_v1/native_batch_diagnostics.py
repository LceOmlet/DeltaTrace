...
class NativeBatchDiagnostics:
    ...
    def capture_fla(self, local, output, call_index):
        ...
        assert local['cu_seqlens'] is None and not local['output_final_state']
        assert local['scale'] == local['k'].shape[-1] ** -0.5
    ...
    def capture_fa(self, local, output, call_index):
        ...
        assert local.get('attn_mask') is None and local.get('s_aux') is None
        assert not local.get('return_max_logit', False)
        assert all(local[k].dtype == torch.bfloat16 for k in ['q','k','v'])
        ...
        self.persist('FA_dense_actual_call0', {k:self.numpy(v) for k,v in self.fa.items() if k!='softmax_scale'},
                     {'dtype':'torch.bfloat16', 'softmax_scale':self.fa['softmax_scale'],
                      'original_strides':{k:list(local[k].stride()) for k in ['q','k','v']},
                      'replay_layout':'CPU capture and GPU replay use contiguous tensors; values and dtype preserved.'})
