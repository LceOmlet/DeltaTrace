...
try:
    ...
    for row in spans['cases']:
        ...
        r['target_selection'].append({'dataset':dataset,'index':j,'author_sink_closed_reference_only':[lo,hi],'scope':'whole_fixed_response_including_EOS','prompt_length':case['prompt_length'],
            'offsets':selected,'target_ids_sha256':sha(case['target_ids'].numpy().tobytes()),'count':len(selected)})
    selection=PackedAnswerTargets(cases,offsets,T,'cuda');assert selection.counts==[248,141]
    ...
    y,z=timed('original_final_norm_and_packed_whole_target_head',native_head_norm)
    r['final_norm_replay_vs_root']=metric(root['final_norm_output'],y)
    archive('actual_target_operands',{'norm_input':x.detach().cpu(),'norm_output':y.detach().cpu(),'packed_logits':z.detach().cpu(),
    ...
    r['original_root_target_logprobs']=original_saved.cpu().tolist();r['replayed_target_logprobs']=logp.detach().cpu().tolist()
    ...
    r['original_root_target_delta']=root_delta.tolist();r['head_replay_delta']=replay_delta.detach().tolist()
    z=z.detach();del y,x,logp
    ...
    persist('target_seed_review',{'finite':m,'input0':xnorm[0::2],'input1':xnorm[1::2],
    ...
    # The new score is persisted above before evaluating the unchanged gold.
