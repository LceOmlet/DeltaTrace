...
if not (A/'source.zip').exists():
    parent=Path('${ARTIFACT_ROOT}/codex_vendor_fa_source_probe_20260907_v1/source.zip')
    assert sha(parent)==p['archive_sha256']
    (A/'source.zip').write_bytes(parent.read_bytes())
...
try:
    ...
    # Keep the original .cu suffix so cucc dispatches GPU compilation; source bytes unchanged.
    staged=A/'flash_bwd_hdim128_fp16_sm80.cu';staged.write_bytes(source.read_bytes());assert sha(staged)==sha(source)
    ...
    with zipfile.ZipFile(A/'review_bundle.zip','w',zipfile.ZIP_DEFLATED) as z:
        for name in ['study.py','protocol.json','source_manifest.json','results.json','compile.log','flash_bwd_hdim128_fp16_sm80.cu']:
