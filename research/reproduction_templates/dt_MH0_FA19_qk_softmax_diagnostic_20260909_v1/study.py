...
isolated, source-traceable finite diagnostic. The existing compiled library is reused without compilation. Actual hybrid R0
is frozen from the completed current MH0 eleven-call operator study. No probabilities are materialized.
"""
import gc
import hashlib
import json
...
r = {'status': 'starting', 'protocol': p, 'native_FA_entered': 0,
     ...
     'FT_calls': 0, 'backward_calls': 0, 'generation_calls': 0, 'compiler_attempts': 0, 'points': {}}
torch = op = None
...
try:
    ...
    reuse = p['reused_build']
    build_path = Path(reuse['build_results_path'])
    assert digest(build_path) == reuse['build_results_sha256']
    build = json.loads(build_path.read_bytes())
    assert build['status'] == 'finite_extension_compiled_not_executed'
    assert digest(reuse['protocol_path']) == reuse['protocol_sha256']
    assert build['protocol'] == json.loads(Path(reuse['protocol_path']).read_bytes())
    library = Path(reuse['library_path'])
    assert digest(library) == build['library']['sha256'] == reuse['library_sha256']
    assert library.stat().st_size == build['library']['bytes'] == reuse['library_bytes']
    r['build_results_sha256'] = digest(build_path)
    r['diagnostic_library_sha256'] = digest(library)
    r['reused_build'] = reuse
    r['compiler_attempts'] = 0
    ...
    assert hybrid['status'] == 'MH0_eleven_native_FA_hybrid_contrasts_complete'

    assert source['input']['input_sha256'] == p['input_sha256']
    assert hybrid['protocol']['source_results_sha256'] == digest(p['source_results_path'])
    assert source['input']['total_length'] == p['total_length'] == 853
    assert p['frozen_input_receipts'] == {step:hybrid['points'][step]['input_receipt'] for step in p['fixed_steps']}
    for point in source['points'].values():
        expected = {k:p['native_FA_kwargs'][k] for k in ('dropout_p','softmax_scale','causal','return_attn_probs')}
        expected['return_attn_probs'] = False
        assert point['native_dense_arguments'] == expected
    ...
    paired = {name: private['native']['B2']['c'][name]
              for name in ('query', 'key', 'value', 'attention_output')}
    actual = {step: {name: private['native'][step]['c'][name]
    ...
    for step in p['fixed_steps']:
        ...
        deleted = set(hybrid['points'][step]['input_receipt']['deleted_positions'])
        assert deleted <= keep
        ...
        row = {'input_receipt': hybrid['points'][step]['input_receipt'],
    ...
    for item in p['protected_sources']:
        assert digest(item['path']) == item['sha256'], item['path']
    r['status'] = 'MH0_three_FA19_conditional_score_contractions_complete'
except Exception:
    r['status'] = 'failed'
    ...
    r['diagnostic_phase_launches_from_returned'] = 3 * r['diagnostic_finite_returned']
    r['phases_inside_nonreturned_diagnostic'] = 'unknown' if r['diagnostic_finite_entered'] != r['diagnostic_finite_returned'] else 0
    if torch is not None and torch.cuda.is_initialized():
        r['peak_allocated_bytes'] = torch.cuda.max_memory_allocated()
        r['peak_reserved_bytes'] = torch.cuda.max_memory_reserved()
    r['scope'] = 'Existing library reused with zero compilation. Saved actual single-layer operator coordinates; no new method, full-model counterfactual, RISE or MAS. Three-term semantic attribution is not inferred from algebraic closure alone.'
    ...
    with zipfile.ZipFile(A / 'review_bundle.zip', 'w', zipfile.ZIP_DEFLATED) as bundle:
        for name in [*p['files_sha256'], 'protocol.json', 'results.json', 'conditional_signed_rows.npz']:
            if (A / name).is_file():
