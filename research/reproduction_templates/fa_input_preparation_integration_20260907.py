...
from vendor_fa_finite_compiled_prepare_runtime import VendorFAFiniteP1CompiledPreparation
compact_extension=VendorFAFiniteP1CompiledPreparation(p['compact_library'],p['compact_library_sha256'])
...
try:
    ...
    for group_index,selection in enumerate(p['integration_groups']):
        ...
        for repeat in range(3):
            pair={}
            for mode in (['old','fused_prepare'] if repeat in (0,2) else ['fused_prepare','old']):
                assert report['fresh_attributions']<12
                ...
                if group_index==1 and repeat==0 and mode=='fused_prepare':
                    with torch.profiler.profile(activities=list(torch.profiler.supported_activities())) as prof:
                        value=invoke()
                    trace=HERE/'fused_prepare_B4_profile.json';prof.export_chrome_trace(str(trace))
                    report['fused_prepare_B4_profile']={'file':trace.name,'sha256':hashlib.sha256(trace.read_bytes()).hexdigest()}
                    del prof
                else:value=invoke()
                report['fresh_attributions']+=1;pair[mode]=value
                group['runs'].append({'mode':mode,'repeat':repeat,'warm':repeat==0,'result':value})
                save();print('FA_INPUT_PREPARATION',len(rows),repeat,mode,value['seconds'],value['peak_allocated_bytes'],flush=True)
            a=torch.tensor(pair['old']['signed_full_sequence'],dtype=torch.float64)
            b=torch.tensor(pair['fused_prepare']['signed_full_sequence'],dtype=torch.float64)
            check={'repeat':repeat,'full_signed_exact':bool(torch.equal(a,b)),
                'max_abs_difference':float((a-b).abs().max()),'sign_flips':int(((a*b)<0).sum()),
                'endpoints_exact':pair['old']['endpoint_scores32']==pair['fused_prepare']['endpoint_scores32']}
            group['comparisons'].append(check);save()
            assert check['full_signed_exact'] and check['endpoints_exact']
    assert report['fresh_attributions']==report['native_root_forwards']==12
    assert report['native_vjps']==report['evaluation_forwards']==report['ft_attribution_forwards']==0
    assert report['extra_layer_replay_calls']==report['extra_native_fa_attention_calls']==report['finite_FA_calls_enqueued']==432
    ...
    report['active']=None;report['status']='FA_input_preparation_B1_and_real_B4_exact'
except Exception:
