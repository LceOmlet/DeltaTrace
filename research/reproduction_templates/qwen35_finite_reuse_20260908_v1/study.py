...
try:
    ...
    def run(kind,phase,profile=False):
        ...
        try:
            ...
            with torch.no_grad():
                ...
                elif kind=='finite':out=baseline_pullback(endpoints,do,scale)
    ...
    r['status']='three_GEMM_reuse_screen_complete'
except Exception:
    ...
    with zipfile.ZipFile(HERE/'review_bundle.zip','w',zipfile.ZIP_DEFLATED) as z:
        for name in ['study.py','protocol.json','finite_fla_gpu.py','finite_fla_gpu_baseline.py','results.json']+[x.name for x in HERE.glob('*.npz')]:z.write(HERE/name,name)
