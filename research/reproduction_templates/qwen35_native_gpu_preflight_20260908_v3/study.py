"""Remaining three actual FA/FLA forwards after a preserved shared-memory failure."""
import os
...
try:
    ...
    assert sha((HERE / 'fla_maca_chunk_schedule.py').read_bytes()) == p['schedule_runtime_sha256']
    from fla_maca_chunk_schedule import apply_schedule
    r['scheduling_change'] = apply_schedule(p['isolated_environment'], Path(p['isolated_site']) / 'fla/ops/common/chunk_delta_h.py', HERE / 'scheduling_receipt.json')
    assert r['scheduling_change']['after_sha256'] == p['fla_scheduled_chunk_sha256']
    r['sources_during'] = source_receipt(scheduled=True)
    ...
    for call_index, selected in enumerate(p['call_selections']):
        assert r['root_forward_attempts'] < p['maximum_root_forward_attempts']
        ...
        def python_event(frame, event, arg):
            ...
            if event == 'return' and code_names.get(frame.f_code) == 'FLA_chunk' and not captured_prefix and arg is not None:
                assert frame.f_locals['initial_state'] is None and frame.f_locals['use_qk_l2norm_in_kernel']
                for key in ['q', 'k', 'v', 'g', 'beta']:
                    captured_prefix[key] = frame.f_locals[key][:, :64].detach().cpu().contiguous()
                captured_prefix['output'] = arg[0][:, :64].detach().cpu().contiguous()
    ...
    # Explicit CPU reference diagnostic on the first actual layer's causal
    # prefix. This is not used in any model forward or attribution.
    assert captured_prefix
    reference_tick = time.perf_counter()
    with torch.no_grad():
        ref, _ = native.torch_recurrent_gated_delta_rule(
            captured_prefix['q'], captured_prefix['k'], captured_prefix['v'],
            captured_prefix['g'], captured_prefix['beta'], initial_state=None,
            output_final_state=False, use_qk_l2norm_in_kernel=True)
    out = captured_prefix['output'].float(); ref32 = ref.float()
    relative = float(torch.linalg.vector_norm(out-ref32) / torch.linalg.vector_norm(ref32))
    r['CPU_reference'] = {'scope': 'First actual NI0 layer0, first64 tokens; official Transformers reference, not a replacement model path.',
        'calls': 1, 'device': 'cpu', 'relative_L2': relative, 'max_abs_error': float((out-ref32).abs().max()),
        'seconds': time.perf_counter()-reference_tick}
    import numpy as np
    np.savez_compressed(HERE / 'first_layer_prefix_reference.npz', **{k: v.float().numpy() for k, v in captured_prefix.items()}, reference=ref32.numpy())
    r['CPU_reference']['arrays_sha256'] = sha((HERE / 'first_layer_prefix_reference.npz').read_bytes())
    assert relative <= p['prefix_relative_L2_limit']
    r['sources_after'] = source_receipt(scheduled=True); assert r['sources_during'] == r['sources_after']
    ...
    r['status'] = 'three_native_FA_FLA_forwards_B2_and_CPU_prefix_reference_passed'
except Exception:
    ...
    with zipfile.ZipFile(HERE / 'review_bundle.zip', 'w', zipfile.ZIP_DEFLATED) as z:
        for name in ['study.py', 'protocol.json', 'official_fixed_text_inputs.py', 'fla_maca_chunk_schedule.py', 'results.json'] + [x.name for x in HERE.glob('*_profile.json')] + [x.name for x in HERE.glob('*.npz')] + [x.name for x in HERE.glob('*receipt.json')]:
