...
try:
    ...
    for group_index,selection in enumerate(p['integration_groups']):
        ...
        for repeat in range(3):
            pair={}
            for mode in (['old','shared_mean'] if repeat in (0,2) else ['shared_mean','old']):
                ...
                if group_index==1 and repeat==0 and mode=='shared_mean':
                    with torch.profiler.profile(activities=list(torch.profiler.supported_activities())) as prof:
                        value=invoke()
                    trace=HERE/'shared_mean_B4_profile.json';prof.export_chrome_trace(str(trace))
                    report['shared_mean_B4_profile']={'file':trace.name,'sha256':hashlib.sha256(trace.read_bytes()).hexdigest()}
                    del prof
                else:value=invoke()
                report['fresh_attributions']+=1;pair[mode]=value
                group['runs'].append({'mode':mode,'repeat':repeat,'warm':repeat==0,'result':value})
                save();print('FA_SHARED_MEAN',len(rows),repeat,mode,value['seconds'],value['peak_allocated_bytes'],flush=True)
            a=torch.tensor(pair['old']['signed_full_sequence'],dtype=torch.float64)
            b=torch.tensor(pair['shared_mean']['signed_full_sequence'],dtype=torch.float64)
            check={'repeat':repeat,'full_signed_exact':bool(torch.equal(a,b)),
                'max_abs_difference':float((a-b).abs().max()),'sign_flips':int(((a*b)<0).sum()),
                'endpoints_exact':pair['old']['endpoint_scores32']==pair['shared_mean']['endpoint_scores32']}
    ...
    report['active']=None;report['status']='FA_shared_mean_B1_and_real_B4_exact'
except Exception:
