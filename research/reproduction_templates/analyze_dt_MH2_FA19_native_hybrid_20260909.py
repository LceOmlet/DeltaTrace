"""Independent NumPy audit of the current MH2 eleven native FA contrasts."""
import hashlib,json,time,zipfile
from pathlib import Path
import numpy as np
A=Path(__file__).resolve().parent;D=A/'snapshot${ARTIFACT_ROOT}/codex_dt_MH2_FA19_native_hybrid_20260909_v1'
...
p,r=read(D/'protocol.json'),read(D/'results.json');assert p==r['protocol']==read(A/'dt_MH2_FA19_native_hybrid_protocol_20260909.json')
assert sha(D/'protocol.json')=='c40a15bc34ea386e9b5f140d9046e53f03cf6e578708e6c3cb1135f66852c2c8'
assert r['status']=='MH2_eleven_native_FA_hybrid_contrasts_complete'
...
assert source['input']==boundary['cases']['morehopqa_2']['input'];T,P=source['input']['total_length'],source['input']['prompt_length'];keep=set(source['input']['keep'])
assert T==710 and r['FA_layout']['padded_length']==T and r['FA_layout']['valid_lengths']==[T]
...
out={'status':'MH2_FA19_native_hybrid_independent_CPU_audit_passed','next':'Root review actual MH2 largest term before selecting next operation; no candidate or automatic dispatch.',
...
for step,row in r['points'].items():
    assert row['input_receipt']==p['frozen_input_receipts'][step]==boundary['cases']['morehopqa_2']['points'][step]['input_receipt']
    ...
    core=fields['core_error_at_stored_seed'];error=source['layers']['19']['ledgers'][step]['internal']['terms']['finite_FA_core_including_seed_cast']
...
out['interpretation']='Compare the measured three terms separately; no MH0 cause transferred without measurement. Signed group and token cancellation retained. Native operator hybrids are not full-model counterfactuals and imply no repair or MAS benefit.'
out['argument_provenance']=p['argument_provenance']
for item in p['argument_provenance']['sources']:assert sha(A/'snapshot'/item['path'].lstrip('/'))==item['sha256']
out['CPU_audit_seconds']=time.perf_counter()-started
path=A/'dt_MH2_FA19_native_hybrid_summary_20260909.json';path.write_text(json.dumps(out,indent=2,allow_nan=False))
