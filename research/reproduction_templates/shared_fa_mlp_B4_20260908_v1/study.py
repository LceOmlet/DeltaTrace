...
try:
    ...
    finite_capture=partial(compact_pair,finite_attention=compact_extension)
    propagate_batch=compact_batch
    for repeat in range(3):
        pair={}
        for mode in (['shared_mean','shared_mean_sac32'] if repeat in (0,2) else ['shared_mean_sac32','shared_mean']):
            assert report['fresh_attributions']<6
    ...
    assert report['fresh_attributions']==report['native_root_forwards']==6
    assert report['native_vjps']==report['evaluation_forwards']==report['ft_attribution_forwards']==0
    assert report['extra_layer_replay_calls']==report['extra_native_fa_attention_calls']==report['finite_FA_calls_enqueued']==216
    ...
    report['active']=None;report['status']='shared_FA_native_MLP_B4_six_call_screen_complete'
except Exception:
