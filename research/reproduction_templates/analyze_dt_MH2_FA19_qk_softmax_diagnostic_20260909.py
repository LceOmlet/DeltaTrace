"""Independent NumPy MH2 QK/softmax ledger and existing query-row concentration audit."""
import hashlib,json,time,zipfile
from pathlib import Path
import numpy as np
A=Path(__file__).resolve().parent;D=A/'snapshot${ARTIFACT_ROOT}/codex_dt_MH2_FA19_qk_softmax_diagnostic_20260909_v1'
...
assert p==r['protocol']==read(A/'dt_MH2_FA19_qk_softmax_diagnostic_protocol_20260909.json')
assert sha(D/'protocol.json')=='42e2274857bddb7038a3f4d61efbfa6f9cfe3bb7d4fcfbf1488002eb8c39b4ae'
assert r['status']=='MH2_three_FA19_conditional_score_contractions_complete'
...
assert h['status']=='MH2_eleven_native_FA_hybrid_contrasts_complete' and source['status']=='MH2_FA19_GDN1_1native10replay2finite_internal_complete'
assert r['input']==source['input'] and r['input']['total_length']==710 and p['fixed_steps']==['3','10','20']
...
boundary=read(local(source['protocol']['source_boundary']['results_path']));ids=boundary['input_freeze_before_model_load']['morehopqa_2']['input_ids']
out={'status':'MH2_FA19_qk_softmax_independent_CPU_and_query_row_audit_passed',
    'next':'No new GPU or automatic candidate from this audit. Root must account for actual native VJP and symmetric-PV failures, and measured route/PV sources, before proposing another rule.',
...
out['interpretation']='Use the actual term and query concentration values below; no MH0 row/window/generalization claim is transferred. PV-reference and routing errors remain separate. No candidate or metric result is produced by this diagnostic.'
out['CPU_audit_seconds']=time.perf_counter()-started
path=A/'dt_MH2_FA19_qk_softmax_diagnostic_summary_20260909.json';path.write_text(json.dumps(out,indent=2,allow_nan=False))
