"""One fixed remaining-case symmetric PV19 pilot;4DT100FLA84originalscores."""
...
def timeout(*args):raise TimeoutError('Frozen4DT100FLA34FA84score symmetric PV19 budget expired.')
def timed(kind,fn):
...
try:
    ...
    r['completion_scope']='Only symmetric PV19 NI0/MH2 pilot; no other cases dispatched or claimed complete for this candidate.'
    ...
    fas={method:BudgetedFinite(original_fa,16 if method=='control' else 18) for method in ('control','candidate')}
    ...
    runners={method:Qwen35DenseFiniteRunner(model,backend,finite_fla,norm_gate_rules={0:'symmetric'},
        ...
        attention_pv_rules=({19:'symmetric'} if method=='candidate' else {})) for method,backend in fas.items()}
    assert runners['control'].attention_pv_rules=={} and runners['candidate'].attention_pv_rules=={19:'symmetric'}
    ...
    for number,(key,method,phase) in enumerate(p['call_schedule']):
        ...
        fa_extra=1 if method=='candidate' else 0
        ...
        assert details['attention_pv_rules']==({'19':'symmetric'} if method=='candidate' else {})
    ...
    assert all(fas[method].entered==fas[method].returned==(16 if method=='control' else 18) for method in fas)
    ...
    r['vectors_sha256']=sha((A/'vectors.npz').read_bytes());r['status']='layer19_PV_symmetric_remaining_segment_4DT100FLA84score_complete'
