...
def attention_finite_pullback(module,values,lse,cos,sin,upstream,finite_fa,layout,boundaries,diagnostics=False,*,pv_rule='content1'):
    ...
    symmetric averages two executions of that endpoint-symmetric backend, in
    native BF16 coefficient storage, before the same input propagation.
    """
    if not isinstance(pv_rule,str) or pv_rule not in ('content1','content0','symmetric'):
    ...
    if pv_rule=='symmetric':
        # The existing logmean/QK rule is endpoint-symmetric and linear in the
        # value reference. Average its two actual allocations before further
        # propagation. This uses TWO backend executions; it is not a fused
        # one-call implementation or an average of final attribution scores.
        reverse=dict(ops,q0=ops['q1'],q1=ops['q0'],k0=ops['k1'],k1=ops['k0'],
            lse0=ops['lse1'],lse1=ops['lse0'],v0=c['value'][1::2])
        reverse_activity={} if diagnostics else None
        other=finite_fa(reverse,module.scaling,layout,reverse_activity)
        coeff={name:((coeff[name].float()+other[name].float())*0.5).to(coeff[name].dtype)
               for name in ('dq','dk','dv')}
        if diagnostics:
            activity.update(pv_rule='symmetric',finite_backend_calls=2,total_phase_launches=6,
                reverse_activity=reverse_activity,coefficient_average='FP32 add and half, native BF16 storage')
        del other,reverse
