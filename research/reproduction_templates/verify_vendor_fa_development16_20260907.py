...
import numpy as np
A=Path(__file__).resolve().parent;F=A/'snapshot${ARTIFACT_ROOT}/codex_vendor_fa_development16_20260907_v1'
...
assert p==json.loads((A/'vendor_fa_development16_protocol_20260907.json').read_text())
assert sha(F/'study.py')==p['study_sha256']==sha(A/'vendor_fa_development16_20260907.py')
...
assert len(d['native_attempt_costs'])==d['manual_attempts']==130 and all(x['completed'] for x in d['native_attempt_costs'])
...
assert sum(len(x['cost']['finite_FA_activity']) for x in d['native_attempt_costs'])==2340
...
numeric={'raw_sha256':out['raw_sha256'],'scope':'Full signed vectors, original thirty-two curves, score vectors, masks and costs; original text/token IDs and activations excluded.','records':[]}
...
assert out['curves_verified']==32
out.update(actual_FA_forwards=130*108+16*36,finite_kernel_calls=2340*3,
    ...
    conclusion_scope='All sixteen original development cases. No multi-example, long-input, independent-quality or causal-token-sign confirmation.')
...
(A/'vendor_fa_development16_summary_20260907.json').write_text(json.dumps(out,indent=2),encoding='utf-8')
(A/'vendor_fa_development16_numeric_20260907.json').write_text(json.dumps(numeric,separators=(',',':')),encoding='utf-8')
