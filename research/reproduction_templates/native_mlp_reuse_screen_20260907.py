...
def strong_run(ex,expected,pv_rule='content_P1',capture_mode='finite'):
    ...
    try:
        with measured() as full:
            ...
            reuse_context=NativeMLPSelectiveReuse(model,reuse_activity) if capture_mode=='sac_mlp' else contextlib.nullcontext()
            with reuse_context:
        ...
        for repeat in range(3):
            ...
            for mode in order:
                ...
                if mode=='sac_mlp':
                    ...
                    assert reuse['saved_mm_outputs']==reuse['reused_mm_outputs']==108
