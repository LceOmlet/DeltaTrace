"""Layer0 FLA order diagnostic and norm-gate decomposition on frozen inputs.

Two baseline DT calls, two additional reverse FLA/GDN propagations, eight original scorer calls.
Only diagnostic candidate input coefficients change. Native forward, FT and finite kernels unchanged.
"""
...
class OrderNormObserver(Layer0Observer):
    def __init__(self,layer,fla,boundaries,info):
        ...
        self.norm_features={};self.order_points={};self.original_scores=None
        self.norm_weight=cpu(layer.linear_attn.norm.weight);self.norm_eps=layer.linear_attn.norm.eps
    def decoder(self,index,d,c,e,upstream,new,terms):
        super().decoder(index,d,c,e,upstream,new,terms)
        candidate,self.reverse,self.order_metadata=compute_layer0_order_average(
            self.layer,d,c,e,upstream,new,terms,self.fla,self.boundaries)
        self.candidate=candidate[0]
    def consume(self,dc,mc):
        super().consume(dc,mc);value=self.last_features
        self.norm_features[str(self.current)]={'e':{'o':value['e']['o']},
            'c':{k:value['c'][k] for k in ['z','norm_output']}}
        if self.current:
            delta={k:self.clean['e'][k].double()-value['e'][k].double() for k in value['e']}
            actual=float((self.coeff['mo_native'].double()*delta['o']).sum())
            effects={}
            for name,coeff in [('current',self.coeff['coeff']),('reverse',self.reverse)]:
                branches={k:float((coeff[k].double()*delta['raw_g' if k=='g' else k]).sum())
                    for k in ['q','k','v','beta','g']}
                effects[name]={'branches':branches,'prediction':sum(branches.values()),
                    'prediction_minus_actual':sum(branches.values())-actual}
            left=effects['current']['prediction_minus_actual'];right=effects['reverse']['prediction_minus_actual']
            assert abs(left+self.points[str(self.current)]['terms']['GDN_FLA_including_raw_g_exp'])<1e-7
            effects.update(actual=actual,average_prediction_minus_actual=(left+right)*.5,
                antisymmetric_prediction=(left-right)*.5,sign_convention='prediction_minus_actual')
            self.order_points[str(self.current)]=effects
    def finish(self,row,parent):
        P=self.info['prompt_length'];T=self.info['total_length'];keep=set(self.info['keep'])
        original=self.original_scores.float();candidate=self.candidate[:P].float()
        S=float(original[list(keep)].double().sum());Snew=float(candidate[list(keep)].double().sum())
        clean_score=row['scoring_points']['0']['original_native_score']
        endpoint=clean_score-row['scoring_points']['20']['original_native_score']
        for step in ['1','10','20']:
            point=row['scoring_points'][step];deleted=point['input_receipt']['deleted_positions'];Aset=set(deleted)
            ng=decompose_normgate(self.paired,self.norm_features['0'],self.norm_features[step],self.coeff,
                self.norm_weight,self.norm_eps,self.norm_features['20'])
            assert abs(ng['measured_prediction_minus_actual']+self.points[step]['terms']['GDN_fused_norm_gate'])<1e-7
            groups={'eligible_deleted':sorted(Aset),'eligible_kept':sorted(keep-Aset),
                'other_prompt':[i for i in range(P) if i not in keep],'fixed_response':list(range(P,T))}
            ng['token_group_terms']={group:{name:sum(values[0][i] for i in positions)
                for name,values in ng['per_token_terms'].items()} for group,positions in groups.items()}
            point['normgate_decomposition']=ng;point['FLA_endpoint_order']=self.order_points[step]
            actual=clean_score-point['original_native_score']
            prediction=float(original[deleted].double().sum());new_prediction=float(candidate[deleted].double().sum())
            shift=float((candidate[deleted].double()-original[deleted].double()).sum())
            old_error=actual-prediction;new_error=actual-new_prediction
            assert abs(new_error-(old_error-shift))<1e-7
            point['complete_input_order_average']={'sign_convention':'actual_minus_predicted',
                'actual_model_effect':actual,'current_prediction':prediction,'candidate_prediction':new_prediction,
                'current_error':old_error,'candidate_error':new_error,'prediction_change':shift,
                'identity_error':new_error-(old_error-shift),
                'current_score_density':1-prediction/S,'candidate_score_density_on_frozen_set':1-new_prediction/Snew,
                'raw_model_density':1-actual/endpoint,
                'scope':'Layer0 FLA change only, complete input prediction on CURRENT fixed deletion set. FP64 audit sums of FP32 scores; not original evaluator FP32 reduction or candidate-new-sort MAS.'}
        row['order_average']={'metadata':self.order_metadata,'candidate_eligible_sum':Snew,
            'current_eligible_sum':S,'actual_allEOS_effect':endpoint,
            'score_relative_L2':float((candidate-original).norm()/original.norm()),
            'limitation':'Only layer0 complete FLA allocation averaged. Candidate ranking curve not measured; no MAS/RISE claim.'}
        row['extra_diagnostic_conv_preactivation_calls']=self.order_metadata['additional_work']['public_paired_conv_preactivation_calls']
        row['normgate_CPU_algebra_seconds']=sum(row['scoring_points'][step]['normgate_decomposition']['CPU_algebra_seconds'] for step in ['1','10','20'])
        norm_path=A/(key+'_normgate_inputs.pt')
        torch.save({'paired':{'e':{'o':self.paired['e']['o']},'c':{k:self.paired['c'][k] for k in ['z','norm_output']}},
            'points':self.norm_features,'coefficients':{k:self.coeff[k] for k in ['mnorm','mo_before_cast','mz']},
            'weight':self.norm_weight,'eps':self.norm_eps},norm_path)
        row['private_normgate_input_artifact']={'file':norm_path.name,'sha256':sha(norm_path.read_bytes()),'bytes':norm_path.stat().st_size}
...
try:
    ...
    from finite_fla_endpoint_order_diagnostic_20260908 import CountedFLA,compute_layer0_order_average
    from normgate_conditional_decomposition_20260908 import decompose_normgate
    ...
    counted_fla=CountedFLA(make_compiled_finite_pullback(reuse_scalar_products=False))
    ...
    for key,rec,ids,base,target,info in cases:
        row={'input':info,'scoring_points':{}};r['cases'][key]=row;observer=OrderNormObserver(layer0,counted_fla,runner.boundaries,info)
        ...
        signed=signed[0];observer.original_scores=signed[:info['prompt_length']];vectors[key+'_DT_full']=signed.numpy()
        vectors[key+'_layer0_order_average_full']=observer.candidate.numpy();np.savez_compressed(A/'vectors.npz',**vectors)
        ...
        row['limitation']='Scoring capture and CPU norm algebra add no conv calls; candidate repropagation adds one public conv preactivation and one autograd call per case, counted separately. FLA includes raw-g exp. Terms are signed conditional prediction errors, not causal token signs.'
        observer.finish(row,prior['cases'][key]);r['extra_conv_preactivation_calls']+=row['extra_diagnostic_conv_preactivation_calls']
        r['finite_FLA_calls']=counted_fla.calls;save();del observer,signed;gc.collect()
    assert r['DT_calls']==2 and r['scoring_forwards']==8 and r['extra_conv_preactivation_calls']==2
    ...
    assert sum(x['kind']=='current' for x in counted_fla.calls)==48 and sum(x['kind']=='reverse' for x in counted_fla.calls)==2
    r['status']='order_norm_localization_complete'
except Exception:
