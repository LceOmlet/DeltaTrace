...
try:
    ...
    def run(kind,phase,profile=False):
        counter={'cached':'cached_adjoint_mixed_attempts','finite':'finite_pullback_attempts','native':'native_backward_attempts'}[kind]
        ceiling={'cached':1,'finite':5,'native':5}[kind]
        ...
        try:
            ...
            save()
    for kind in ['cached','finite','native']:
        out=run(kind,'cold');persist(kind+'_coefficients',out);del out
    # Three bounded alternated pairs; no retained output or synchronization hidden in the next call.
    for iteration,order in enumerate([['finite','native'],['native','finite'],['finite','native']]):
        for kind in order:
            out=run(kind,'formal_'+str(iteration))
            assert all(torch.isfinite(v).all() for v in out.values());del out
    for kind in ['finite','native']:
        out=run(kind,'diagnostic',profile=True);del out
    r['sources_after']=source_receipt();assert r['sources_before']==r['sources_after']
    assert r['cached_adjoint_mixed_attempts']==1 and r['finite_pullback_attempts']==r['native_backward_attempts']==5
    r['status']='GPU_mixed_finite_and_matched_native_backward_executed'
except Exception:
    ...
    print(json.dumps({'status':r['status'],'attempts':[r['cached_adjoint_mixed_attempts'],r['finite_pullback_attempts'],r['native_backward_attempts']],
