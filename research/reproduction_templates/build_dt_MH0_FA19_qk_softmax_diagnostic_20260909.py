"""Freeze current MH0 QK/softmax diagnosis using the existing compiled library; no launch."""
import ast,base64,copy,hashlib,json,shlex,zlib
from pathlib import Path
A=Path(__file__).resolve().parent;R=A.parent/'DeltaTrace';sha=lambda b:hashlib.sha256(b).hexdigest()
S=A/'snapshot${ARTIFACT_ROOT}/codex_dt_MH0_FA19_internal_20260909_v1';H=A/'snapshot${ARTIFACT_ROOT}/codex_dt_MH0_FA19_native_hybrid_20260909_v1';V=A/'snapshot${ARTIFACT_ROOT}/codex_dt_fa19_qk_softmax_diagnostic_20260908_v1'
...
assert sr['status']=='MH0_current_FA19_1native_kwargs5replay1finite_internal_complete'
assert hr['status']=='MH0_eleven_native_FA_hybrid_contrasts_complete' and hr['native_FA_calls_entered']==hr['native_FA_calls_returned']==11
...
summary=A/'dt_MH0_FA19_native_hybrid_summary_20260909.json';su=json.loads(summary.read_bytes())
assert su['status']=='MH0_FA19_native_hybrid_independent_CPU_audit_passed' and su['results_sha256']==sha((H/'results.json').read_bytes())
assert hp['source_results_sha256']==sha((S/'results.json').read_bytes())
assert hp['input_sha256']==sr['input']['input_sha256']=='6e1df21d426c5eb28217976f63b7c7932e502bde4fb064f77867df8205ebd618'
assert sr['input']['total_length']==853
files={'study.py':(R/'research/reproduction_templates/dt_MH0_FA19_qk_softmax_diagnostic_20260909.py').read_bytes()}
...
remote='${ARTIFACT_ROOT}/codex_dt_MH0_FA19_qk_softmax_diagnostic_20260909_v1';python='${ARTIFACT_ROOT}/codex_qwen35_isolated_import_20260908_v1/env/bin/python'
protected=[]
...
p.update({
    ...
    'fixed_steps':['3','10','20'],'case':'morehopqa_0','layer':19,'total_length':853,
    ...
    'source_adaptation':'Current private[coeff], private[native][B2] and private[native][0/3/10/20] replace prior MH1 nested schema. Shape T853, exact source masks, actual native scale and post-H2D contiguous layouts. Wrapper/extension byte-identical; prediction function and four-term algebra unchanged.',
    ...
    'artifact_contract':'Current private1.272GB capture stays remote with SHA guard. New diagnostic_coefficients_private.pt retains actual dq/dk/dv/tau/center/LSE/rows privately; public binary NPZ retains contractions. Existing library is read at original remote path, never overwritten or included as new build.',
    'files_sha256':{name:sha(raw) for name,raw in files.items()}})
for name,data in files.items():
    if name.endswith('.py'):ast.parse(data,filename=name)
files['protocol.json']=json.dumps(p,indent=2).encode();pp=A/'dt_MH0_FA19_qk_softmax_diagnostic_protocol_20260909.json';lp=A/'launch_dt_MH0_FA19_qk_softmax_diagnostic_20260909.json'
