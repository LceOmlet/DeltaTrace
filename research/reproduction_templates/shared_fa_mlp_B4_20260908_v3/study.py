...
def batch_run(examples,expected,reuse=None):
    ...
    report['manual_attempts']=report.get('manual_attempts',0)+1;save()
    cost={k:0 for k in ['native_forwards','native_forward_trajectories','extra_replay_calls','extra_replay_trajectories','peak_allocated_bytes']}
    ...
    try:
        with measured() as cost:
            ...
            reuse_context=NativeMLPSelectiveReuse(model,reuse,first_cached_layer=4) if reuse is not None else contextlib.nullcontext()
            with reuse_context:
