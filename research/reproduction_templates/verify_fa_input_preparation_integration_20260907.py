...
import numpy as np
A=Path(__file__).resolve().parent;F=A/'snapshot${ARTIFACT_ROOT}/codex_fa_input_preparation_integration_20260907_v1'
...
assert d['status']=='FA_input_preparation_B1_and_real_B4_exact'
...
out={'status':'FA_input_preparation_B1_and_real_B4_verified','raw_sha256':sha(F/'results.json'),
...
for index,g in enumerate(d['integration_groups']):
    ...
    for repeat in range(3):
        old,new=[next(r['result'] for r in g['runs'] if r['mode']==mode and r['repeat']==repeat) for mode in ['old','fused_prepare']]
        ...
        for mode,result in [('old',old),('fused_prepare',new)]:
            ...
            for item in finite:
                ...
                for pos in [1,3,4,7]:assert shapes[pos][1]==8
    ...
    measured={mode:[r['result']['seconds'] for r in g['runs'] if r['mode']==mode and not r['warm']] for mode in ['old','fused_prepare']}
    ...
    numeric.append(g)
profile=d['fused_prepare_B4_profile'];assert sha(F/profile['file'])==profile['sha256']
...
(A/'fa_input_preparation_integration_summary_20260907.json').write_text(json.dumps(out,indent=2))
(A/'fa_input_preparation_integration_numeric_20260907.json').write_text(json.dumps({'groups':numeric},separators=(',',':')))
