"""Four normal DT calls; explicit layer0 FLA average;84 original scorer forwards."""
...
def timeout(*args):raise TimeoutError('Frozen4DT98FLA84score layer0 endpoint-average pilot budget expired.')
def timed(kind,fn):
...
try:
    ...
    for number,(key,method) in enumerate(p['call_schedule']):
        ...
        gc.collect();torch.cuda.synchronize();assert r['DT_entered']<4;r['DT_entered']+=1;tick=time.perf_counter();details=None
        try:
    ...
    r['vectors_sha256']=sha((A/'vectors.npz').read_bytes());r['status']='layer0_FLA_endpoint_average_4DT98FLA84score_pilot_complete'
    ...
    r['cost_scope']='One attribution per method/case, opposite order across cases. Existing normal-runner diagnostics and all extra layer0 FLA work, endpoint copies and six pointwise averages remain in measured attribution time/peak memory. No candidate diagnostic synchronizations. Compilation, initialization, scoring, and failures remain in total wall time. Pilot cost only; no stable speed or universal quality claim.'
    save()
