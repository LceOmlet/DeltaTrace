...
struct FiniteParams {
    const mctlass::half_t *q0,*k0,*q1,*k1,*v0,*u,*qmean,*kmean;
    const float *lse0,*lse1;
    float *tau,*center;
    mctlass::half_t *dq,*dk,*dv;
    int batch,heads,length;
    float scale;
};
...
__global__ void deltatrace_fa_finite_p1_kernel(FiniteParams p) {
    ...
    for(int col0=begin;col0<end;col0+=N) {
        auto pair=[&](const E *left,const E *right,auto &acc) {
            __syncthreads();
            auto gA=make_tensor(make_gmem_ptr(left+head_offset+int64_t(row0)*D),
                               Shape<Int<M>,Int<D>>{},Stride<Int<D>,_1>{});
            auto gB=make_tensor(make_gmem_ptr(right+head_offset+int64_t(col0)*D),
        ...
        if constexpr(Phase==2) {
            pair(p.k0,p.q0,a0);pair(p.k1,p.q1,a1);pair(p.v0,p.u,at);
        } else {
            pair(p.q0,p.k0,a0);pair(p.q1,p.k1,a1);pair(p.u,p.v0,at);
        }
        #pragma unroll
        ...
        if constexpr(Phase==0) {
            ...
            auto multiply=[&](auto &weights,const E *values,auto &acc) {
                __syncthreads();
                auto gB=make_tensor(make_gmem_ptr(values+head_offset+int64_t(col0)*D),
                                   Shape<Int<N>,Int<D>>{},Stride<Int<D>,_1>{});
                auto fromGB=global_thread.partition_S(gB);
                flash::copy<false,true,true>(global_copy,fromGB,toB,coordsB,predB,length-col0);
                __syncthreads();
            ...
            if constexpr(Phase==1) multiply(a0,p.kmean,accQ);
...
extern "C" int deltatrace_fa_finite_p1(
    const void *q0,const void *k0,const void *q1,const void *k1,const void *v0,
    const void *u,const void *qmean,const void *kmean,const void *lse0,const void *lse1,
    void *tau,void *center,void *dq,void *dk,void *dv,int batch,int heads,int length,
    float scale,void *stream_ptr) {
    using E=mctlass::half_t;
    if(batch<1||heads<1||length<1)return -1;
    FiniteParams p{(const E*)q0,(const E*)k0,(const E*)q1,(const E*)k1,(const E*)v0,
        (const E*)u,(const E*)qmean,(const E*)kmean,(const float*)lse0,(const float*)lse1,
        (float*)tau,(float*)center,(E*)dq,(E*)dk,(E*)dv,batch,heads,length,scale};
