 ...
 * DIAGNOSTIC ONLY: derived from the pinned production BF16/D256 finite source.
 * Phase 1 additionally reuses pair/flash::gemm for saved actual C/A score tiles.
 * Export only two row contractions: W_FP32 * delta_score and W_BF16 * delta_score.
 * Original finite Q/K/V coefficients and all native/vendor sources are retained.
 */
#include <cuda.h>
...
struct ConditionalDiagParams {
    const void *q0,*k0,*q1,*k1,*v0,*u;
    const float *lse0,*lse1;
    float *tau,*center;
    void *dq,*dk,*dv;
    const int *valid_lengths;
    const void *actual_qc,*actual_kc,*actual_qa,*actual_ka;
    float *conditional_fp32,*conditional_bf16;
...
__global__ void deltatrace_fa_finite_p1_conditional_diag_kernel(ConditionalDiagParams p) {
    ...
    if(row0>=valid) {
        if constexpr(Phase==0) {
            ...
            if constexpr(Phase==1) {
                for(int j=tid;j<M;j+=blockDim.x) if(row0+j<length) {
                    const int64_t pos=int64_t(bh)*length+row0+j;
                    p.conditional_fp32[pos]=0.f;p.conditional_bf16[pos]=0.f;
                }
            }
        }
        return;
    ...
    for(int col0=begin;col0<end;col0+=N) {
        ...
        if constexpr(Phase==0) {
            ...
            if constexpr(Phase==1) {
                // endpoint=-1 preserves firstEndpointB, the original frozen midpoint.
                // a0 remains the original FP32 finite softmax weight tile throughout.
                auto actual_c=make_fragment_like(a0);
                auto actual_a=make_fragment_like(a0);
                pair(static_cast<const E*>(p.actual_qc),static_cast<const E*>(p.actual_kc),actual_c,-1);
                pair(static_cast<const E*>(p.actual_qa),static_cast<const E*>(p.actual_ka),actual_a,-1);
                // Match the exact original weight conversion before its multiplier GEMM.
                CONVERT_TENSOR_TYPE(float,E,a0,diagnostic_half_weights)
                #pragma unroll
                for(int i=0;i<size(a0);++i) {
                    const int query=row0+get<0>(coords(i)),key=col0+get<1>(coords(i));
                    const float ds=(query<valid && key<valid && key<=query)
                        ? (actual_c(i)-actual_a(i))*p.scale : 0.f;
                    actual_c(i)=a0(i)*ds;
                    actual_a(i)=float(diagnostic_half_weights(i))*ds;
                }
                auto fp_rows=make_tensor(actual_c.data(),flash::convert_layout_acc_rowcol(actual_c.layout()));
                auto bf_rows=make_tensor(actual_a.data(),flash::convert_layout_acc_rowcol(actual_a.layout()));
                flash::SumOp<float> sum;
                flash::thread_reduce_<false>(fp_rows,denominator,sum);
                flash::thread_reduce_<false>(bf_rows,numerator,sum);
            }
            ...
            if(get<1>(rowcoords(0))==0) {
                ...
                for(int i=0;i<size(denominator);++i) {
                    ...
                    if(r<length) {
                        ...
                        p.conditional_fp32[pos]=r<valid?denominator(i):0.f;
                        p.conditional_bf16[pos]=r<valid?numerator(i):0.f;
                    }
                }
            }
        }
...
extern "C" int deltatrace_fa_finite_p1_bf16_d256_conditional_diag(
    ...
    const void *actual_qc,const void *actual_kc,const void *actual_qa,const void *actual_ka,
    void *conditional_fp32,void *conditional_bf16,int batch,int heads,int kv_heads,int length,
    float scale,void *stream_ptr) {
    using E=mctlass::bfloat16_t;
    if(batch<1||heads<1||kv_heads<1||heads%kv_heads!=0||length<1)return -1;
    ConditionalDiagParams p{(const E*)q0,(const E*)k0,(const E*)q1,(const E*)k1,(const E*)v0,
        (const E*)u,(const float*)lse0,(const float*)lse1,
        (float*)tau,(float*)center,(E*)dq,(E*)dk,(E*)dv,(const int*)valid_lengths,
        actual_qc,actual_kc,actual_qa,actual_ka,(float*)conditional_fp32,(float*)conditional_bf16,
    ...
    deltatrace_fa_finite_p1_conditional_diag_kernel<0><<<grid,FiniteTraits::kNThreads,shared,stream>>>(p);
    auto error=cudaGetLastError();if(error!=cudaSuccess)return int(error);
    deltatrace_fa_finite_p1_conditional_diag_kernel<1><<<grid,FiniteTraits::kNThreads,shared,stream>>>(p);
    error=cudaGetLastError();if(error!=cudaSuccess)return int(error);
    deltatrace_fa_finite_p1_conditional_diag_kernel<2><<<grid,FiniteTraits::kNThreads,shared,stream>>>(p);
