...
class VendorFAFiniteP1CompiledPreparation:
    def __init__(self,library,expected_sha256):
        path=Path(library)
        assert hashlib.sha256(path.read_bytes()).hexdigest()==expected_sha256
        self.library=ctypes.CDLL(str(path));self.operation=self.library.deltatrace_fa_finite_p1_gqa
        self.operation.argtypes=[ctypes.c_void_p]*15+[ctypes.c_int]*4+[ctypes.c_float,ctypes.c_void_p]
    ...
    def __call__(self,operands,scale,activity=None):
        ...
        values=list(prepare_finite_fa_inputs(*(operands[name] for name in names)))
        ...
        if activity is not None:
            activity['input_preparation']='public_torch_compile_inductor_same_expressions'
