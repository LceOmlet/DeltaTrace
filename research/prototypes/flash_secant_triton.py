...
QK products use original FP16 operands with FP32 accumulation. dQ now uses a
separate query-owned traversal instead of colliding FP32 atomic accumulation;
the extra traversal is charged. FP64 streaming audit credits are retained.
"""
import torch
import triton
...
@triton.jit
def _secant_query_pullback(Q0,K0,V0,LSE0,Q1,K1,V1,LSE1,U,C,DQ,
                          N:tl.constexpr,H:tl.constexpr,KV:tl.constexpr,D:tl.constexpr,
                          SCALE:tl.constexpr,BM:tl.constexpr,BN:tl.constexpr):
    qm=tl.program_id(0)*BM;bh=tl.program_id(1);b=bh//H;head=bh%H;kh=head//(H//KV)
    qi=qm+tl.arange(0,BM);ki=tl.arange(0,BN);di=tl.arange(0,D)
    ...
    for start in range(0,tl.minimum(qm+BM,N),BN):
        ...
        dq+=tl.dot(ds,(k0.to(tl.float32)+k1.to(tl.float32))*.5,input_precision='ieee')
    tl.store(DQ+up,dq*SCALE,mask=qi[:,None]<N)
...
def finite_attention_pullback(q0,k0,v0,lse0,q1,k1,v1,lse1,upstream):
    ...
    dq=torch.empty((b,h,n,d),device=q0.device,dtype=torch.float32)
    ...
    COMPILED_KERNELS['query']=_secant_query_pullback[(triton.cdiv(n,bm),b*h)](*args,center,dq,n,h,kv,d,d**-.5,bm,bn,num_warps=4,num_stages=1)
    ...
    return {'dq':dq,'dk_per_query_head':dkh,'dv_per_query_head':dvh,
        ...
        'scope':'Isolated finite attribution pullback, not native model gradient. Actual paired QKV/LSE. Query-owned FP32 dQ accumulation, extra traversal charged; FP64 streaming audit. No dense NxN device tensor.'}
