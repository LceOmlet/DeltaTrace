...
class VendorFAFiniteP1TwoSweeps:
    def __init__(self,library,expected_sha256):
        path=Path(library)
        assert hashlib.sha256(path.read_bytes()).hexdigest()==expected_sha256
        self.library=ctypes.CDLL(str(path));self.operation=self.library.deltatrace_fa_finite_p1_two_sweeps
    ...
    def __call__(self,operands,scale,activity=None):
        ...
        if activity is not None:
            ...
            activity['extra_shared_tile']='One native-FA-layout32x32FP16 coefficient tile,2048 bytes'
