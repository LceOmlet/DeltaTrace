...
using OwnerTraits=Flash_fwd_kernel_traits<128,64,32,4,false,false,mctlass::half_t>;
...
template<int Phase,typename Traits=std::conditional_t<Phase==0,FiniteTraits,OwnerTraits>>
__global__ void deltatrace_fa_finite_p1_kernel(FiniteParams p) {
    ...
    // All three owner operands are in MMA registers before any RHS use.
    // Follow native Share_Q_K_smem: reuse the now-dead shared owner staging tile.
    auto sB=make_tensor(sA.data(),typename Traits::SmemLayoutKV{});
    ...
    auto currentRightU=make_fragment_like(toB);
    ...
    // Each sweep owns fixed left operand rows. Cache their original FP16 bits
    // once per owner block; original MMA, geometry and accumulation stay intact.
    ...
    auto ownerU=make_fragment_like(regA);
    {
        auto load_owner=[&](const E *left,auto &reg) {
            ...
            CUTE_STATIC_ASSERT_V(size<1>(fromA)==size<1>(register_view));
            cute::copy(copyA,fromA,register_view);
            __syncthreads();
        };
        if constexpr(Phase==2) {
            load_owner(p.k0,owner0);load_owner(p.k1,owner1);load_owner(p.v0,ownerU);
        } else {
            load_owner(p.q0,owner0);load_owner(p.q1,owner1);load_owner(p.u,ownerU);
        }
    }
    ...
    for(int col0=begin;col0<end;col0+=N) {
        auto pair=[&](const E *left,const E *right,auto &acc,int endpoint) {
            ...
            __syncthreads();
            if constexpr(Phase==2) {
                if(endpoint==-1)cute::copy(toB,currentRightU);
            }
            ...
            clear(acc);
            // The original vendor implementation already supports A_in_regs.
            // Same FP16 operands and identical native MMA K loop; no shared-A reload.
            if(endpoint==0)flash::gemm<true,false>(acc,owner0,regB,fromA,fromB,mma,copyA,copyB,threadA,threadB);
            else if(endpoint==1)flash::gemm<true,false>(acc,owner1,regB,fromA,fromB,mma,copyA,copyB,threadA,threadB);
            else flash::gemm<true,false>(acc,ownerU,regB,fromA,fromB,mma,copyA,copyB,threadA,threadB);
        };
        ...
        if constexpr(Phase==0) {
            ...
            auto multiply=[&](auto &weights,const E *values,auto &acc,bool shared_mean) {
                ...
                if(shared_mean) {
                    ...
                    // Phase2 already loaded this exact U tile for V0 x U.
                    // Preserve it across the mean-Q multiplier; no second load.
                    cute::copy(currentRightU,toB);
                }
                __syncthreads();
...
extern "C" int deltatrace_fa_finite_p1_shared_mean_reuse(
    ...
    constexpr int shared=std::max<int>(size(typename FiniteTraits::SmemLayoutQ{}),size(typename FiniteTraits::SmemLayoutKV{}))*sizeof(E);
    ...
    dim3 owner_grid((length+OwnerTraits::kBlockM-1)/OwnerTraits::kBlockM,batch*heads);
    constexpr int owner_shared=std::max<int>(size(typename OwnerTraits::SmemLayoutQ{}),size(typename OwnerTraits::SmemLayoutKV{}))*sizeof(E);
    deltatrace_fa_finite_p1_kernel<1><<<owner_grid,OwnerTraits::kNThreads,owner_shared,stream>>>(p);
    error=cudaGetLastError();if(error!=cudaSuccess)return int(error);
    deltatrace_fa_finite_p1_kernel<2><<<owner_grid,OwnerTraits::kNThreads,owner_shared,stream>>>(p);
