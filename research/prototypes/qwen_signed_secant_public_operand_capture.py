...
from qwen_signed_secant_pv_operand_capture import propagate_signed_secant as full_secant_pullback
...
def propagate_paired_secant(model,before,after,progress=None,pv_rule='symmetric',activity=None):
    ...
    try:
        result=full_secant_pullback(model,left,right,'rescale',progress=progress,pv_rule=pv_rule)
        ...
        result['recomputation_scope']='One original native FA batch=2 root forward and36 original native FA batch=2 decoder replays. Two endpoint trajectories in each; same full-secant arithmetic on real endpoint tensor views. 36 additional original public FA batch=2 metadata calls, documented LSE only, testing matrix empty. No surrogate/native-autograd replacement. Boundaries checked against actual batch=2 root outputs.'
        return result
