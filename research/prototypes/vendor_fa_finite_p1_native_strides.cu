...
__global__ void deltatrace_fa_finite_p1_kernel_strided(FiniteParams p) {
    ...
    for(int col0=begin;col0<end;col0+=N) {
        auto pair=[&](const E *left,const E *right,int left_id,int right_id,auto &acc,int endpoint) {
            __syncthreads();
            auto gA=make_tensor(make_gmem_ptr(left+input_offset(left_id)+int64_t(row0)*p.strides[left_id*3+2]),
                               Shape<Int<M>,Int<D>>{},make_stride(p.strides[left_id*3+2],_1{}));
            auto gB=make_tensor(make_gmem_ptr(right+input_offset(right_id)+int64_t(col0)*p.strides[right_id*3+2]),
        ...
        if constexpr(Phase==0) {
            ...
            auto multiply=[&](auto &weights,const E *values,auto &acc,bool shared_mean) {
                ...
                auto gB=make_tensor(make_gmem_ptr(values+input_offset(5)+int64_t(col0)*p.strides[17]),
...
extern "C" int deltatrace_fa_finite_p1_native_strides(
    ...
    float scale,const int64_t *input_strides,void *stream_ptr) {
    ...
    // Copy host metadata by value before launch; no host pointer is used by a kernel.
    ...
    deltatrace_fa_finite_p1_kernel_strided<0><<<grid,FiniteTraits::kNThreads,shared,stream>>>(p);
    auto error=cudaGetLastError();if(error!=cudaSuccess)return int(error);
    deltatrace_fa_finite_p1_kernel_strided<1><<<grid,FiniteTraits::kNThreads,shared,stream>>>(p);
    error=cudaGetLastError();if(error!=cudaSuccess)return int(error);
    deltatrace_fa_finite_p1_kernel_strided<2><<<grid,FiniteTraits::kNThreads,shared,stream>>>(p);
