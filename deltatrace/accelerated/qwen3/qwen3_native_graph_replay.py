...
"""
class GraphLayerReplay:
    def __init__(self,model,checkpoint,validation,reference=None,activity=None):
    ...
    def __getitem__(self,index):
        ...
        layer=self.model.model.layers[index];values={};handles=[];versions={}
        def retained(name,value):
            value=value.detach();versions[name]=value._version;return value
        def recorder(in_name=None,out_name=None):
            def record(_module,args,output):
                if in_name is not None:
                    values[in_name]=retained(in_name,args[0])
                if out_name is not None:
                    values[out_name]=retained(out_name,output)
            return record
        ...
        def observer(frame,event,result):
            if event=='return' and frame.f_code is public_code:
                ...
                for name in ['q','k','v']:
                    values['fa_'+name]=retained('fa_'+name,local[name])
                values['fa_out']=retained('fa_out',result)
        ...
        try:
            with torch.no_grad():
                ...
                with LocalCaptureEvents([public_code],observer,returns_only=True):
        ...
        for name,version in versions.items():assert values[name]._version==version,(index,name)
        ...
        check={'layer':index,'native_input_exact':self.validation.equal(values['x'],self.checkpoint['layers'][index]['x'],f'layer{index}.root_input'),
            'native_output_exact':self.validation.equal(values['out'],expected_output,f'layer{index}.root_output')}
        self.boundary_checks.append(check)
        # Both exact boundary predicates are resolved before public return.
