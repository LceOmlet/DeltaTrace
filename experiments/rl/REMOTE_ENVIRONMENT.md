...
- 随后的原模型缓存对照（已完成 actor checkpoint1、B4、原始记录 IDs）在
  不运行 DT 的条件下定位到第 0 层 GDN：norm、投影、卷积、q/k/v/beta/g
  相同，递归 h/o 不同。`native-cached-gdn-reference.json` 中，原 FLA
  FP32 reference/0.005 断言下，完整输出误差比 0.002895，缓存输出相对
  完整参考 0.003013，条件于原缓存初态的输出 0.002932，均通过。
  这部分不能再仅凭整链向量差异判为 DT 接口错误。
- 真实缓存操作数的 B4/T1200 反向检查则揭示未解决项：原生 dK 比率
  0.011068，DT 重合端点/原 head8 有限反向 0.010684，超过原 0.008；
  其余 q/v/beta/g 通过。此前 layer2 无缓存操作数通过不能覆盖此用例。
  原随机 BF16 失败用例通过已有 `use_qk_l2norm_in_kernel=False` 选项后
  均通过，仅用于定位融合归一化影响，未修改模型配置。
- 原 FLA 的 FP16 运算有界诊断消除了这批缓存梯度超差：dK 0.002404，
  五项原断言均通过；输入转换最大差 2.98e-8，热算子 10.15ms，对照 BF16
  10.48ms。首次编译 73.12 秒单独保留；不是整网速度结果。FP32 原生尝试
  因 kernel shared-memory 需求 90624>65536 失败，未采用。
  后续隔离候选已接通 Qwen 的原 FLA 算子 dtype 转换、实际 norm 输入捕获、
  DT 原有限 GEMM 的 dtype 跟随；未复制算子/模型前向。真实缓存重合端点
  的有限 dK 比率降至 0.002171，五项原断言通过；默认 BF16 六个系数字段
  与旧模块在真实非重合端点上逐值一致。两项 FP16 融合归一化原测试函数通过。
  真实已训练 actor 的短链三条路径均完成且有限，但 prefix-on 与原默认路径
  signed 相对 L2 差仍为 0.6363，prefix-off 为 0.00741；未把算子改善当作
  整条前缀复用已修好。首次候选探针的 norm 观察计数错误已修正，失败保留，
  正式任务没有使用该探针。候选目录为 `candidates/native-fla-fp16-20260925`，
  默认仍不启用 dtype 转换；诊断 helper 在 rollout-major-cost，生产设置不变。
  `native-fp16-capacity.json` 已通过精确32768、DT/actor B4、1024 action槽位、
  休眠原 vLLM32、两次原 PPO 非零更新及原 LoRA 同步。两次 DT 90.80/28.26秒；
  第二次原 PPO 更新72.61秒，历史相同输入 SHA256/卸载配置为72.42秒。
  梯度范数0.022705/0.023193；阶段结束时物理占用最高约50.84GiB，不是连续峰值。
  Torch 虚拟 allocator 的84–92GiB含休眠映射，不能当作物理超限；该次无OOM。
  新回执统一以 `native-` 开头，源脚本位于本机
  `research/temporary/rl_recovery_20260925` 和远端 rollout-major-cost；
  3.47GB 原始 GDN 操作数仅保存在远端 `native-prefix-gdn-operands.pt`，
  后续直接复用，不再次加载整模型采集同一组数据。
