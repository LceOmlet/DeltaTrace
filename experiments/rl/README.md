...
- MetaX C550 实测：已复用现有环境并复现/避开 categorical seed 的编译器非法
  地址错误；随后在首个 Sokoban 奖励事件的正式 DT 校验失败。原生目标差
  `0.01541`，head 输入端有限贡献 `0.08402`，source 归因和 `0.05779`。
  失败及逐层诊断已保存；未放宽阈值、运行 PPO 更新或宣称三个任务归因通过。
