...
**HotpotQA 公平性修正已完成全部 48 例。** 新增 [`hotpot_evidence.py`](hotpot_evidence.py) 原生支持句评分模块，修复第 8 例标题/正文 gold 坐标及前导空格导致的分句偏移；双方共用句均值和整句实际词元预算。最终答案目标、10% 正文有效词元预算下，支持事实 Recall 为 DT **68.58%**、live FT K3 **66.15%**；另修订两处缺失关系标签后为 **68.58% 对 67.19%**，区间均跨零。完整回答目标、官方标签、修订标签、45 例敏感性子集均完整报告。新评分需显式使用 [hotpot-evidence-v2 的入口与复现命令](../../research/temporary/hotpot_fairness_20260910/RESULTS.md)；下述历史入口的 regex、旧标签分数保持冻结。
