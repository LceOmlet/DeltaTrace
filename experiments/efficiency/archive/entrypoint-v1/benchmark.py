...
The pinned author's exp1 module supplies token construction and the FT runner.
    ...
    try:
        ...
        if args.method == 'DT':
            ...
            def run():
                ...
                return propagate_paired_secant(model, before, after, pv_rule='content_P1', finite_attention=finite_fa)
        else:
            run = author.make_attr_runner('ifr_multi_hop_both', model, tokenizer,
                protocol['chunk_tokens'], protocol['sink_chunk_tokens'], 1, prompt, target)
            engine = next(cell.cell_contents for cell in run.__closure__
                          if isinstance(cell.cell_contents, ft.LLMIFRAttributionBoth))
        ...
        for index in range(protocol['warmups'] + protocol['repeats']):
            ...
            actual_digest = sha(torch.cat((engine.prompt_ids, engine.generation_ids), 1).cpu().numpy().tobytes())
            assert actual_digest == expected_digest
            row['input_ids_sha256'] = actual_digest
            if args.method == 'DT':
                scores = np.asarray(value['signed_full_sequence'], dtype=np.float64)
                assert np.isfinite(scores).all()
                row['scores_sha256'] = sha(scores.tobytes())
                row['scores_shape'] = list(scores.shape)
            else:
                matrix = value.attribution_matrix
                row['scores_shape'] = list(matrix.shape)
                # The original matrix intentionally contains NaNs in unused rows.
                user_scores = torch.nan_to_num(matrix[:, :len(report['input']['user_positions'])]).sum(0)
                row['scores_sha256'] = sha(user_scores.detach().cpu().numpy().tobytes())
