...
inline __device__ void compute_dq_dk_dv_1colblock_hdim128_32_32(const Params &params, const int bidb, const int bidh,
                                                                const int n_block) {
    ...
    const index_t row_offset_dq_accum =
        ...
        ((m_block_max - 1) * kBlockM + (params.cu_seqlens_q == nullptr ? 0 : 128 * bidb)) * params.h *
            params.d_rounded +
        bidh * params.d_rounded + (!params.deterministic ? 0 : blockIdx.x * params.dq_accum_split_stride);
    ...
    Tensor sV = make_tensor(Kernel_traits::Is_K_in_regs ? sdO.data() : sK.data() + size(sK),
                            typename Kernel_traits::SmemLayoutKV{});
    ...
    using GmemLayoutAtomdQaccum =
        std::conditional_t<!Seq_parallel, typename Kernel_traits::GmemTiledCopydQaccum,
                           typename Kernel_traits::GmemTiledCopydQaccumAtomicAdd_hdim128_32_32>;
...
#pragma unroll
    ...
    if constexpr (is_tile32x32 == true) {
        gdQaccum.data() = gdQaccum.data() + kBlockM * params.h * params.d_rounded;
    }
...
#pragma unroll
    ...
    if constexpr (Kernel_traits::Is_K_in_regs || Kernel_traits::Is_V_in_regs) {
        __syncthreads();
    }
...
#pragma unroll
        ...
        flash::gemm<false, Kernel_traits::Is_K_in_regs>(acc_s, tSrQ, tSrK, tSsQ, tSsK, tiled_mma_sdp, smem_tiled_copy_QdO,
...
#pragma unroll
        ...
        if constexpr (is_tile32x32 == true) {
            gdQaccum.data() = gdQaccum.data() + (-int(kBlockM * params.h * params.d_rounded));
        }
        ...
        flash::gemm<false, Kernel_traits::Is_K_in_regs>(acc_dq, tdQrdS, tdQrKt, tdQsdS, tdQsKt, tiled_mma_dq,
...
template <typename Kernel_traits, bool Is_dropout, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN,
          bool Is_even_K, bool Is_deterministic, typename Params>
inline __device__ void compute_dq_dk_dv_seqk_parallel_hdim128_32_32(const Params &params) {
    ...
    if constexpr (Is_deterministic) {
        for (int n_block = blockIdx.x;
            ...
            compute_dq_dk_dv_1colblock_hdim128_32_32<Kernel_traits, Is_dropout, Is_causal, Is_local, Has_alibi,
                                                     Is_even_MN, Is_even_K, false, false, true>(params, bidb, bidh,
                                                                                                n_block);
        }
    } else {
        const int n_block = blockIdx.x;
        compute_dq_dk_dv_1colblock_hdim128_32_32<Kernel_traits, Is_dropout, Is_causal, Is_local, Has_alibi, Is_even_MN,
                                                 Is_even_K, false, false, true>(params, bidb, bidh, n_block);
    }
}
