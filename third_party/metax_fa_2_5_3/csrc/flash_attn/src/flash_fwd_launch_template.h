...
bool Is_even_MN, bool Is_even_K, bool Return_softmax, int Rowblock_Parallel_Num = 1>
__global__ void flash_fwd_kernel(const Flash_fwd_params params) {
    ...
    flash::compute_attn<Kernel_traits, Is_dropout, Is_causal, Is_local, Has_alibi, Has_attn_mask, Is_even_MN, Is_even_K,
                        Return_softmax, Flash_fwd_params, Rowblock_Parallel_Num>(params);
}
...
void run_flash_fwd(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    if (Is_causal == true && num_m_block % 2 == 0) {
        grid.x = (num_m_block + 1) / 2;
        select_flag = true;
    }
            ...
            LOCAL_SWITCH((params.window_size_left >= 0 || params.window_size_right >= 0) && !Is_causal, Is_local, [&] {
                BOOL_SWITCH(return_softmax, ReturnSoftmaxConst, [&] {
                    ALIBI_SWITCH(params.alibi_slopes_ptr != nullptr, Has_alibi, [&] {
                        BOOL_SWITCH(params.has_attn_mask, Has_attn_mask, [&] {
                            ROWNUM_SWITCH(select_flag, Rowblock_Parallel_Num, [&]  {
                                auto kernel = &flash_fwd_kernel<Kernel_traits, Is_dropout, Is_causal, Is_local &&
                                                                !Is_causal, Has_alibi && !Has_attn_mask, Has_attn_mask &&
                                                                !Is_causal && !Is_local && !Has_alibi, IsEvenMNConst &&
                                                                IsEvenKConst && !Is_local && !ReturnSoftmaxConst &&
                                                                Kernel_traits::kHeadDim <= 128, IsEvenKConst,
                                                                ReturnSoftmaxConst && Is_dropout,Rowblock_Parallel_Num>;
...
void run_flash_splitkv_fwd(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    dim3 grid(num_m_block, params.num_splits > 1 ? params.b * params.h : params.h, params.num_splits > 1 ? params.num_splits : params.b);
    const bool is_even_MN = params.cu_seqlens_q == nullptr && params.cu_seqlens_k == nullptr &&
                            params.seqlen_k % Kernel_traits::kBlockN == 0 && params.seqlen_q % Kernel_traits::kBlockM == 0;
    const bool is_even_K = params.d == Kernel_traits::kHeadDim;
                        ...
                        BOOL_SWITCH(params.knew_ptr != nullptr, Append_KV, [&] {
                            ALIBI_SWITCH(params.alibi_slopes_ptr != nullptr, Has_alibi, [&] {
                                auto kernel = &flash_fwd_splitkv_kernel<Kernel_traits, Is_causal, Is_local && !Is_causal, Has_alibi,
                                                IsEvenMNConst && !Append_KV && IsEvenKConst && !Is_local &&
                                                Kernel_traits::kHeadDim <= 128, IsEvenKConst, Split, Append_KV>;
                                if (smem_size >= 32 * 1024) {
                                    C10_CUDA_CHECK(cudaFuncSetAttribute(
                                        kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size));
                                }
...
void run_mha_fwd_splitkv_dispatch(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    if constexpr (Headdim == 128) {
        if (params.seqlen_q == 1) {
            ...
            run_flash_splitkv_fwd<Flash_fwd_kernel_traits<Headdim, kBlockM, kBlockN, 2, true, true, T>>(params, stream);
            return;
        }
...
void run_mha_fwd_hdim32(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    DROPOUT_SWITCH(params.p_dropout < 1.f, Is_dropout, [&] {
        BOOL_SWITCH(params.is_causal, Is_causal, [&] {
            run_flash_fwd<Flash_fwd_kernel_traits<Headdim, 64, 64, 4, true, true, T>, Is_dropout, Is_causal>(params, stream);
            #if 0
...
template<typename T>
void run_mha_fwd_splitkv_hdim32(Flash_fwd_params &params, cudaStream_t stream) {
    constexpr static int kBlockM = 64;
    constexpr static int kBlockN = 64;
    run_flash_splitkv_fwd<Flash_fwd_kernel_traits<32, kBlockM, kBlockN, 4, false, false, T>>(params, stream);
}

template<typename T>
void run_mha_fwd_splitkv_hdim64(Flash_fwd_params &params, cudaStream_t stream) {
    constexpr static int kBlockM = 64;
    constexpr static int kBlockN = 64;
    run_flash_splitkv_fwd<Flash_fwd_kernel_traits<64, kBlockM, kBlockN, 4, false, false, T>>(params, stream);
}

template<typename T>
void run_mha_fwd_splitkv_hdim96(Flash_fwd_params &params, cudaStream_t stream) {
    constexpr static int kBlockM = 64;
    constexpr static int kBlockN = 64;
    run_flash_splitkv_fwd<Flash_fwd_kernel_traits<96, kBlockM, kBlockN, 4, false, false, T>>(params, stream);
}

template<typename T>
void run_mha_fwd_splitkv_hdim128(Flash_fwd_params &params, cudaStream_t stream) {
    if (params.seqlen_q == 1) {
        constexpr static int kBlockM = 32;
        constexpr static int kBlockN = 32;
        run_flash_splitkv_fwd<Flash_fwd_kernel_traits<128, kBlockM, kBlockN, 2, true, true, T>>(params, stream);
        return;
    }
    constexpr static int kBlockM = 64;
    constexpr static int kBlockN = 64;
...
template<typename T>
void run_mha_fwd_splitkv_hdim160(Flash_fwd_params &params, cudaStream_t stream) {
    constexpr static int kBlockM = 64;
    constexpr static int kBlockN = 64;
    run_flash_splitkv_fwd<Flash_fwd_kernel_traits<160, kBlockM, kBlockN, 4, false, false, T>>(params, stream);
}

template<typename T>
void run_mha_fwd_splitkv_hdim192(Flash_fwd_params &params, cudaStream_t stream) {
    constexpr static int kBlockM = 64;
    constexpr static int kBlockN = 64;
    run_flash_splitkv_fwd<Flash_fwd_kernel_traits<192, kBlockM, kBlockN, 4, false, false, T>>(params, stream);
}

template<typename T>
void run_mha_fwd_splitkv_hdim224(Flash_fwd_params &params, cudaStream_t stream) {
    constexpr static int kBlockM = 64;
    constexpr static int kBlockN = 64;
    run_flash_splitkv_fwd<Flash_fwd_kernel_traits<224, kBlockM, kBlockN, 4, false, false, T>>(params, stream);
}

template<typename T>
void run_mha_fwd_splitkv_hdim256(Flash_fwd_params &params, cudaStream_t stream) {
    constexpr static int kBlockM = 64;
    constexpr static int kBlockN = 64;
    run_flash_splitkv_fwd<Flash_fwd_kernel_traits<256, kBlockM, kBlockN, 4, false, false, T>>(params, stream);
}
