...
inline __device__ void apply_attn_mask(Tensor<Engine, Layout> &tensor, const int col_idx_offset_,
                                       ...
                                       const int warp_row_stride, const int head_idx, const float softmax_scale,
                                       const T *bias, const int bias_row_stride) {
...
#pragma unroll
            for (int j = 0; j < size<1, 0>(tensor); ++j) {
                ...
                if (col_idx < max_seqlen_k && row_idx < max_seqlen_q) {
                    tensor(make_coord(0, mi), make_coord(j, nj)) += *(bias + col_idx) / softmax_scale;
                }
            }
        }
    }
