...
template <typename Kernel_traits, bool Is_dropout, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN,
          bool Is_even_K, bool Is_first, bool Is_last, bool Seq_parallel = false, typename Params>
inline __device__ void compute_dq_dk_dv_1colblock_hdim64(const Params &params, const int bidb, const int bidh,
                                                         const int n_block) {
    ...
    Tensor sQ =
        make_tensor(make_smem_ptr(reinterpret_cast<Element *>(smem_)), typename Kernel_traits::SmemLayoutQdO_hdim64{});
    Tensor sQt = make_tensor(sQ.data(), typename Kernel_traits::SmemLayoutQdOtransposed_hdim64{});
    Tensor sQtNoSwizzle = make_tensor(sQ.data(), typename Kernel_traits::SmemLayoutQdOtransposedNoSwizzle{});

    Tensor sdO =
        make_tensor(sQ.data() + (Double_buffer ? 2 : 1) * size(sQ), typename Kernel_traits::SmemLayoutQdO_hdim64{});
    Tensor sdOt = make_tensor(sdO.data(), typename Kernel_traits::SmemLayoutQdOtransposed_hdim64{});
    Tensor sdOtransposedNoSwizzle = make_tensor(sdO.data(), typename Kernel_traits::SmemLayoutQdOtransposedNoSwizzle{});
    Tensor sK = make_tensor(Kernel_traits::Is_K_in_regs ? sQ.data() : sdO.data() + size(sdO),
                            typename Kernel_traits::SmemLayoutKV_hdim64{});
    Tensor sV = make_tensor(Kernel_traits::Is_K_in_regs ? sdO.data() + size(sdO) : sK.data() + size(sK),
                            typename Kernel_traits::SmemLayoutKV_hdim64{});
    Tensor sKt = make_tensor(sK.data(), typename Kernel_traits::SmemLayoutKtransposed_hdim64{});
    Tensor sKtNoSwizzle = make_tensor(sK.data(), typename Kernel_traits::SmemLayoutKtransposedNoSwizzle{});
    Tensor sdS = make_tensor(!Kernel_traits::Is_V_in_regs
                                 ? sV.data() + size(sV)
                                 : (Kernel_traits::Is_K_in_regs ? sdO.data() + size(sdO) : sK.data() + size(sK)),
    ...
    Tensor tdKrQt = thr_mma_dkv.partition_fragment_A(sQtNoSwizzle);
    Tensor tdKrdSt = thr_mma_dkv.partition_fragment_B(sdSt);

    Tensor tdKsQt = smem_thr_copy_PdSt.partition_S(sQt);
    Tensor tdKsdSt = smem_thr_copy_QdOt.partition_S(sdSt);

    Tensor tdVrPt = thr_mma_dkv.partition_fragment_B(sPtNoSwizzle);
    Tensor tdVrdO = thr_mma_dkv.partition_fragment_A(sdOtransposedNoSwizzle);

    Tensor tdVsPt = smem_thr_copy_QdOt.partition_S(sPt);
    Tensor tdVsdOt = smem_thr_copy_PdSt.partition_S(sdOt);
...
#pragma unroll
    ...
    auto stsP_ptr0 = (warp_id + 0) * 4 * 64 + sp_ptr;
    auto stsP_ptr1 = (warp_id + 4) * 4 * 64 + sp_ptr;
    auto stsP_ptr2 = (warp_id + 8) * 4 * 64 + sp_ptr;
    auto stsP_ptr3 = (warp_id + 12) * 4 * 64 + sp_ptr;

    auto row = lane_id / 16;
    auto col = lane_id % 16;
    col ^= (row * 4);

    stsP_ptr0 += row * 64 + 4 * col;
    stsP_ptr1 += row * 64 + 4 * col;
    stsP_ptr2 += row * 64 + 4 * col;
    stsP_ptr3 += row * 64 + 4 * col;
    ...
    auto ldsp_ptr0 = sp_ptr;
    auto ldsp_ptr1 = ldsp_ptr0 + 4 * 64;
    auto ldsp_ptr2 = ldsp_ptr0 + 8 * 64;
    auto ldsp_ptr3 = ldsp_ptr0 + 12 * 64;

    ldsp_ptr0 += lds_row * 64 + lds_col * 4;
    ldsp_ptr1 += lds_row * 64 + lds_col * 4;
    ldsp_ptr2 += lds_row * 64 + lds_col * 4;
    ldsp_ptr3 += lds_row * 64 + lds_col * 4;

    auto sds_ptr = sdS.data().ptr_;
    auto stsdS_ptr0 = (warp_id + 0) * 4 * 64 + sds_ptr;
    auto stsdS_ptr1 = (warp_id + 4) * 4 * 64 + sds_ptr;
    auto stsdS_ptr2 = (warp_id + 8) * 4 * 64 + sds_ptr;
    auto stsdS_ptr3 = (warp_id + 12) * 4 * 64 + sds_ptr;

    stsdS_ptr0 += row * 64 + 4 * col;
    stsdS_ptr1 += row * 64 + 4 * col;
    stsdS_ptr2 += row * 64 + 4 * col;
    stsdS_ptr3 += row * 64 + 4 * col;

    auto ldsdS_ptr0 = sds_ptr;
    auto ldsdS_ptr1 = ldsdS_ptr0 + 4 * 64;
    auto ldsdS_ptr2 = ldsdS_ptr0 + 8 * 64;
    auto ldsdS_ptr3 = ldsdS_ptr0 + 12 * 64;

    ldsdS_ptr0 += lds_row * 64 + lds_col * 4;
    ldsdS_ptr1 += lds_row * 64 + lds_col * 4;
    ldsdS_ptr2 += lds_row * 64 + lds_col * 4;
    ldsdS_ptr3 += lds_row * 64 + lds_col * 4;

    auto sQ_ptr = sQ.data().ptr_;
    ...
    auto ldsQ_ptr0 = sQ_ptr + ldsQ_row * 64 + ldsQ_col * 4;
    auto ldsQ_ptr1 = ldsQ_ptr0 + 1 * 16 * 64;
    auto ldsQ_ptr2 = ldsQ_ptr0 + 2 * 16 * 64;
    auto ldsQ_ptr3 = ldsQ_ptr0 + 3 * 16 * 64;

    auto sdO_ptr = sdO.data().ptr_;

    auto ldsdO_ptr0 = sdO_ptr + ldsQ_row * 64 + ldsQ_col * 4;
    auto ldsdO_ptr1 = ldsdO_ptr0 + 1 * 16 * 64;
    auto ldsdO_ptr2 = ldsdO_ptr0 + 2 * 16 * 64;
    auto ldsdO_ptr3 = ldsdO_ptr0 + 3 * 16 * 64;
...
#pragma unroll
    ...
    flash::Dropout dropout(params.rng_state[0], params.rng_state[1], params.p_dropout_in_uint8_t, bidb, bidh, tidx,
                           params.h);
...
#pragma unroll
        ...
        flash::gemm_opt<false, Kernel_traits::Is_K_in_regs>(acc_s, tSrQ, tSrK, tSsQ, tSsK, tiled_mma_sdp, smem_tiled_copy_QdO,
        ...
        if (Is_dropout) {
            ...
            dropout.template mc_apply_dropout<true>(acc_s, block_row_idx, block_col_idx, AtomLayoutMS);
        }
        ...
        flash::sts_transpose(tPrP, stsP_ptr0, stsP_ptr1, stsP_ptr2, stsP_ptr3);
        ...
        flash::gemm_opt<false, Kernel_traits::Is_V_in_regs>(acc_dp, tdPrdO, tdPrV, tdPsdO, tdPsV, tiled_mma_sdp,
...
#pragma unroll
        ...
        flash::sts_transpose(tdSrdS, stsdS_ptr0, stsdS_ptr1, stsdS_ptr2, stsdS_ptr3);

        __syncthreads();

        lds_transpose(tdVrdO, ldsdO_ptr0, ldsdO_ptr1, ldsdO_ptr2, ldsdO_ptr3);

        gemm_A_in_reg(acc_dv, tdVrdO, tdVrPt, ldsp_ptr0, ldsp_ptr1, ldsp_ptr2, ldsp_ptr3, tiled_mma_dkv);
        ...
        *(reinterpret_cast<uint64_t *>(tdQrdS.data())) = *(reinterpret_cast<uint64_t *>(tdSrdS.data().ptr_));
        *(reinterpret_cast<uint64_t *>(tdQrdS.data() + 4)) = *(reinterpret_cast<uint64_t *>(tdSrdS.data().ptr_ + 4));
        *(reinterpret_cast<uint64_t *>(tdQrdS.data() + 8)) = *(reinterpret_cast<uint64_t *>(tdSrdS.data().ptr_ + 8));
        *(reinterpret_cast<uint64_t *>(tdQrdS.data() + 12)) = *(reinterpret_cast<uint64_t *>(tdSrdS.data().ptr_ + 12));

        flash::gemm_opt<true, Kernel_traits::Is_K_in_regs>(acc_dq, tdQrdS, tdQrKt, tdQsdS, tdQsKt, tiled_mma_dq,
...
#pragma unroll
        ...
        lds_transpose(tdKrQt, ldsQ_ptr0, ldsQ_ptr1, ldsQ_ptr2, ldsQ_ptr3);
        gemm_A_in_reg(acc_dk, tdKrQt, tdKrdSt, ldsdS_ptr0, ldsdS_ptr1, ldsdS_ptr2, ldsdS_ptr3, tiled_mma_dkv);
...
#pragma unroll
    ...
    auto sdK_ptr = sdK.data().ptr_;

    auto stsdKV_col = lane_id % 16;
    auto stsdKV_row = lane_id / 16;

    auto col_col = stsdKV_col / 4;
    auto col_row = stsdKV_col % 4;

    auto stsdKV_offset0 = warp_id * 16;
    auto stsdKV_offset1 = stsdKV_offset0 + 1 * 16 * 64;
    auto stsdKV_offset2 = stsdKV_offset0 + 2 * 16 * 64;
    auto stsdKV_offset3 = stsdKV_offset0 + 3 * 16 * 64;

    stsdKV_offset0 += stsdKV_row * 4 * 64 + col_col * 4 + col_row * 64;
    stsdKV_offset1 += stsdKV_row * 4 * 64 + col_col * 4 + col_row * 64;
    stsdKV_offset2 += stsdKV_row * 4 * 64 + col_col * 4 + col_row * 64;
    stsdKV_offset3 += stsdKV_row * 4 * 64 + col_col * 4 + col_row * 64;

    auto stsdK_ptr0 = sdK_ptr + stsdKV_offset0;
    auto stsdK_ptr1 = sdK_ptr + stsdKV_offset1;
    auto stsdK_ptr2 = sdK_ptr + stsdKV_offset2;
    auto stsdK_ptr3 = sdK_ptr + stsdKV_offset3;

    sts_transpose(taccdKrdK, stsdK_ptr0, stsdK_ptr1, stsdK_ptr2, stsdK_ptr3);

    auto sdV_ptr = sdV.data().ptr_;
    auto stsdV_ptr0 = sdV_ptr + stsdKV_offset0;
    auto stsdV_ptr1 = sdV_ptr + stsdKV_offset1;
    auto stsdV_ptr2 = sdV_ptr + stsdKV_offset2;
    auto stsdV_ptr3 = sdV_ptr + stsdKV_offset3;

    sts_transpose(taccdVrdV, stsdV_ptr0, stsdV_ptr1, stsdV_ptr2, stsdV_ptr3);
