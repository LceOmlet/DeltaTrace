...
__forceinline__ __device__ void compute_dq_dk_dv_1colblock_hdim128_32x64(const Params &params, const int bidb,
    ...
    typename Kernel_traits::GmemTiledCopyQKV_hdim128 gmem_tiled_copy_QKV;
    auto gmem_thr_copy_QKV = gmem_tiled_copy_QKV.get_thread_slice(tidx);
    using GmemTiledCopydO = typename Kernel_traits::GmemTiledCopyQKV_hdim128;
    ...
    Tensor tQsQt = gmem_thr_copy_QKV.partition_D(sQt);
    Tensor tdOgdO = gmem_thr_copy_dO.partition_S(gdO);
    Tensor tdOsdO = gmem_thr_copy_dO.partition_D(sdO);
    Tensor tdOsdOt = gmem_thr_copy_dO.partition_D(sdOt);
    ...
    Tensor tdStsdSt = smem_thr_copy_PdS.partition_D(sdSt);
    ...
    Tensor tdKsdSt = smem_thr_copy_PdSt.partition_S(sdSt_r);
...
#pragma unroll
    ...
    flash::copy<Is_even_MN, Is_even_K, true>(gmem_tiled_copy_dO, tdOgdO, tdOsdOt, tQcQ, tQpQ,
                                             binfo.actual_seqlen_q - m_block * kBlockM);
    flash::copy<Is_even_MN, Is_even_K, true>(gmem_tiled_copy_QKV, tQgQ, tQsQt, tQcQ, tQpQ,
                                             binfo.actual_seqlen_q - m_block * kBlockM);
...
#pragma unroll
        ...
        if (Is_dropout) {
            ...
            dropout.template mc_apply_dropout<true, AtomLayoutNS>(acc_s, block_row_idx, block_col_idx, AtomLayoutMS);
        }
        ...
        __syncthreads();
        flash::stmatrix_trans(tPaP, sP.data().get());
...
#pragma unroll
        ...
        flash::ldmatrix_trans(tdVrPt, sPt.data().get());
        flash::permute_4x4_b16(tdVrdO);
        flash::gemm<true, true>(acc_dv, tdVrPt, tdVrdO, tdVsPt, tdVsdOt, tiled_mma_dkv, smem_tiled_copy_PdSt,
                                smem_tiled_copy_QdOt, smem_thr_copy_PdSt, smem_thr_copy_QdOt);

        cute::copy(tdQsdS, tdQrdS);
        __syncthreads();
        flash::stmatrix_trans(tdSadS, sdSt.data().get());

        if (m_block > m_block_min) {
            tdOgdO.data() = tdOgdO.data() + (-int(kBlockM * params.do_row_stride));
            flash::copy<true, Is_even_K>(gmem_tiled_copy_dO, tdOgdO, tdOsdOt, tQcQ, tQpQ);
        }

        flash::swap(*reinterpret_cast<uint64_t *>(tdQrdS(_, _, 1).data()),
                    *reinterpret_cast<uint64_t *>(tdQrdS(_, _, 2).data()));
        flash::gemm<true, true>(acc_dq, tdQrdS, tdQrKt, tdQsdS, tdQsKt, tiled_mma_dq, smem_tiled_copy_dS,
                                smem_tiled_copy_Kt, smem_thr_copy_dS, smem_thr_copy_Kt);
...
#pragma unroll
        ...
        __syncthreads();
        flash::ldmatrix_trans(tdKrdSt, sdSt.data().get());
        flash::permute_4x4_b16(tdKrQt);
        flash::gemm<true, true>(acc_dk, tdKrdSt, tdKrQt, tdKsdSt, tdKsQt, tiled_mma_dkv, smem_tiled_copy_PdSt,
                                smem_tiled_copy_QdOt, smem_thr_copy_PdSt, smem_thr_copy_QdOt);
        ...
        if (!Double_buffer && m_block > m_block_min) {
            ...
            flash::copy<true, Is_even_K>(gmem_tiled_copy_QKV, tQgQ, tQsQt, tQcQ, tQpQ);
        }
    }
